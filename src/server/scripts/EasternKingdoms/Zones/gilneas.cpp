...
public:
	...
	struct npc_prince_liam_greymane_phase1AI: public ScriptedAI {
		npc_prince_liam_greymane_phase1AI(Creature *c) :
				ScriptedAI(c) {
		}
			...
			if (!me->getVictim() && me->GetPhaseMask() == 1) {
				...
				if (tEmote <= diff) {
					...
					me->HandleEmoteCommand(
							RAND(EMOTE_ONESHOT_TALK, EMOTE_ONESHOT_EXCLAMATION,
									EMOTE_ONESHOT_CRY, EMOTE_ONESHOT_BEG,
									EMOTE_ONESHOT_COWER));
...
public:
	...
	struct npc_panicked_citizen_2AI: public ScriptedAI {
		npc_panicked_citizen_2AI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void LoadWaypoints(Waypoint *waypoints) {
			...
			result[0] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851000 and `point` = 1");
			result[1] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851001 and `point` = 1");
			result[2] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851002 and `point` = 1");
			result[3] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851003 and `point` = 1");
			result[4] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851004 and `point` = 1");
			result[5] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851005 and `point` = 1");
			result[6] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851006 and `point` = 1");
			result[7] =
					WorldDatabase.Query(
							"SELECT `id`, `position_x`, `position_y` FROM waypoint_data WHERE id = 34851007 and `point` = 1");
...
public:
	...
	struct npc_lieutenant_waldenAI: public ScriptedAI {
		npc_lieutenant_waldenAI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	...
	struct npc_gilneas_city_guard_phase1AI: public ScriptedAI {
		npc_gilneas_city_guard_phase1AI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (me->GetGUIDLow() == 3486400) {
				...
				if (tSay <= diff) {
					//Random say
					DoScriptText(
							RAND(SAY_GILNEAS_CITY_GUARD_GATE_1,
									SAY_GILNEAS_CITY_GUARD_GATE_2,
									SAY_GILNEAS_CITY_GUARD_GATE_3), me);
...
public:
	...
	struct npc_gilneas_city_guard_phase2AI: public ScriptedAI {
		npc_gilneas_city_guard_phase2AI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	...
	struct npc_prince_liam_greymane_phase2AI: public ScriptedAI {
		npc_prince_liam_greymane_phase2AI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	...
	struct npc_rampaging_worgenAI: public ScriptedAI {
		npc_rampaging_worgenAI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void DamageDealt(Unit* target, uint32& damage,
				DamageEffectType damageType) {
			if (target->GetEntry() == NPC_GILNEAS_CITY_GUARD
					|| target->GetEntry() == NPC_PRINCE_LIAM_GREYMANE)
				dmgCount++;
		}
...
public:
	...
	struct npc_rampaging_worgen2AI: public ScriptedAI {
		npc_rampaging_worgen2AI(Creature *c) :
				ScriptedAI(c) {
		}
...
public:
	...
	struct npc_frightened_citizenAI: public ScriptedAI {
		npc_frightened_citizenAI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		Paths LoadPaths() {
			...
			result[0] =
					WorldDatabase.Query(
							"SELECT `id`, `point`, `position_x`, `position_y` FROM waypoint_data WHERE id = 349810 ORDER BY `point`");
			result[1] =
					WorldDatabase.Query(
							"SELECT `id`, `point`, `position_x`, `position_y` FROM waypoint_data WHERE id = 349811 ORDER BY `point`");
			if (result[0])
		...
		WayPointID GetNearestPoint(Paths paths) {
			...
			for (uint8 i = 0; i <= PATHS_COUNT - 1; i++) {
				MultiDistanceMeter(paths.paths[i], paths.pointsCount[i],
						dist[i]);
		...
		void UpdateAI(const uint32 diff) {
			...
			if (tSay <= diff && onceSay) {
				if (me->GetEntry() == NPC_FRIGHTENED_CITIZEN_1)
					DoScriptText(
							RAND(SAY_CITIZEN_1, SAY_CITIZEN_2, SAY_CITIZEN_3,
									SAY_CITIZEN_4, SAY_CITIZEN_5, SAY_CITIZEN_6,
									SAY_CITIZEN_7, SAY_CITIZEN_8), me);
				else
					DoScriptText(
							RAND(SAY_CITIZEN_1b, SAY_CITIZEN_2b, SAY_CITIZEN_3b,
									SAY_CITIZEN_4b, SAY_CITIZEN_5b), me);
				onceSay = false;
			} else
				tSay -= diff;
			...
			if (tRun2 <= diff) {
				if (onceGet) {
					...
					if (me->GetDistance2d(
							paths.paths[nearestPointID.pathID][nearestPointID.pointID].x,
							paths.paths[nearestPointID.pathID][nearestPointID.pointID].y)
							> 1)
					...
					if (nearestPointID.pointID
							>= paths.pointsCount[nearestPointID.pathID])
