    ...
    struct boss_chogallAI : public BossAI
    {
        ...
        void SummonCreation(uint32 entry, uint32 num)
        {
            for (uint32 i = 0; i < num; ++i)
            {
                ...
                CreationPos[pos][3], TEMPSUMMON_CORPSE_DESPAWN, 60000);
            }
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (IsHeroic())
            {
                ...
                {
                    if (GameObject* pGoFloor =  me->GetMap()->GetGameObject(instance->GetData64(GO_CHOGALL_FLOOR)))
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SUMMON:
                    ...
                    {
                        DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100,true), SPELL_SUMMON_CORRUPTING_ADHERENT);
                        /*SelectTargetList(playerList, RAID_MODE(TARGETS_10_H,TARGETS_25_H), SELECT_TARGET_RANDOM, 0, true);
                        if (true )
                ...
                if (IsHeroic())
                {
                    case EVENT_FLAMING_SUMMON:
                        SummonFlamming(NPC_FIRE_LORD, 0);
                ...
                if (IsHeroic())
                {
                    case EVENT_SUMMON_SHADOW:
                        SummonShadow(NPC_SHADOW_LORD, 0);
