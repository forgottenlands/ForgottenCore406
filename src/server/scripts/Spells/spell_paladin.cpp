...
public:
	...
	class spell_pal_ardent_defender_AuraScript: public AuraScript {
		...
		void Register() {
			DoEffectCalcAmount +=
					AuraEffectCalcAmountFn(spell_pal_ardent_defender_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
			OnEffectAbsorb +=
					AuraEffectAbsorbFn(spell_pal_ardent_defender_AuraScript::Absorb, EFFECT_0);
		}
	};
...
public:
	...
	class spell_pal_blessing_of_faith_SpellScript: public SpellScript {
		...
		void Register() {
			...
			OnEffect +=
					SpellEffectFn(spell_pal_blessing_of_faith_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
		}
	};
...
public:
	...
	class spell_pal_holy_shock_SpellScript: public SpellScript 
    {
		...
		bool Validate(SpellEntry const *spellEntry) 
        {
			...
			if (sSpellMgr->GetFirstSpellInChain(PALADIN_SPELL_HOLY_SHOCK_R1)
...
public:
	...
	class spell_pal_judgements_of_the_bold_AuraScript: public AuraScript {
		...
		void CalculateMana(AuraEffect const* /*aurEff*/, int32& amount,
				bool& canBeRecalculated) {
		...
		void Register() {
			DoEffectCalcAmount +=
					AuraEffectCalcAmountFn(spell_pal_judgements_of_the_bold_AuraScript::CalculateMana, EFFECT_0, SPELL_AURA_PERIODIC_ENERGIZE);
		}
	};
...
public:
	...
	class spell_pal_word_of_glory_heal_SpellScript: public SpellScript {
		...
		void ChangeHeal(SpellEffIndex /*effIndex*/) {
			...
			if (GetCaster()->HasAura(SPELL_DIVINE_PURPOSE_PROC)) {
				...
				if (AuraEffect const* auraEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_PALADIN, 3924, EFFECT_0))
					if (target != caster)
						totalheal += totalheal
								+ (totalheal * auraEff->GetAmount()) / 100;
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_pal_word_of_glory_heal_SpellScript::ChangeHeal, EFFECT_0, SPELL_EFFECT_HEAL);
		}
	};
...
public:
	...
	class spell_pal_selfless_healer_AuraScript: public AuraScript {
		...
		void CalculateBonus(AuraEffect const* /*aurEff*/, int32& amount,
				bool& canBeRecalculated) {
			if (Unit* caster = GetCaster()) {
				canBeRecalculated = true;
		...
		void Register() {
			DoEffectCalcAmount +=
					AuraEffectCalcAmountFn(spell_pal_selfless_healer_AuraScript::CalculateBonus, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
		}
	};
