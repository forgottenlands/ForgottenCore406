...
void LoginDatabaseConnection::DoPrepareStatements() {
    ...
    PREPARE_STATEMENT(
            ...
            CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_GET_IPBANNED,
            "SELECT * FROM ip_banned WHERE ip = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(
            LOGIN_SET_IPAUTOBANNED,
            "INSERT INTO ip_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Trinity realmd', 'Failed login autoban')",
            CONNECTION_ASYNC);
    PREPARE_STATEMENT(
            LOGIN_GET_ACCBANNED,
    ...
    PREPARE_STATEMENT(
            ...
            "SELECT a.sessionkey, a.id, aa.gmlevel  FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE username = ?",
            CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SET_VS,
    ...
    PREPARE_STATEMENT(
            ...
            "SELECT a.sha_pass_hash, a.id, a.locked, a.last_ip, aa.gmlevel, a.v, a.s FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.username = ?",
            CONNECTION_SYNCH);
    PREPARE_STATEMENT(
            LOGIN_SET_FAILEDLOGINS,
            "UPDATE account SET failed_logins = failed_logins + 1 WHERE username = ?",
            CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_GET_FAILEDLOGINS,
    ...
    PREPARE_STATEMENT(
            ...
            CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_BY_IP,
            "SELECT id FROM account WHERE last_ip = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(
            LOGIN_SET_IP_BANNED,
            "INSERT INTO ip_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, ?, ?)",
            CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SET_IP_NOT_BANNED,
            "DELETE FROM ip_banned WHERE ip = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(
