...
void CharacterDatabaseConnection::DoPrepareStatements() {
	...
	PREPARE_STATEMENT(CHAR_DEL_QUEST_POOL_SAVE,
			"DELETE FROM pool_quest_save WHERE pool_id = ?", CONNECTION_ASYNC);
	...
	PREPARE_STATEMENT(
			...
			"DELETE FROM group_member WHERE NOT EXISTS (SELECT guid FROM characters WHERE guid=memberGuid)",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_DEL_LEADERLESS_GROUPS,
			"DELETE FROM groups WHERE NOT EXISTS (SELECT guid FROM characters WHERE guid=leaderGuid)",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_GET_GUID_BY_NAME,
			"SELECT guid FROM characters WHERE name = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(
			CHAR_ADD_BAN,
	...
	PREPARE_STATEMENT(
			CHAR_GET_BANINFO,
			"SELECT FROM_UNIXTIME(bandate), unbandate-bandate, active, unbandate, banreason, bannedby FROM character_banned WHERE guid = ? ORDER BY bandate ASC",
			CONNECTION_SYNCH);
	PREPARE_STATEMENT(
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_WEKLYQUESTSTATUS,
	...
	PREPARE_STATEMENT(
			...
			"SELECT a.button, a.action, a.type FROM character_action as a, characters as c WHERE a.guid = c.guid AND a.spec = c.activespec AND a.guid = ? ORDER BY button",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_GUILD,
	...
	PREPARE_STATEMENT(
			...
			"SELECT setguid, setindex, name, iconname, item0, item1, item2, item3, item4, item5, item6, item7, item8, "
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_TALENTS,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_SKILLS,
			"SELECT skill, value, max FROM character_skills WHERE guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_RANDOMBG,
			"SELECT guid FROM character_battleground_random WHERE guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_BANNED,
			"SELECT guid FROM character_banned WHERE guid = ? AND active = 1",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_QUESTSTATUSREW,
	...
	PREPARE_STATEMENT(CHAR_LOAD_ACCOUNT_DATA,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_AUCTION, "DELETE FROM auctionhouse WHERE id = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_ADD_MAIL,
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_MAIL, "DELETE FROM mail WHERE id = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_ADD_MAIL_ITEM,
	...
	PREPARE_STATEMENT(
			...
			"SELECT item_guid, itemEntry FROM mail_items mi INNER JOIN item_instance ii ON ii.guid = mi.item_guid WHERE mail_id = ?",
			CONNECTION_SYNCH);
	PREPARE_STATEMENT(
			CHAR_SET_MAIL_RETURNED,
			"UPDATE mail SET sender = ?, receiver = ?, expire_time = ?, deliver_time = ?, cod = 0, checked = ? WHERE id = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SET_MAIL_ITEM_RECEIVER,
			"UPDATE mail_items SET receiver = ? WHERE item_guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SET_ITEM_OWNER,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_DEL_ITEM_BOP_TRADE,
			"DELETE FROM item_soulbound_trade_data WHERE itemGuid = ? LIMIT 1",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_ADD_ITEM_BOP_TRADE,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_INVENTORY_ITEM,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_ITEM_INSTANCE,
			"DELETE FROM item_instance WHERE guid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPDATE_GIFT_OWNER,
			"UPDATE character_gifts SET guid = ? WHERE item_guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_GIFT,
			"DELETE FROM character_gifts WHERE item_guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_TALENTBRANCHSPECS,
			"SELECT branchSpec, spec FROM character_branchspec WHERE guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_GET_ACCOUNT_BY_NAME,
			"SELECT account FROM characters WHERE name = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_DEL_ACCOUNT_INSTANCE_LOCK_TIMES,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_LOAD_PLAYER_NAME_CLASS,
	...
	PREPARE_STATEMENT(
			CHAR_ADD_CHANNEL,
			"INSERT INTO channels(name, team, lastUsed) VALUES (?, ?, UNIX_TIMESTAMP())",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_SET_CHANNEL,
	...
	PREPARE_STATEMENT(
			CHAR_ADD_EQUIP_SET,
			"INSERT INTO character_equipmentsets (guid, setguid, setindex, name, iconname, item0, item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11, item12, item13, item14, item15, item16, item17, item18) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_EQUIP_SET,
			"DELETE FROM character_equipmentsets WHERE setguid=?",
			CONNECTION_ASYNC);

	// Auras
	PREPARE_STATEMENT(CHAR_DEL_AURA,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPDATE_INSTANCE_DATA,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_PLAYER_BGDATA,
			"DELETE FROM character_battleground_data WHERE guid = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_RESET_PLAYERS_BGDATA,
			"UPDATE character_battleground_data SET instanceId = 0",
			CONNECTION_SYNCH);

	// Petitions
	PREPARE_STATEMENT(CHAR_LOAD_PETITION,
			"SELECT ownerguid, name FROM petition WHERE petitionguid = ?",
			CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_LOAD_PETITION_SIGNATURE,
	...
	PREPARE_STATEMENT(CHAR_DEL_ARENA_TEAM_MEMBERS,
			"DELETE FROM arena_team_member WHERE arenaTeamId = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPDATE_ARENA_TEAM_CAPTAIN,
			"UPDATE arena_team SET captainGuid = ? WHERE arenaTeamId = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_ARENA_TEAM_MEMBER,
	...
	PREPARE_STATEMENT(
			...
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_CORPSE,
			"DELETE FROM corpse WHERE corpseGuid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_PLAYER_CORPSES,
			"DELETE FROM corpse WHERE guid = ? AND corpseType <> 0",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
	...
	PREPARE_STATEMENT(CHAR_DEL_CREATURE_RESPAWN,
			"DELETE FROM creature_respawn WHERE guid = ? AND instanceId = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_ADD_CREATURE_RESPAWN,
			"REPLACE INTO creature_respawn (guid, respawnTime, instanceId) VALUES (?, ?, ?)",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
	...
	// Gameobject respawn
	PREPARE_STATEMENT(CHAR_DEL_GO_RESPAWN,
			"DELETE FROM gameobject_respawn WHERE guid = ? AND instanceId = ?",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
			CHAR_ADD_GO_RESPAWN,
			"REPLACE INTO gameobject_respawn (guid, respawnTime, instanceId) VALUES (?, ?, ?)",
			CONNECTION_ASYNC);
	PREPARE_STATEMENT(
