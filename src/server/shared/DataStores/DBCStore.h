...
struct SqlDbc {
	...
	int32 indexPos;
	int32 sqlIndexPos;
	SqlDbc(const std::string * _filename, const std::string * _format,
			const std::string * _idname, const char * fmt) :
...
public:
	explicit DBCStorage(const char *f) :
			fmt(f), nCount(0), fieldCount(0), indexTable(NULL), m_dataTable(
					NULL) {
	}
	~DBCStorage() {
		Clear();
	...
	bool Load(char const* fn, SqlDbc * sql) {
		...
		if (sql) {
			...
			if (result) {
				...
				if (int32(result->GetFieldCount() - 1) < sql->sqlIndexPos) {
					sLog->outError("Invalid index pos for dbc:'%s'",
							sql->sqlTableName.c_str());
					return false;
				}
			}
		}
		...
		m_dataTable = (T*) dbc.AutoProduceData(fmt, nCount,
				(char**&) indexTable, sqlRecordCount, sqlHighestIndex,
				sqlDataTable);

		m_stringPoolList.push_back(
				dbc.AutoProduceStrings(fmt, (char*) m_dataTable));
			...
			if (indexTable) {
				...
				do {
					...
					if (sql->indexPos >= 0) {
						...
						if (indexTable[id]) {
							sLog->outError(
									"Index %d already exists in dbc:'%s'", id,
									sql->sqlTableName.c_str());
							return false;
						}
					...
					for (; columnNumber < sql->formatString->size();
							++columnNumber) {
						if ((*sql->formatString)[columnNumber]
								== FT_SQL_ABSENT) {
							...
							case FT_IND:
							case FT_INT:
								*((uint32*) (&sqlDataTable[offset])) = uint32(
										0);
								offset += 4;
								break;
							case FT_BYTE:
							...
							case FT_STRING:
								...
								*((char**) (&sqlDataTable[offset])) =
										m_stringPoolList.back();
								offset += sizeof(char*);
								break;
							}
						} else if ((*sql->formatString)[columnNumber]
								== FT_SQL_PRESENT) {
							bool validSqlColumn = true;
							switch (fmt[columnNumber]) {
							case FT_FLOAT:
								*((float*) ((&sqlDataTable[offset]))) =
										fields[sqlColumnNumber].GetFloat();
								offset += 4;
								break;
							case FT_IND:
							case FT_INT:
								*((uint32*) ((&sqlDataTable[offset]))) =
										fields[sqlColumnNumber].GetUInt32();
								offset += 4;
								break;
							case FT_BYTE:
								*((uint8*) ((&sqlDataTable[offset]))) =
										fields[sqlColumnNumber].GetUInt8();
								offset += 1;
								break;
							case FT_STRING:
								sLog->outError(
							...
							if (validSqlColumn
									&& (columnNumber
											!= (sql->formatString->size() - 1)))
								sqlColumnNumber++;
						} else {
							sLog->outError(
									"Incorrect sql format string '%s' at char %d",
									sql->sqlTableName.c_str(), columnNumber);
							return false;
						}
					}
