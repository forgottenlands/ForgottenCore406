...
bool TileAssembler::convertWorld2() {
	...
	for (MapData::iterator map_iter = mapData.begin();
			map_iter != mapData.end() && success; ++map_iter) {
		// build global map tree
		std::vector<ModelSpawn*> mapSpawns;
		UniqueEntryMap::iterator entry;
		printf("Calculating model bounds for map %u...\n", map_iter->first);
		for (entry = map_iter->second->UniqueEntries.begin();
				entry != map_iter->second->UniqueEntries.end(); ++entry) {
			...
			if (entry->second.flags & MOD_M2) {
				...
				entry->second.iBound = entry->second.iBound
		...
		for (TileMap::iterator glob = globalRange.first;
				glob != globalRange.second && success; ++glob) {
			success = ModelSpawn::writeToFile(mapfile,
					map_iter->second->UniqueEntries[glob->second]);
		}
		...
		for (tile = tileEntries.begin(); tile != tileEntries.end(); ++tile) {
			...
			for (uint32 s = 0; s < nSpawns; ++s) {
				...
				std::map<uint32, uint32>::iterator nIdx = modelNodeIdx.find(
						spawn2.ID);
				if (success
	...
	for (std::set<std::string>::iterator mfile = spawnedModelFiles.begin();
			mfile != spawnedModelFiles.end(); ++mfile) {
...
bool TileAssembler::readMapSpawns() {
	...
	while (!feof(dirf)) {
		...
		current->UniqueEntries.insert(
				pair<uint32, ModelSpawn>(spawn.ID, spawn));
		current->TileEntries.insert(
				pair<uint32, uint32>(StaticMapTree::packTileID(tileX, tileY),
						spawn.ID));
	}
...
#define CMP_OR_RETURN(V,S)  if(strcmp((V),(S)) != 0)        { \
	...
	for (uint32 g = 0; g < groups; ++g) // should be only one for M2 files...
		...
		for (uint32 i = 0, indexNo = 0; indexNo < nvectors; indexNo++, i += 3) {
			Vector3 v = Vector3(vectorarray[i + 0], vectorarray[i + 1],
					vectorarray[i + 2]);
...
#define CMP_OR_RETURN(V,S)  if(strcmp((V),(S)) != 0)        { \
	...
	for (uint32 g = 0; g < groups; ++g) {
		...
		if (liquidflags & 1) {
			...
			liquid = new WmoLiquid(hlq.xtiles, hlq.ytiles,
					Vector3(hlq.pos_x, hlq.pos_y, hlq.pos_z), hlq.type);
		...
		groupsArray.push_back(
				GroupModel(mogpflags, GroupWMOID,
						AABox(Vector3(bbox1), Vector3(bbox2))));
