...
#ifndef ARKCORECORE_GUILD_H
#define ARKCORECORE_GUILD_H
...
enum GuildMemberFlags {
	...
	GUILD_MEMBER_FLAG_MOBILE = 0x08,
// Not used; On retail it's set if the plr is logged on with Mobile Armory
};
...
enum GuildEvents {
	...
	GE_GUILDBANK_TEXT_CHANGED = 0x17,
...
enum GuildBankRights {
	...
	GUILD_BANK_RIGHT_DEPOSIT_ITEM = GUILD_BANK_RIGHT_VIEW_TAB
...
public:
	EmblemInfo() :
			m_style(0), m_color(0), m_borderStyle(0), m_borderColor(0), m_backgroundColor(
					0) {
	}
	...
	void ReadPacket(WorldPacket& recv) {
		recv >> m_style >> m_color >> m_borderStyle >> m_borderColor
				>> m_backgroundColor;
	}
...
struct GuildBankRightsAndSlots {
	...
	GuildBankRightsAndSlots(uint8 _rights, uint32 _slots) :
			rights(_rights), slots(_slots) {
	}
...
private:
	...
	public:
		Member(uint32 guildId, const uint64& guid, uint8 rankId) :
				m_guildId(guildId), m_guid(guid), m_flags(
						GUILD_MEMBER_FLAG_OFFLINE), m_logoutTime(::time(NULL)), m_rankId(
						rankId) {
		}

		void SetStats(Player* player);
		void SetStats(const std::string& name, uint8 level, uint8 _class,
		...
		void ResetFlags() {
			m_flags = GUILD_MEMBER_FLAG_OFFLINE;
		}
		...
		uint64 GetLogoutTime() {
			return m_logoutTime;
		}
		std::string GetPublicNote() {
			return m_publicNote;
		}
		std::string GetOfficerNote() {
		...
		bool IsOnline() {
			return (m_flags & GUILD_MEMBER_FLAG_ONLINE);
		}
		...
		inline bool IsRank(uint8 rankId) const {
			return m_rankId == rankId;
		}
		inline bool IsRankNotLower(uint8 rankId) const {
			return m_rankId <= rankId;
		}
		inline bool IsSamePlayer(const uint64& guid) const {
			return m_guid == guid;
		}

		void DecreaseBankRemainingValue(SQLTransaction& trans, uint8 tabId,
				uint32 amount);
		...
		inline Player* FindPlayer() const {
			return sObjectMgr->GetPlayer(m_guid);
		}
	...
	public:
		LogEntry(uint32 guildId, uint32 guid) :
				m_guildId(guildId), m_guid(guid), m_timestamp(::time(NULL)) {
		}
	...
	public:
		EventLogEntry(uint32 guildId, uint32 guid, GuildEventLogTypes eventType,
				uint32 playerGuid1, uint32 playerGuid2, uint8 newRank) :
				LogEntry(guildId, guid), m_eventType(eventType), m_playerGuid1(
						playerGuid1), m_playerGuid2(playerGuid2), m_newRank(
						newRank) {
		}

		EventLogEntry(uint32 guildId, uint32 guid, time_t timestamp,
				GuildEventLogTypes eventType, uint32 playerGuid1,
				uint32 playerGuid2, uint8 newRank) :
				LogEntry(guildId, guid, timestamp), m_eventType(eventType), m_playerGuid1(
						playerGuid1), m_playerGuid2(playerGuid2), m_newRank(
						newRank) {
		}
	...
	public:
		...
		BankEventLogEntry(uint32 guildId, uint32 guid,
				GuildBankEventLogTypes eventType, uint8 tabId,
				uint32 playerGuid, uint32 itemOrMoney, uint16 itemStackCount,
				uint8 destTabId) :
				LogEntry(guildId, guid), m_eventType(eventType), m_bankTabId(
						tabId), m_playerGuid(playerGuid), m_itemOrMoney(
						itemOrMoney), m_itemStackCount(itemStackCount), m_destTabId(
						destTabId) {
		}

		BankEventLogEntry(uint32 guildId, uint32 guid, time_t timestamp,
				uint8 tabId, GuildBankEventLogTypes eventType,
				uint32 playerGuid, uint32 itemOrMoney, uint16 itemStackCount,
				uint8 destTabId) :
				LogEntry(guildId, guid, timestamp), m_eventType(eventType), m_bankTabId(
						tabId), m_playerGuid(playerGuid), m_itemOrMoney(
						itemOrMoney), m_itemStackCount(itemStackCount), m_destTabId(
						destTabId) {
		}
	...
	class LogHolder {
	public:
		LogHolder(uint32 guildId, uint32 maxRecords) :
				m_guildId(guildId), m_maxRecords(maxRecords), m_nextGUID(
						GUILD_EVENT_LOG_GUID_UNDEFINED) {
		}
		~LogHolder();

		uint8 GetSize() const {
			return uint8(m_log.size());
		}
	...
	public:
		RankInfo(uint32 guildId) :
				m_guildId(guildId), m_rankId(GUILD_RANK_NONE), m_rights(
						GR_RIGHT_EMPTY), m_bankMoneyPerDay(0) {
		}
		RankInfo(uint32 guildId, uint8 rankId, const std::string& name,
				uint32 rights, uint32 money) :
				m_guildId(guildId), m_rankId(rankId), m_name(name), m_rights(
						rights), m_bankMoneyPerDay(money) {
		}
		...
		uint32 GetBankMoneyPerDay() const {
			return m_rankId == GR_GUILDMASTER ?
					GUILD_WITHDRAW_MONEY_UNLIMITED : m_bankMoneyPerDay;
		}
		void SetBankMoneyPerDay(uint32 money);

		inline uint8 GetBankTabRights(uint8 tabId) const {
			return tabId < GUILD_BANK_MAX_TABS ?
					m_bankTabRightsAndSlots[tabId].rights : 0;
		}
		inline uint32 GetBankTabSlotsPerDay(uint8 tabId) const {
			if (tabId < GUILD_BANK_MAX_TABS)
				return m_rankId == GR_GUILDMASTER ?
						GUILD_WITHDRAW_SLOT_UNLIMITED :
						m_bankTabRightsAndSlots[tabId].slots;
			return 0;
		}
		void SetBankTabSlotsAndRights(uint8 tabId,
	...
	public:
		...
		inline Item* GetItem(uint8 slotId) const {
			return slotId < GUILD_BANK_MAX_SLOTS ? m_items[slotId] : NULL;
		}
	...
	class MoveItemData {
	public:
		MoveItemData(Guild* pGuild, Player* player, uint8 container,
				uint8 slotId) :
				m_pGuild(pGuild), m_pPlayer(player), m_container(container), m_slotId(
						slotId), m_pItem(NULL), m_pClonedItem(NULL) {
		}
		...
		virtual void RemoveItem(SQLTransaction& trans, MoveItemData* pOther,
				uint32 splitedAmount = 0) = 0;
		// Saves item to container
		virtual Item* StoreItem(SQLTransaction& trans, Item* pItem) = 0;
		// Log bank event
		virtual void LogBankEvent(SQLTransaction& trans, MoveItemData* pFrom,
	...
	public:
		PlayerMoveItemData(Guild* pGuild, Player* player, uint8 container,
				uint8 slotId) :
		...
		bool InitItem();
		void RemoveItem(SQLTransaction& trans, MoveItemData* pOther,
				uint32 splitedAmount = 0);
		Item* StoreItem(SQLTransaction& trans, Item* pItem);
		void LogBankEvent(SQLTransaction& trans, MoveItemData* pFrom,
				uint32 count) const;
	protected:
	...
	public:
		BankMoveItemData(Guild* pGuild, Player* player, uint8 container,
				uint8 slotId) :
				MoveItemData(pGuild, player, container, slotId) {
		}
	...
	private:
		Item* _StoreItem(SQLTransaction& trans, BankTab* pTab, Item *pItem,
				ItemPosCount& pos, bool clone) const;
		bool _ReserveSpace(uint8 slotId, Item* pItem, Item* pItemDest,
				uint32& count);
		void _CanStoreItemInTab(Item* pItem, uint8 skipSlotId, bool merge,
				uint32& count);
	};
