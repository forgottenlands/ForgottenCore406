...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec,
		...
		uint32 recruiter) :
		m_muteTime(mute_time), m_timeOutTime(0), _player(NULL), m_Socket(sock), _security(
				sec), _ispremium(ispremium), _accountId(id), m_expansion(expansion), _logoutTime(0), m_inQueue(
				false), m_playerLoading(false), m_playerLogout(false), m_playerRecentlyLogout(
				false), m_playerSave(false), m_sessionDbcLocale(
				sWorld->GetAvailableDbcLocale(locale)), m_sessionDbLocaleIndex(
				locale), m_latency(0), m_TutorialsChanged(false), recruiterId(
				recruiter) {
	if (sock) {
		...
		LoginDatabase.PExecute(
				"UPDATE account SET online = '%u' WHERE id = %u;", realmID,
				GetAccountId());
	}
}
...
void WorldSession::SizeError(WorldPacket const& packet, uint32 size) const {
	sLog->outError(
			"Client (account %u) send packet %s (%u) with size " SIZEFMTD " but expected %u (attempt crash server?), skipped",
			GetAccountId(), LookupOpcodeName(packet.GetOpcode()),
			packet.GetOpcode(), packet.size(), size);
}
...
void WorldSession::LogUnprocessedTail(WorldPacket *packet) {
	sLog->outError(
			"SESSION: opcode %s (0x%.4X) have unprocessed tail data (read stop at %u from %u)",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater) {
	...
	while (m_Socket && !m_Socket->IsClosed() && _recvQueue.next(packet, updater)) {
		...
		sLog->outDebug(
				...
				"SESSION: Received opcode 0x%.4X (%s)",
				packet->GetOpcode(),
				packet->GetOpcode() > OPCODE_NOT_FOUND ?
						"nf" : LookupOpcodeName(packet->GetOpcode()));
				...
				case STATUS_LOGGEDIN:
					if (!_player) {
						...
						if (!m_playerRecentlyLogout)
							LogUnexpectedOpcode(packet, "STATUS_LOGGEDIN",
									"the player has not logged in yet");
					} else if (_player->IsInWorld()) {
						sScriptMgr->OnPacketReceive(m_Socket,
								WorldPacket(*packet));
	...
	if (_player) {
		...
		if (_player->GetDeathTimer()) {
			...
			for (Unit::AttackerSet::const_iterator itr =
					_player->getAttackers().begin();
					itr != _player->getAttackers().end(); ++itr) {
			...
			for (std::set<Player*>::const_iterator itr = aset.begin();
					itr != aset.end(); ++itr)
		...
		for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
			if (BattlegroundQueueTypeId bgQueueTypeId = _player->GetBattlegroundQueueTypeId(i)) {
				...
				sBattlegroundMgr->m_BattlegroundQueues[bgQueueTypeId].RemovePlayer(
		...
		if (Save) {
			...
			for (int j = BUYBACK_SLOT_START; j < BUYBACK_SLOT_END; ++j) {
				...
				_player->SetUInt64Value(
						PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + (eslot * 2), 0);
		...
		sSocialMgr->SendFriendStatus(_player, FRIEND_OFFLINE,
				_player->GetGUIDLow(), true);
		...
		CharacterDatabase.PExecute(
				"UPDATE characters SET online = 0 WHERE account = '%u'",
				GetAccountId());
		sLog->outDebug(LOG_FILTER_NETWORKIO,
...
void WorldSession::Handle_NULL(WorldPacket& recvPacket) {
	sLog->outError("SESSION: received unhandled opcode %s (0x%.4X)",
			LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask) {
	...
	do {
		...
		if (type >= NUM_ACCOUNT_DATA_TYPES) {
			sLog->outError(
					"Table `%s` have invalid account data type (%u), ignore.",
					mask == GLOBAL_CACHE_MASK ?
		...
		if ((mask & (1 << type)) == 0) {
			sLog->outError(
					"Table `%s` have non appropriate for table  account data type (%u), ignore.",
					mask == GLOBAL_CACHE_MASK ?
							"account_data" : "character_account_data", type);
			continue;
		}
...
void WorldSession::SetAccountData(AccountDataType type, time_t time_,
		std::string data) {
	if ((1 << type) & GLOBAL_CACHE_MASK) {
		...
		trans->PAppend(
				"DELETE FROM account_data WHERE account='%u' AND type='%u'",
				acc, type);
		CharacterDatabase.EscapeString(data);
		trans->PAppend(
				"INSERT INTO account_data VALUES ('%u', '%u', '%u', '%s')", acc,
				type, (uint32) time_, data.c_str());
		...
		SQLTransaction trans = CharacterDatabase.BeginTransaction();
		trans->PAppend(
				"DELETE FROM character_account_data WHERE guid='%u' AND type='%u'",
				m_GUIDLow, type);
		CharacterDatabase.EscapeString(data);
		trans->PAppend(
				"INSERT INTO character_account_data VALUES ('%u', '%u', '%u', '%s')",
				m_GUIDLow, type, (uint32) time_, data.c_str());
		CharacterDatabase.CommitTransaction(trans);
	}
...
void WorldSession::SaveTutorialsData(SQLTransaction& trans) {
	...
	QueryResult result = CharacterDatabase.PQuery(
			"SELECT count(*) AS r FROM character_tutorial WHERE account = '%u'",
			GetAccountId());
	if (result)
	...
	if (Rows)
		trans->PAppend(
				"UPDATE character_tutorial SET tut0='%u', tut1='%u', tut2='%u', tut3='%u', tut4='%u', tut5='%u', tut6='%u', tut7='%u' WHERE account = '%u'",
				m_Tutorials[0], m_Tutorials[1], m_Tutorials[2], m_Tutorials[3],
				m_Tutorials[4], m_Tutorials[5], m_Tutorials[6], m_Tutorials[7],
				GetAccountId());
	else
		trans->PAppend(
				"INSERT INTO character_tutorial (account, tut0, tut1, tut2, tut3, tut4, tut5, tut6, tut7) VALUES ('%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
				GetAccountId(), m_Tutorials[0], m_Tutorials[1], m_Tutorials[2],
				m_Tutorials[3], m_Tutorials[4], m_Tutorials[5], m_Tutorials[6],
				m_Tutorials[7]);
...
void WorldSession::ReadAddonsInfo(WorldPacket &data) {
	...
	if (size > 0xFFFFF) {
		sLog->outError(
				"WorldSession::ReadAddonsInfo addon info too big, size %u",
				size);
		return;
	}
	...
	if (uncompress(const_cast<uint8*>(addonInfo.contents()), &uSize,
			const_cast<uint8*>(data.contents() + pos),
			data.size() - pos) == Z_OK) {
		uint32 addonsCount;
...
void WorldSession::SendAddonsInfo() {
	uint8 addonPublicKey[256] = {  };

	WorldPacket data(SMSG_ADDON_INFO, 4);

	for (AddonsList::iterator itr = m_addonsList.begin();
			itr != m_addonsList.end(); ++itr) {
