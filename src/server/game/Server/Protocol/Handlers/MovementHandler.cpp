...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data) {
    ...
    // ---- anti-cheat features -->>>
   if (plMover && (plMover->m_transport == 0) && sWorld->GetMvAnticheatEnable() && GetPlayer()->GetSession()->GetSecurity() <= sWorld->GetMvAnticheatGmLevel() 
       && GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType()!=FLIGHT_MOTION_TYPE
       && // POINT_MOTION_TYPE 8 = charge; TARGETED_MOTION_TYPE 5=chase,follow (Mind Control by NPCs?) - better use "IDLE_MOTION_TYPE"
       (plMover->Anti__GetLastTeleTime() + sWorld->GetMvAnticheatIgnoreAfterTeleport()) < time(NULL))
   {
        ...
        if(getMSTimeDiff(GetPlayer()->m_anti_LastLenCheck,CurTime) >= 500)
        {
           ...
           {
               GetPlayer()->Anti__CheatOccurred("Speed hack", delta_xyt, opcode, GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType(), getMSTimeDiff(plMover->m_movementInfo.time,movementInfo.time),&movementInfo);
           }
        }
       ...
       if(delta > 100.0f &&
           ((movementInfo.flags & MOVEMENTFLAG_ONTRANSPORT) == 0 || (GetPlayer()->GetZoneId() != 2257 && GetPlayer()->GetAreaId() != 3992 && GetPlayer()->GetAreaId() != 3988 && GetPlayer()->GetAreaId() != 2618 && GetPlayer()->GetZoneId() != 4384)))
       {
           GetPlayer()->Anti__CheatOccurred("Tele hack", delta, opcode, GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType() /*Val1*/, getMSTimeDiff(plMover->m_movementInfo.time,movementInfo.time) /*Val2*/, &movementInfo /*MvInfo*/, true /*ForceReport*/);
       }

       // Check for waterwalking
       if(((movementInfo.flags & MOVEMENTFLAG_WATERWALKING) != 0) && ((movementInfo.flags ^ MOVEMENTFLAG_WATERWALKING) != 0)
           && ((movementInfo.flags & MOVEMENTFLAG_JUMPING) == 0) // Client sometimes set waterwalk where it shouldn't do that... 
       ...
       if (!GetPlayer()->CanFly() && !GetPlayer()->GetBaseMap()->IsUnderWater(movementInfo.pos.GetPositionX(), movementInfo.pos.GetPositionY(), movementInfo.pos.GetPositionZ()-4.0f)
           && Anti__MapZ < GetPlayer()->GetPositionZ() && Anti__MapZ > (INVALID_HEIGHT+DIFF_OVERGROUND + 5.0f))
       {
           ...
           {
               GetPlayer()->Anti__CheatOccurred("Fly hack", GetPlayer()->GetPositionZ()-Anti__MapZ, opcode,GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType(),
       ...
       if(Anti__FloorZ < -199900.0f && Anti__GroundZ >= -199900.0f && GetPlayer()->GetPositionZ()+5.0f < Anti__GroundZ
          ...
          (movementInfo.flags & MOVEMENTFLAG_FALLING) == 0)
       {
           GetPlayer()->Anti__CheatOccurred("Teleport2Plane hack", Anti__GroundZ,opcode,GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType(),0,&movementInfo);
       }
    }
