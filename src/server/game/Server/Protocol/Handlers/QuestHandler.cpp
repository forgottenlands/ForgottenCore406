...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket & recv_data) {
	...
	Object* questgiver = ObjectAccessor::GetObjectByTypeMask(*_player, guid,
			TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT);
	if (!questgiver) {
		sLog->outDetail(
				"Error in CMSG_QUESTGIVER_STATUS_QUERY, called for not found questgiver (Typeid: %u GUID: %u)",
				GuidHigh2TypeId(GUID_HIPART(guid)), GUID_LOPART(guid));
		return;
	}
	...
	case TYPEID_UNIT: {
		sLog->outDebug(
				...
				"WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for npc, guid = %u",
	...
	case TYPEID_GAMEOBJECT: {
		sLog->outDebug(
				LOG_FILTER_NETWORKIO,
				"WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for GameObject guid = %u",
				uint32(GUID_LOPART(guid)));
	...
	default:
		sLog->outError("QuestGiver called for unexpected type %u",
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket & recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTGIVER_HELLO npc = %u",
			GUID_LOPART(guid));
	...
	if (!pCreature) {
		sLog->outDebug(
				LOG_FILTER_NETWORKIO,
				"WORLD: HandleQuestgiverHelloOpcode - Unit (GUID: %u) not found or you can't interact with him.",
				GUID_LOPART(guid));
		return;
	}
	...
	_player->PrepareGossipMenu(pCreature,
			pCreature->GetCreatureInfo()->GossipMenuId, true);
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket & recv_data) {
	...
	sLog->outDebug(
			LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc = %u, quest = %u, unk1 = %u",
	...
	if (qInfo) {
		...
		if (_player->GetDivider() != 0) {
			...
			if (pPlayer) {
				pPlayer->SendPushToPartyResponse(_player,
			...
			case TYPEID_CONTAINER: {
				...
				for (int i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i) {
					if ((qInfo->ReqItemId[i] == ((Item*) pObject)->GetEntry())
							&& (((Item*) pObject)->GetProto()->MaxCount > 0)) {
						destroyItem = false;
						break;
					}
				}
				...
				if (destroyItem)
					_player->DestroyItem(((Item*) pObject)->GetBagSlot(),
							((Item*) pObject)->GetSlot(), true);

				break;
			}
			case TYPEID_GAMEOBJECT:
				sScriptMgr->OnQuestAccept(_player, ((GameObject*) pObject),
						qInfo);
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket & recv_data) {
	...
	recv_data >> quest;
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUEST_QUERY quest = %u", quest);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket & recv_data) {
	...
	if (reward >= QUEST_REWARD_CHOICES_COUNT) {
		sLog->outError(
				"Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player %s (guid %d) tried to get invalid reward (%u) (probably packet hacking)",
				_player->GetName(), _player->GetGUIDLow(), reward);
		return;
	}
	...
	sLog->outDebug(
			LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc = %u, quest = %u, reward = %u",
			uint32(GUID_LOPART(guid)), quest, reward);

	Object* pObject = ObjectAccessor::GetObjectByTypeMask(*_player, guid,
			TYPEMASK_UNIT | TYPEMASK_GAMEOBJECT | TYPEMASK_PLAYER);
	if (!pObject)
		return;
	...
	if (pQuest) {
		if (_player->CanRewardQuest(pQuest, reward, true)) {
			...
			switch (pObject->GetTypeId()) {
			case TYPEID_UNIT:
				if (!(sScriptMgr->OnQuestReward(_player,
						(pObject->ToCreature()), pQuest, reward))) {
					// Send next quest
					if (Quest const* nextquest = _player->GetNextQuest(guid , pQuest))
						_player->PlayerTalkClass->SendQuestGiverQuestDetails(
								nextquest, guid, true);

					(pObject->ToCreature())->AI()->sQuestReward(_player, pQuest,
							reward);
				}
				break;
			case TYPEID_GAMEOBJECT:
				if (!sScriptMgr->OnQuestReward(_player, ((GameObject*) pObject),
						pQuest, reward)) {
					// Send next quest
					if (Quest const* nextquest = _player->GetNextQuest(guid , pQuest))
						_player->PlayerTalkClass->SendQuestGiverQuestDetails(
								nextquest, guid, true);
					pObject->ToGameObject()->AI()->QuestReward(_player, pQuest,
							reward);
				}
				break;
			case TYPEID_PLAYER:
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(
	...
	sLog->outDebug(
			LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTGIVER_REQUEST_REWARD npc = %u, quest = %u",
			uint32(GUID_LOPART(guid)), quest);
...
void WorldSession::HandleQuestLogSwapQuest(WorldPacket& recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTLOG_SWAP_QUEST slot 1 = %u, slot 2 = %u",
			slot1, slot2);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = %u", slot);
	...
	if (slot < MAX_QUEST_LOG_SIZE) {
		if (uint32 quest = _player->GetQuestSlotQuestId(slot)) {
			...
			_player->GetAchievementMgr().RemoveTimedAchievement(
					ACHIEVEMENT_TIMED_TYPE_QUEST, quest);

			sLog->outDetail("Player %u abandoned quest %u",
					_player->GetGUIDLow(), quest);
		}
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT quest = %u", quest);

	if (const Quest* pQuest = sObjectMgr->GetQuestTemplate(quest)) {
		if (!pQuest->HasFlag(QUEST_FLAGS_PARTY_ACCEPT))
			return;

		Player* pOriginalPlayer = ObjectAccessor::FindPlayer(
				_player->GetDivider());
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recv_data) {
	...
	sLog->outDebug(
			LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_QUESTGIVER_COMPLETE_QUEST npc = %u, quest = %u, unk = %u",
			uint32(GUID_LOPART(guid)), quest, unk);

	Quest const *pQuest = sObjectMgr->GetQuestTemplate(quest);
	if (pQuest) {
		if (!_player->CanSeeStartQuest(pQuest)
				&& _player->GetQuestStatus(quest) == QUEST_STATUS_NONE) {
			sLog->outError(
					"Possible hacking attempt: Player %s [guid: %u] tried to complete quest [entry: %u] without being in possession of the quest!",
		...
		if (_player->GetQuestStatus(quest) != QUEST_STATUS_COMPLETE) {
			if (pQuest->IsRepeatable())
				_player->PlayerTalkClass->SendQuestGiverRequestItems(pQuest,
						guid, _player->CanCompleteRepeatableQuest(pQuest),
						false);
			else
				_player->PlayerTalkClass->SendQuestGiverRequestItems(pQuest,
			...
			if (pQuest->GetReqItemsCount()) // some items required
				...
				// no items required
				_player->PlayerTalkClass->SendQuestGiverOfferReward(pQuest,
						guid, true);
		}
	}
	// Now save player every accept/deliver a quest
...
void WorldSession::HandlePushQuestToParty(WorldPacket& recvPacket) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_PUSHQUESTTOPARTY quest = %u", questId);
	...
	if (Quest const *pQuest = sObjectMgr->GetQuestTemplate(questId)) {
		if (Group* pGroup = _player->GetGroup()) {
			for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL;
				...
				_player->SendPushToPartyResponse(pPlayer,
				...
				pPlayer->PlayerTalkClass->SendQuestGiverQuestDetails(pQuest,
...
uint32 WorldSession::getDialogStatus(Player *pPlayer, Object* questgiver,
		uint32 defstatus) {
	...
	default:
		...
		sLog->outError("Warning: GetDialogStatus called for unexpected type %u",
				questgiver->GetTypeId());
	...
	for (QuestRelations::const_iterator i = qir.first; i != qir.second; ++i) {
		...
		if ((status == QUEST_STATUS_COMPLETE
				...
				|| (pQuest->IsAutoComplete()
						&& pPlayer->CanTakeQuest(pQuest, false))) {
			...
			if (pPlayer->CanSeeStartQuest(pQuest)) {
				if (pPlayer->SatisfyQuestLevel(pQuest, false)) {
					if (pQuest->IsAutoComplete()
							|| (pQuest->IsRepeatable()
									&& pPlayer->getRewardedQuests().find(
											quest_id)
											!= pPlayer->getRewardedQuests().end()))
						result2 = DIALOG_STATUS_REWARD_REP;
					else if (pPlayer->getLevel()
							<= ((pPlayer->GetQuestLevel(pQuest) == -1) ?
									pPlayer->getLevel() :
									pPlayer->GetQuestLevel(pQuest)
											+ sWorld->getIntConfig(
													CONFIG_QUEST_LOW_LEVEL_HIDE_DIFF))) {
						if (pQuest->HasFlag(QUEST_FLAGS_DAILY)
								|| pQuest->HasFlag(QUEST_FLAGS_WEEKLY))
...
void WorldSession::HandleQuestgiverStatusMultipleQuery(
	...
	for (Player::ClientGUIDs::const_iterator itr =
			_player->m_clientGUIDs.begin(); itr != _player->m_clientGUIDs.end();
			++itr) {
		uint32 questStatus = DIALOG_STATUS_NONE;
		uint32 defstatus = DIALOG_STATUS_NONE;
		...
		if (IS_CRE_OR_VEH_OR_PET_GUID(*itr)) {
			...
			Creature *questgiver = ObjectAccessor::GetCreatureOrPetOrVehicle(
					*GetPlayer(), *itr);
...
void WorldSession::HandleQueryQuestsCompleted(WorldPacket & /*recv_data*/) {
	WorldPacket data(SMSG_QUERY_QUESTS_COMPLETED_RESPONSE,
			4 + 4 * _player->getRewardedQuests().size());
	data << uint32(_player->getRewardedQuests().size());

	for (RewardedQuestSet::const_iterator itr =
			_player->getRewardedQuests().begin();
			itr != _player->getRewardedQuests().end(); ++itr)
		data << uint32(*itr);
