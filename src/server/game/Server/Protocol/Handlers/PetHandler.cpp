...
void WorldSession::HandleDismissCritter(WorldPacket &recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_DISMISS_CRITTER for GUID " UI64FMTD, guid);
	...
	if (!pet) {
		sLog->outDebug(
				...
				"Vanitypet (guid: %u) does not exist - player '%s' (guid: %u / account: %u) attempted to dismiss it (possibly lagged out)",
				uint32(GUID_LOPART(guid)), GetPlayer()->GetName(),
				GetPlayer()->GetGUIDLow(), GetAccountId());
		return;
	}
...
void WorldSession::HandlePetAction(WorldPacket & recv_data) {
	...
	sLog->outDetail(
			"HandlePetAction: Pet %u - flag: %u, spellid: %u, target: %u.",
			uint32(GUID_LOPART(guid1)), uint32(flag), spellid,
	...
	if (!pet->isAlive()) {
		SpellEntry const* spell =
				(flag == ACT_ENABLED || flag == ACT_PASSIVE) ?
						sSpellStore.LookupEntry(spellid) : NULL;
		if (!spell)
			return;
	...
	if (pet->GetTypeId() == TYPEID_PLAYER
			&& !(flag == ACT_COMMAND && spellid == COMMAND_ATTACK))
		return;
...
void WorldSession::HandlePetStopAttack(WorldPacket &recv_data) {
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_PET_STOP_ATTACK for GUID " UI64FMTD "", guid);

	Unit* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player, guid);

	if (!pet) {
		sLog->outError("HandlePetStopAttack: Pet %u does not exist",
	...
	if (pet != GetPlayer()->GetPet() && pet != GetPlayer()->GetCharm()) {
		sLog->outError(
				"HandlePetStopAttack: Pet GUID %u isn't a pet or charmed creature of player %s",
				uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
		return;
	}
...
void WorldSession::HandlePetActionHelper(Unit *pet, uint64 guid1,
		uint16 spellid, uint16 flag, uint64 guid2) {
	CharmInfo *charmInfo = pet->GetCharmInfo();
	if (!charmInfo) {
		sLog->outError(
				"WorldSession::HandlePetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
	...
	case ACT_COMMAND: //0x07
		...
		case COMMAND_FOLLOW: //spellid=1792  //FOLLOW
			pet->AttackStop();
			pet->InterruptNonMeleeSpells(false);
			pet->GetMotionMaster()->MoveFollow(_player, PET_FOLLOW_DIST,
					pet->GetFollowAngle());
...
void WorldSession::SendPetNameQuery(uint64 petguid, uint32 petnumber) {
	Creature* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*_player,
			petguid);
	if (!pet) {
...
bool WorldSession::CheckStableMaster(uint64 guid) {
	...
	if (guid == GetPlayer()->GetGUID()) {
		if (!GetPlayer()->isGameMaster()
			...
			sLog->outStaticDebug(
					"Player (GUID:%u) attempt open stable in cheating way.",
					GUID_LOPART(guid));
			return false;
		}
	}
	// stable master case
		...
		if (!GetPlayer()->GetNPCIfCanInteractWith(guid,
			...
			sLog->outStaticDebug(
					"Stablemaster (GUID:%u) not found or you can't interact with him.",
					GUID_LOPART(guid));
			return false;
		}
	}
...
void WorldSession::HandlePetSetAction(WorldPacket & recv_data) {
	...
	if (!pet || pet != _player->GetFirstControlled()) {
		sLog->outError(
				"HandlePetSetAction: Unknown pet (GUID: %u) or pet owner (GUID: %u)",
				GUID_LOPART(petguid), _player->GetGUIDLow());
		return;
	}
	...
	if (!charmInfo) {
		sLog->outError(
				"WorldSession::HandlePetSetAction: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
				pet->GetGUIDLow(), pet->GetTypeId());
		return;
	}
	...
	for (uint8 i = 0; i < count; ++i) {
		...
		sLog->outDetail(
				"Player %s has changed pet spell action. Position: %u, Spell: %u, State: 0x%X",
		...
		if (!((act_state == ACT_ENABLED || act_state == ACT_DISABLED
				|| act_state == ACT_PASSIVE) && spell_id
				&& !pet->HasSpell(spell_id))) {
			//sign for autocast
			...
			charmInfo->SetActionBar(position[i], spell_id,
					ActiveStates(act_state));
		}
	}
}
...
void WorldSession::HandlePetRename(WorldPacket & recv_data) {
	...
	if (isdeclined) {
		...
		trans->PAppend(
				"DELETE FROM character_pet_declinedname WHERE owner = '%u' AND id = '%u'",
				_player->GetGUIDLow(), pet->GetCharmInfo()->GetPetNumber());
		trans->PAppend(
				"INSERT INTO character_pet_declinedname (id, owner, genitive, dative, accusative, instrumental, prepositional) VALUES ('%u', '%u', '%s', '%s', '%s', '%s', '%s')",
	...
	trans->PAppend(
			"UPDATE character_pet SET name = '%s', renamed = '1' WHERE owner = '%u' AND id = '%u'",
...
void WorldSession::HandlePetAbandon(WorldPacket & recv_data) {
	...
	sLog->outDetail("HandlePetAbandon. CMSG_PET_ABANDON pet guid is %u",
			GUID_LOPART(guid));
...
void WorldSession::HandlePetSpellAutocastOpcode(WorldPacket& recvPacket) {
	...
	if (!pet
			|| (pet != _player->GetGuardianPet() && pet != _player->GetCharm())) {
		sLog->outError(
				"HandlePetSpellAutocastOpcode.Pet %u isn't pet of player %s .",
				uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
		return;
	}
	...
	if (!charmInfo) {
		sLog->outError(
				"WorldSession::HandlePetSpellAutocastOpcod: object (GUID: %u TypeId: %u) is considered pet-like but doesn't have a charminfo!",
				pet->GetGUIDLow(), pet->GetTypeId());
		return;
	}
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& recvPacket) {
	...
	sLog->outDebug(
			LOG_FILTER_NETWORKIO,
			"WORLD: CMSG_PET_CAST_SPELL, guid: " UI64FMTD ", castCount: %u, spellId %u, castFlags %u",
			guid, castCount, spellId, castFlags);
	...
	if (!caster
			|| (caster != _player->GetGuardianPet()
					&& caster != _player->GetCharm())) {
		sLog->outError(
				"HandlePetCastSpellOpcode: Pet %u isn't pet of player %s .",
				uint32(GUID_LOPART(guid)), GetPlayer()->GetName());
		return;
	}
	...
	if (spellInfo->StartRecoveryCategory > 0) // Check if spell is affected by GCD
		if (caster->GetTypeId() == TYPEID_UNIT
				&& caster->ToCreature()->GetGlobalCooldown() > 0) {
