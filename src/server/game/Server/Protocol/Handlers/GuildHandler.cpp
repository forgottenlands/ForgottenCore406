...
inline Guild* _GetPlayerGuild(WorldSession* session, bool sendError = false) {
	if (uint32 guildId = session->GetPlayer()->GetGuildId()) // If guild id = 0, player is not in guild
		if (Guild * pGuild = sObjectMgr->GetGuildById(guildId)) // Find guild by id
			return pGuild;
	if (sendError)
		Guild::SendCommandResult(session, GUILD_CREATE_S,
...
void WorldSession::HandleGuildQueryOpcode(WorldPacket& recvPacket) {
	...
	if (Guild * pGuild = sObjectMgr->GetGuildById(lowGuildId))
		...
		Guild::SendCommandResult(this, GUILD_CREATE_S,
				ERR_GUILD_PLAYER_NOT_IN_GUILD);
}
...
void WorldSession::HandleGuildChangeInfoTextOpcode(WorldPacket& recvPacket) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_GUILD_INFO_TEXT");
...
void WorldSession::HandleGuildRewardsOpcode(WorldPacket& recvPacket) {
	if (!_player->GetGuildId()
			|| !sWorld->getBoolConfig(CONFIG_GUILD_ADVANCEMENT_ENABLED))
		return;
...
void WorldSession::HandleGuildRankOpcode(WorldPacket& recvPacket) {
	...
	if (GetPlayer()->GetGUID() != playerGuid) {
		printf(
				"CMSG_GUILD_RANK: The playerGUID in the packet does not match the current player!\n");
	...
	if (old_rankId != GR_GUILDMASTER) {
		for (uint8 tabId = 0; tabId < GUILD_BANK_MAX_TABS; ++tabId) {
			rightsAndSlots[tabId] = GuildBankRightsAndSlots(BankRights[tabId],
					BankStacks[tabId]);
		}

		pGuild->HandleSetRankInfo(this, new_rankId, rankName, new_rights, money,
				rightsAndSlots);
	}
	if (old_rankId != new_rankId && old_rankId != GR_GUILDMASTER
			&& new_rankId != GR_GUILDMASTER)
...
void WorldSession::HandleSaveGuildEmblemOpcode(WorldPacket& recvPacket) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received MSG_SAVE_GUILD_EMBLEM");
	...
	if (GetPlayer()->GetNPCIfCanInteractWith(vendorGuid,
		...
		sLog->outDebug(
				...
				"WORLD: HandleSaveGuildEmblemOpcode - Unit (GUID: %u) not found or you can't interact with him.",
...
void WorldSession::HandleGuildEventLogQueryOpcode(
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (MSG_GUILD_EVENT_LOG_QUERY)");
...
void WorldSession::HandleGuildBankerActivate(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (CMSG_GUILD_BANKER_ACTIVATE)");
	...
	if (GetPlayer()->GetGameObjectIfCanInteractWith(GoGuid,
		...
		if (Guild* pGuild = _GetPlayerGuild(this))
			...
			Guild::SendCommandResult(this, GUILD_UNK1,
					ERR_GUILD_PLAYER_NOT_IN_GUILD);
	}
}
...
void WorldSession::HandleGuildBankDepositMoney(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (CMSG_GUILD_BANK_DEPOSIT_MONEY)");
...
void WorldSession::HandleGuildBankWithdrawMoney(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (CMSG_GUILD_BANK_WITHDRAW_MONEY)");
...
void WorldSession::HandleGuildBankSwapItems(WorldPacket & recv_data) {
	...
	if (bankToBank) {
		...
		pGuild->SwapItems(GetPlayer(), tabId, slotId, destTabId, destSlotId,
				splitedAmount);
	} else {
		...
		if (!Player::IsInventoryPos(playerBag, playerSlotId)
			...
			pGuild->SwapItemsWithInventory(GetPlayer(), toChar, tabId, slotId,
					playerBag, playerSlotId, splitedAmount);
	}
}
...
void WorldSession::HandleGuildBankBuyTab(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (CMSG_GUILD_BANK_BUY_TAB)");
...
void WorldSession::HandleGuildBankUpdateTab(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (CMSG_GUILD_BANK_UPDATE_TAB)");
...
void WorldSession::HandleGuildBankLogQuery(WorldPacket & recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received (MSG_GUILD_BANK_LOG_QUERY)");
...
void WorldSession::HandleQueryGuildBankTabText(WorldPacket &recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received MSG_QUERY_GUILD_BANK_TEXT");
...
void WorldSession::HandleSetGuildBankTabText(WorldPacket &recv_data) {
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Received CMSG_SET_GUILD_BANK_TEXT");
