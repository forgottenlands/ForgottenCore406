...
{ 
    ...
    &Spell::EffectNULL,                       // 152 SPELL_EFFECT_SUMMON_FRIEND            summon Refer-a-Friend
    ...
    &Spell::EffectNULL,                       // 163 SPELL_EFFECT_163                      unused
    &Spell::EffectRemoveAura,                 // 164 SPELL_EFFECT_REMOVE_AURA
    &Spell::EffectDamageSelfPct,              // 165 SPELL_EFFECT_165                      area of impact ?
    &Spell::EffectAddHonorCurrency,           // 166 SPELL_EFFECT_ADD_HONOR_2              miscvalue = currencyid bp0 = amount
    &Spell::EffectNULL,                       // 167 SPELL_EFFECT_167                      something related to phases
    &Spell::EffectNULL,                       // 168 SPELL_EFFECT_CONTROL_MINION           allows you to give commands to control your demon/pet      NYI      
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                {
                    damage = uint32(m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.56); //wowhead formula
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                // Eviscerate
                else if (m_spellInfo->Id == 2098 && m_caster->GetTypeId() == TYPEID_PLAYER)
                {
            ...
            case SPELLFAMILY_MAGE:
            {
                ...
                if (m_spellInfo->Id == 71757) 
                    if (unitTarget->GetTypeId() != TYPEID_UNIT || !(unitTarget->IsImmunedToSpellEffect(sSpellStore.LookupEntry(44572), 0))) return;
    ...
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        if (m_caster->getClass() == CLASS_PALADIN)
        {
            if (m_caster->HasAuraType(SPELL_AURA_MASTERY))
            {
                ...
                {
                    int32 bp0 = int32(addhealth	* (10.0f + (1.25f * m_caster->ToPlayer()->GetMasteryPoints())) / 100);
                    if (unitTarget->HasAura(86273))
                        if (bp0 < unitTarget->GetAura(86273)->GetEffect(0)->GetAmount())
                            bp0 = unitTarget->GetAura(86273)->GetEffect(0)->GetAmount();
    ...
    if (m_caster->ToPlayer() && m_caster->ToPlayer()->HasSkill(SKILL_ARCHAEOLOGY))
    {
        for (uint8 i = 1; i < 28; ++i)
        {
            ...
            if (rp->spellId == m_spellInfo->Id)
            {
                ...
                m_caster->ToPlayer()->ModifyCurrency(currencyId, -(reqFragments * PLAYER_CURRENCY_PRECISION));
...
void Spell::EffectAddHonorCurrency(SpellEffIndex effIndex)
{
    if (unitTarget->GetTypeId() != TYPEID_PLAYER) 
        return;
    ...
    {
        uint32 currencyId = m_spellInfo->EffectMiscValue[effIndex];
        int32 honorAmount = m_spellInfo->EffectBasePoints[effIndex] * PLAYER_CURRENCY_PRECISION;
        target->ModifyCurrency(currencyId, honorAmount);
    }
}
