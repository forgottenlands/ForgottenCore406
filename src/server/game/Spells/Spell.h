...
enum SpellCastTargetFlags {
    TARGET_FLAG_SELF = 0x00000000, TARGET_FLAG_UNUSED1 = 0x00000001, // not used in any spells as of 3.2.2a (can be set dynamically)
...
enum SpellCastFlags {
    CAST_FLAG_NONE = 0x00000000, CAST_FLAG_PENDING = 0x00000001, // Pending == 1
...
enum SpellLogType {
    ...
    SPELL_LOG_REMOVE_3 = 0xD557,
// currently unused (n/u)
};
...
public:
    ...
    bool IsEmpty() const {
        return m_GOTargetGUID == 0 && m_unitTargetGUID == 0 && m_itemTarget == 0
    ...
    float GetDist2d() const {
        return m_srcPos.GetExactDist2d(&m_dstPos);
    }
    float GetSpeedXY() const {
        return m_speed * cos(m_elevation);
    }
    float GetSpeedZ() const {
        return m_speed * sin(m_elevation);
    }
...
public:
    ...
    Spell(Unit* Caster, SpellEntry const *info, bool triggered,
            uint64 originalCasterGUID = 0, bool skipCheck = false);
    ~Spell();
    ...
    int32 CalculateDamage(uint8 i, Unit* target) {
        return m_caster->CalculateSpellDamage(target, m_spellInfo, i,
                &m_spellValue->EffectBasePoints[i]);
    }
    ...
    static void SendCastResult(Player* caster, SpellEntry const* spellInfo,
    ...
    void SendLogExecute();
    void ExecuteLogEffectTakeTargetPower(uint8 effIndex, Unit * target,
            uint32 powerType, uint32 powerTaken, float gainMultiplier);
    void ExecuteLogEffectExtraAttacks(uint8 effIndex, Unit * victim,
            uint32 attCount);
    void ExecuteLogEffectInterruptCast(uint8 effIndex, Unit * victim,
            uint32 spellId);
    void ExecuteLogEffectDurabilityDamage(uint8 effIndex, Unit * victim,
    ...
    void SetMagnetingAura(Aura* aura) {
        m_magnetingAura = aura;
    }
...
protected:
    ...
    void SearchAreaTarget(std::list<Unit*> &unitList, float radius,
            SpellNotifyPushType type, SpellTargets TargetType,
            uint32 entry = 0);
    void SearchGOAreaTarget(std::list<GameObject*> &gobjectList, float radius,
            SpellNotifyPushType type, SpellTargets TargetType,
            uint32 entry = 0);
    void SearchChainTarget(std::list<Unit*> &unitList, float radius,
            uint32 unMaxTargets, SpellTargets TargetType);
    ...
    int32 CalculateDamageDone(Unit *unit, const uint32 effectMask,
            float *multiplier);
    ...
    void CallScriptAfterUnitTargetSelectHandlers(std::list<Unit*>& unitTargets,
            SpellEffIndex effIndex);
    ...
    void GetSummonPosition(uint32 i, Position &pos, float radius = 0.0f,
            uint32 count = 0);
    void SummonGuardian(uint32 i, uint32 entry,
            SummonPropertiesEntry const *properties);
    void CalculateJumpSpeeds(uint8 i, float dist, float & speedxy,
            float & speedz);

    SpellCastResult CanOpenLock(uint32 effIndex, uint32 lockid,
...
struct SpellNotifierCreatureAndPlayer {
    ...
    SpellNotifierCreatureAndPlayer(Unit *source, std::list<Unit*> &data,
            float radius, SpellNotifyPushType type, SpellTargets TargetType =
                    SPELL_TARGETS_ENEMY, const Position *pos = NULL,
            uint32 entry = 0, bool requireDeadTarget = false) :
            i_data(&data), i_push_type(type), i_radius(radius), i_TargetType(
                    TargetType), i_source(source), i_entry(entry), i_pos(pos), i_requireDeadTarget(
                    requireDeadTarget) {
        ASSERT(i_source);
    }

    template<class T> inline void Visit(GridRefManager<T> &m) {
        for (typename GridRefManager<T>::iterator itr = m.begin();
                itr != m.end(); ++itr) {
            ...
            default:
                ...
                break;
            case PUSH_IN_FRONT:
                if (i_source->isInFront(target, i_radius,
                        static_cast<float>(M_PI / 2)))
                    i_data->push_back(target);
                break;
            case PUSH_IN_BACK:
                if (i_source->isInBack(target, i_radius,
                        static_cast<float>(M_PI / 2)))
                    i_data->push_back(target);
                break;
            case PUSH_IN_LINE:
                if (i_source->HasInLine(target, i_radius,
                        i_source->GetObjectSize()))
                    i_data->push_back(target);
                break;
...
template<> inline void SpellNotifierCreatureAndPlayer::Visit(
        GameObjectMapType&) {
}
template<> inline void SpellNotifierCreatureAndPlayer::Visit(
