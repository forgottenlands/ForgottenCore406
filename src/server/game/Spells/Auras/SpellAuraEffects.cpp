...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS] = { &AuraEffect::HandleNULL, //  0 SPELL_AURA_NONE
        ...
        &AuraEffect::HandleNoImmediateEffect, //308 SPELL_AURA_MOD_CRIT_CHANCE_FOR_CASTER implemented in Unit::IsSpellCrit
...
int32 AuraEffect::CalculateAmount(Unit *caster) {
    ...
    case SPELL_AURA_PERIODIC_ENERGIZE:
        ...
        {
            // Replenishment (1% over 10 secs, so 0.1% mana every tick)
            // Infinite Replenishment
            if (m_spellProto->SpellIconID == 3184 && m_spellProto->SpellVisual[0] == 12495)
                amount = GetBase()->GetUnitOwner()->GetMaxPower(POWER_MANA) * 10 / 10000;
        }
        // Innervate
...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit * caster) const 
{
    ...
    switch (GetAuraType()) 
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT: 
        {
            ...
            if (GetSpellProto()->Effect[GetEffIndex()] == SPELL_EFFECT_PERSISTENT_AREA_AURA && caster->SpellHitResult(target, GetSpellProto(), false) != SPELL_MISS_NONE)
                break;
            ...
            sLog->outDetail("PeriodicTick: %u (TypeId: %u) attacked %u (TypeId: %u) for %u dmg inflicted by %u abs is %u", 
                    ...
                    target->GetGUIDLow(), target->GetTypeId(), damage, GetId(), absorb);
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
        {
            ...
            switch (GetId())
            {
                ...
                // Drain Mana
                if (m_spellProto->SpellFamilyName == SPELLFAMILY_WARLOCK && m_spellProto->SpellFamilyFlags[0] & 0x00000010) 
                {
                    int32 manaFeedVal = 0;
