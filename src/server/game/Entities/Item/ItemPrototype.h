...
enum ItemFieldFlags {
    ...
    ITEM_FLAG_MAIL_TEXT_MASK = ITEM_FLAG_READABLE | ITEM_FLAG_UNK13
            | ITEM_FLAG_UNK14,
};
...
// uses caster specific dbc file for DPS calculations
};
...
enum ItemSubclassKey {
    ITEM_SUBCLASS_KEY = 0, ITEM_SUBCLASS_LOCKPICK = 1
};
...
const uint32 MaxItemSubclassValues[MAX_ITEM_CLASS] = {
        MAX_ITEM_SUBCLASS_CONSUMABLE, MAX_ITEM_SUBCLASS_CONTAINER,
        MAX_ITEM_SUBCLASS_WEAPON, MAX_ITEM_SUBCLASS_GEM,
        MAX_ITEM_SUBCLASS_ARMOR, MAX_ITEM_SUBCLASS_REAGENT,
        MAX_ITEM_SUBCLASS_PROJECTILE, MAX_ITEM_SUBCLASS_TRADE_GOODS,
        MAX_ITEM_SUBCLASS_GENERIC, MAX_ITEM_SUBCLASS_RECIPE,
        MAX_ITEM_SUBCLASS_MONEY, MAX_ITEM_SUBCLASS_QUIVER,
        MAX_ITEM_SUBCLASS_QUEST, MAX_ITEM_SUBCLASS_KEY,
...
struct ItemPrototype {
    ...
    uint32 GetMaxStackSize() const {
        return (Stackable == 2147483647 || Stackable <= 0) ?
                uint32(0x7FFFFFFF - 1) : uint32(Stackable);
    }
    ...
    bool IsConjuredConsumable() const {
        return Class == ITEM_CLASS_CONSUMABLE
                && (Flags & ITEM_PROTO_FLAG_CONJURED);
    }
};
