...
void AddItemsSetItem(Player*player, Item *item) {
	...
	if (!set) {
		sLog->outErrorDb(
				"Item set %u for item (id %u) not found, mods not applied.",
				setid, proto->ItemId);
		return;
	}
...
void RemoveItemsSetItem(Player*player, ItemPrototype const *proto) {
	...
	if (!set) {
		sLog->outErrorDb(
				"Item set #%u for item #%u not found, mods not removed.", setid,
				proto->ItemId);
		return;
	}
...
bool ItemCanGoIntoBag(ItemPrototype const *pProto,
...
uint32 ItemPrototype::GetArmor() const {
	...
	ItemArmorQualityEntry const* iaq = sItemArmorQualityStore.LookupEntry(
			ItemLevel);
	ItemArmorTotalEntry const* iat = sItemArmorTotalStore.LookupEntry(
			ItemLevel);
	...
	if (InventoryType != INVTYPE_HEAD && InventoryType != INVTYPE_CHEST
			&& InventoryType != INVTYPE_SHOULDERS
			&& InventoryType != INVTYPE_LEGS && InventoryType != INVTYPE_FEET
			&& InventoryType != INVTYPE_WRISTS && InventoryType != INVTYPE_HANDS
			&& InventoryType != INVTYPE_WAIST && InventoryType != INVTYPE_CLOAK
...
void Item::UpdateDuration(Player* owner, uint32 diff) {
	...
	sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
			"Item::UpdateDuration Item (Entry: %u Duration %u Diff %u)",
			GetEntry(), GetUInt32Value(ITEM_FIELD_DURATION), diff);
...
void Item::SaveToDB(SQLTransaction& trans) {
	...
	case ITEM_CHANGED: {
		...
		PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(
				uState == ITEM_NEW ?
						CHAR_ADD_ITEM_INSTANCE : CHAR_UPDATE_ITEM_INSTANCE);
		...
		if ((uState == ITEM_CHANGED)
				&& HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED)) {
...
uint32 Item::GetSkill() {
	const static uint32 item_weapon_skills[MAX_ITEM_SUBCLASS_WEAPON] = {
			...
			SKILL_STAVES, 0, 0, SKILL_FIST_WEAPONS, 0, SKILL_DAGGERS,
			SKILL_THROWN, SKILL_ASSASSINATION, SKILL_CROSSBOWS, SKILL_WANDS,
			SKILL_FISHING };

	const static uint32 item_armor_skills[MAX_ITEM_SUBCLASS_ARMOR] = { 0,
			SKILL_CLOTH, SKILL_LEATHER, SKILL_MAIL, SKILL_PLATE_MAIL, 0,
...
int32 Item::GenerateItemRandomPropertyId(uint32 item_id) {
	ItemPrototype const *itemProto = sItemStorage.LookupEntry<ItemPrototype>(
			item_id);
	...
	if ((itemProto->RandomProperty) && (itemProto->RandomSuffix)) {
		sLog->outErrorDb(
				"Item template %u have RandomProperty == %u and RandomSuffix == %u, but must have one from field =0",
				itemProto->ItemId, itemProto->RandomProperty,
	...
	if (itemProto->RandomProperty) {
		...
		if (!random_id) {
			sLog->outErrorDb(
					"Enchantment id #%u used but it doesn't have records in 'ItemRandomProperties.dbc'",
					randomPropId);
			return 0;
		}
	...
	// RandomSuffix case
		...
		if (!random_id) {
			sLog->outErrorDb(
					"Enchantment id #%u used but it doesn't have records in sItemRandomSuffixStore.",
					randomPropId);
			return 0;
		}
...
void Item::SetItemRandomProperties(int32 randomPropId) {
	...
	if (randomPropId > 0) {
		...
		if (item_rand) {
			...
			for (uint32 i = PROP_ENCHANTMENT_SLOT_2;
				...
				SetEnchantment(EnchantmentSlot(i),
						item_rand->enchant_id[i - PROP_ENCHANTMENT_SLOT_2], 0,
						0);
		}
	} else {
		...
		if (item_rand) {
			if (GetInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID)
					!= -int32(item_rand->ID) || !GetItemSuffixFactor()) {
			...
			for (uint32 i = PROP_ENCHANTMENT_SLOT_0;
				...
				SetEnchantment(EnchantmentSlot(i),
						item_rand->enchant_id[i - PROP_ENCHANTMENT_SLOT_0], 0,
						0);
		}
	}
}
...
void Item::AddToUpdateQueueOf(Player *player) {
	...
	if (player->GetGUID() != GetOwnerGUID()) {
		sLog->outDebug(
				...
				"Item::AddToUpdateQueueOf - Owner's guid (%u) and player's guid (%u) don't match!",
				GUID_LOPART(GetOwnerGUID()), player->GetGUIDLow());
		return;
	}
...
void Item::RemoveFromUpdateQueueOf(Player *player) {
	...
	if (player->GetGUID() != GetOwnerGUID()) {
		sLog->outDebug(
				LOG_FILTER_PLAYER_ITEMS,
				"Item::RemoveFromUpdateQueueOf - Owner's guid (%u) and player's guid (%u) don't match!",
				GUID_LOPART(GetOwnerGUID()), player->GetGUIDLow());
		return;
	}
...
bool Item::CanBeTraded(bool mail, bool trade) const {
	...
	if ((!mail || !IsBoundAccountWide())
			&& (IsSoulBound()
					&& (!HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_BOP_TRADEABLE)
							|| !trade)))
		return false;
...
bool Item::HasEnchantRequiredSkill(const Player *pPlayer) const {
	...
	for (uint32 enchant_slot = PERM_ENCHANTMENT_SLOT;
			enchant_slot < MAX_ENCHANTMENT_SLOT; ++enchant_slot)
		if (uint32 enchant_id = GetEnchantmentId(EnchantmentSlot(enchant_slot)))
			if (SpellItemEnchantmentEntry const* enchantEntry = sSpellItemEnchantmentStore.LookupEntry(enchant_id))
				if (enchantEntry->requiredSkill
						&& pPlayer->GetSkillValue(enchantEntry->requiredSkill)
...
bool Item::IsFitToSpellRequirements(SpellEntry const* spellInfo) const {
	...
	if (spellInfo->EquippedItemClass != -1) // -1 == any item class
		...
		if (spellInfo->EquippedItemSubClassMask != 0) // 0 == any subclass
				{
			if ((spellInfo->EquippedItemSubClassMask & (1 << proto->SubClass))
					== 0)
...
bool Item::IsTargetValidForItemUse(Unit* pUnitTarget) {
	ConditionList conditions = sConditionMgr->GetConditionsForNotGroupedEntry(
			CONDITION_SOURCE_TYPE_ITEM_REQUIRED_TARGET, GetProto()->ItemId);
	if (conditions.empty())
		return true;
	...
	for (ConditionList::const_iterator itr = conditions.begin();
			itr != conditions.end(); ++itr) {
		ACE_Auto_Ptr<ItemRequiredTarget> irt(
				new ItemRequiredTarget(
						(ItemRequiredTargetType) (*itr)->mConditionValue1,
						(*itr)->mConditionValue2));
...
void Item::SetEnchantment(EnchantmentSlot slot, uint32 id, uint32 duration,
		uint32 charges) {
	...
	SetUInt32Value(
			ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET
					+ ENCHANTMENT_ID_OFFSET, id);
	SetUInt32Value(
			ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET
					+ ENCHANTMENT_DURATION_OFFSET, duration);
	SetUInt32Value(
			ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET
					+ ENCHANTMENT_CHARGES_OFFSET, charges);
	SetState(ITEM_CHANGED, GetOwner());
}
...
void Item::ClearEnchantment(EnchantmentSlot slot) {
	...
	for (uint8 x = 0; x < MAX_ITEM_ENCHANTMENT_EFFECTS; ++x)
		SetUInt32Value(
				ITEM_FIELD_ENCHANTMENT_1_1 + slot * MAX_ENCHANTMENT_OFFSET + x,
				0);
	SetState(ITEM_CHANGED, GetOwner());
}
...
bool Item::GemsFitSockets() const {
	...
	for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
			...
			++enchant_slot) {
		uint8 SocketColor = GetProto()->Socket[enchant_slot
...
uint8 Item::GetGemCountWithLimitCategory(uint32 limitCategory) const {
	...
	for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT;
		...
		ItemPrototype const* gemProto = ObjectMgr::GetItemPrototype(
				enchantEntry->GemID);
		if (!gemProto)
			continue;
...
void Item::SaveRefundDataToDB() {
	...
	trans->PAppend("DELETE FROM item_refund_instance WHERE item_guid = '%u'",
			GetGUIDLow());
	trans->PAppend(
			"INSERT INTO item_refund_instance (`item_guid`, `player_guid`, `paidMoney`, `paidExtendedCost`)"
					" VALUES('%u', '%u', '%u', '%u')", GetGUIDLow(),
			GetRefundRecipient(), GetPaidMoney(), GetPaidExtendedCost());
...
void Item::SetSoulboundTradeable(AllowedLooterSet* allowedLooters,
		Player* currentOwner, bool apply) {
	if (apply) {
