...
bool ItemCanGoIntoBag(ItemPrototype const *proto,
		ItemPrototype const *pBagProto);

class Item: public Object {
public:
	static Item* CreateItem(uint32 item, uint32 count, Player const* player =
			NULL);
	...
	void SetEnchantment(EnchantmentSlot slot, uint32 id, uint32 duration,
			uint32 charges);
