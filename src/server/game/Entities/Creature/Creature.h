...
#ifndef ARKCORECORE_CREATURE_H
#define ARKCORECORE_CREATURE_H
...
struct CreatureInfo {
    ...
    bool isTameable(bool exotic) const {
        if (type != CREATURE_TYPE_BEAST || family == 0
                || (type_flags & CREATURE_TYPEFLAGS_TAMEABLE) == 0)
            return false;
...
// Vendors
struct VendorItem {
    VendorItem(uint32 _item, int32 _maxcount, uint32 _incrtime,
            uint32 _ExtendedCost) :
            item(_item), maxcount(_maxcount), incrtime(_incrtime), ExtendedCost(
                    _ExtendedCost) {
    }
    ...
    //helpers
    bool IsGoldRequired(ItemPrototype const* pProto) const {
...
struct VendorItemData {
    ...
    void AddItem(uint32 item, int32 maxcount, uint32 ptime,
            uint32 ExtendedCost) {
        m_items.push_back(new VendorItem(item, maxcount, ptime, ExtendedCost));
    }
    bool RemoveItem(uint32 item_id);
    VendorItem const* FindItemCostPair(uint32 item_id,
...
struct TrainerSpell {
    TrainerSpell() :
            spell(0), spellCost(0), reqSkill(0), reqSkillValue(0), reqLevel(0), KillCredit(
                    0) {
...
public:
    ...
    uint32 GetEquipmentId() const {
        return GetCreatureInfo()->equipmentId;
    }
    ...
    bool isCivilian() const {
        return GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_CIVILIAN;
    }
    bool isTrigger() const {
        return GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_TRIGGER;
    }
    bool isGuard() const {
    ...
    bool IsImmunedToSpellEffect(SpellEntry const* spellInfo,
            uint32 index) const;
    ...
    bool IsInEvadeMode() const {
        return HasUnitState(UNIT_STAT_EVADE);
    }
    ...
    bool UpdateEntry(uint32 entry, uint32 team = ALLIANCE,
            const CreatureData* data = NULL);
    ...
    void SetCurrentEquipmentId(uint32 entry) {
        m_equipmentId = entry;
    }
    ...
    bool CanAssistTo(const Unit* u, const Unit* enemy,
    ...
    void LowerPlayerDamageReq(uint32 unDamage) {
        if (m_PlayerDamageReq)
            m_PlayerDamageReq > unDamage ?
                    m_PlayerDamageReq -= unDamage : m_PlayerDamageReq = 0;
    }
    ...
    void SetGUIDTransport(uint32 guid) {
        guid_transport = guid;
    }
    uint32 GetGUIDTransport() {
        return guid_transport;
    }

    void FarTeleportTo(Map* map, float X, float Y, float Z, float O);
protected:
    bool CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId,
            uint32 team, const CreatureData *data = NULL);
    bool InitEntry(uint32 entry, uint32 team = ALLIANCE,
            const CreatureData* data = NULL);
