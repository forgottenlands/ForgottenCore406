...
inline bool _ModifyUInt32(bool apply, uint32& baseValue, int32& amount)
{
    ...
    if (apply) baseValue += amount;
    else
    {
        // Make sure we do not get uint32 overflow.
        if (amount > int32(baseValue)) amount = baseValue;
        baseValue -= amount;
    }
    return apply;
}
...
{
    if (stat > STAT_SPIRIT) return false;
    ...
    if (stat == STAT_STRENGTH)
    {
        ...
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_RANGED_ATTACK_POWER_OF_STAT_PERCENT, stat)) UpdateAttackPowerAndDamage(
                true);
    }
    ...
    else
    {
        ...
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_ATTACK_POWER_OF_STAT_PERCENT, stat)) UpdateAttackPowerAndDamage(
                false);
        if (HasAuraTypeWithMiscvalue(
                SPELL_AURA_MOD_RANGED_ATTACK_POWER_OF_STAT_PERCENT, stat)) UpdateAttackPowerAndDamage(
                true);
    ...
    for (AuraEffectList::const_iterator i = modRatingFromStat.begin();
        ...
        if (Stats((*i)->GetMiscValueB()) == stat) mask |= (*i)->GetMiscValue();
    if (mask)
    {
        for (uint32 rating = 0; rating < MAX_COMBAT_RATING; ++rating)
            if (mask & (1 << rating)) ApplyRatingMod(CombatRating(rating), 0,
                    true);
    }
    return true;
}
...
void Player::ApplySpellPowerBonus(int32 amount, bool apply)
{
    ...
    for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
        ApplyModUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i, amount,
                apply);
...
void Player::UpdateResistances(uint32 school)
{
    if (school > SPELL_SCHOOL_NORMAL)
    {
        float value = GetTotalAuraModValue(
                UnitMods(UNIT_MOD_RESISTANCE_START + school));
        SetResistance(SpellSchools(school), int32(value));

        Pet *pet = GetPet();
        if (pet) pet->UpdateResistances(school);
    }
    else UpdateArmor();
}
...
void Player::UpdateSpellPower()
{
    ...
    ApplyModUInt32Value(PLAYER_FIELD_MOD_HEALING_DONE_POS,
            spellPowerFromIntellect - m_spellPowerFromIntellect, true);
    for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
        ApplyModUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i,
                spellPowerFromIntellect - m_spellPowerFromIntellect, true);
                ...
                switch (GetShapeshiftForm())
                {
                    case FORM_CAT:
                        val2 = getLevel() * (mLevelMult + 2.0f)
                                + GetStat(STAT_STRENGTH) * 2.0f
                                + GetStat(STAT_AGILITY) - 20.0f + weapon_bonus
                                + m_baseFeralAP;
                        break;
                    case FORM_BEAR:
                    case FORM_DIREBEAR:
                    ...
                    i != mRAPbyStat.end(); ++i)
            {
                int32 temp = int32(
                        GetStat(Stats((*i)->GetMiscValue())) * (*i)->GetAmount()
                                / 100.0f);
                if (temp > 0) attPowerMod_pos += temp;
                else attPowerMod_neg -= temp;
            }
        }
    }
    else
    {
        AuraEffectList const& mAPbyStat = GetAuraEffectsByType(
...
void Player::UpdateDodgePercentage()
{
    ...
    // Modify value from defense skill
    value += (int32(GetDefenseSkillValue()) - int32(GetMaxSkillValueForLevel()))
            * 0.04f;
...
{
    m_modMeleeHitChance = (float) GetTotalAuraModifier(
...
{
    m_modRangedHitChance = (float) GetTotalAuraModifier(
            SPELL_AURA_MOD_HIT_CHANCE);
...
{
    m_modSpellHitChance = (float) GetTotalAuraModifier(
...
{
    if (attack == RANGED_ATTACK) return;

    int32 expertise = int32(GetRatingBonusValue(CR_EXPERTISE));

    Item *weapon = GetWeaponForAttack(attack, true);

    AuraEffectList const& expAuras = GetAuraEffectsByType(
            SPELL_AURA_MOD_EXPERTISE);
    for (AuraEffectList::const_iterator itr = expAuras.begin();
            itr != expAuras.end(); ++itr)
    {
        // item neutral spell
        if ((*itr)->GetSpellProto()->EquippedItemClass == -1) expertise +=
                (*itr)->GetAmount();
        // item dependent spell
        else if (weapon
                && weapon->IsFitToSpellRequirements((*itr)->GetSpellProto())) expertise +=
                (*itr)->GetAmount();
    }

    if (expertise < 0) expertise = 0;
...
void Player::UpdateManaRegen()
{
    ...
    int32 modManaRegenInterrupt = GetTotalAuraModifier(
            SPELL_AURA_MOD_MANA_REGEN_INTERRUPT);
    if (modManaRegenInterrupt > 100) modManaRegenInterrupt = 100;
...
void Player::UpdateMastery()
{
    if (HasAuraType(SPELL_AURA_MASTERY))
    {
        SetInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + CR_MASTERY,
                m_baseRatingValue [CR_MASTERY]);
...
void Creature::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    //automatically update weapon damage after attack power modification
    if (ranged) UpdateDamagePhysical(RANGED_ATTACK);
    else
    {
        UpdateDamagePhysical(BASE_ATTACK);
        UpdateDamagePhysical(OFF_ATTACK);
...
void Creature::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    float base_value = GetModifierValue(unitMod, BASE_VALUE)
            + (att_pwr_change * GetAPMultiplier(attType, false) / 14.0f);
    float base_pct = GetModifierValue(unitMod, BASE_PCT);
    float total_value = GetModifierValue(unitMod, TOTAL_VALUE);
    ...
    float mindamage = ((base_value + weapon_mindamage) * dmg_multiplier
            * base_pct + total_value) * total_pct;
    float maxdamage = ((base_value + weapon_maxdamage) * dmg_multiplier
            * base_pct + total_value) * total_pct;
...
{
    if (stat >= MAX_STATS) return false;
    ...
    else if (stat == STAT_STAMINA)
    {
        ...
        else
        {
            ...
            if (isPet())
            {
                ...
                PetSpellMap::const_iterator itr =
                        (ToPet()->m_spells.find(62758)); // Wild Hunt rank 1
                ...
                if (itr != ToPet()->m_spells.end()) // If pet has Wild Hunt
                {
                    ...
                    mod += mod
                            * (SpellMgr::CalculateSpellEffectAmount(sProto, 0)
                                    / 100.0f);
                }
            }
            ownersBonus = float(owner->GetStat(stat)) * mod;
...
void Guardian::UpdateResistances(uint32 school)
{
    if (school > SPELL_SCHOOL_NORMAL)
    {
        ...
        if (isPet()) value += float(
                m_owner->GetResistance(SpellSchools(school))) * 0.4f;
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
        ...
        else if (GetEntry() == ENTRY_FIRE_ELEMENTAL)
        {
            int32 spellDmg = int32(
                    ...
                    - m_owner->GetUInt32Value(
                            PLAYER_FIELD_MOD_DAMAGE_DONE_NEG
                                    + SPELL_SCHOOL_FIRE);
            if (spellDmg > 0) bonusDamage = spellDmg * 0.4f;
        }
    }
    ...
    float mindamage = ((base_value + weapon_mindamage) * base_pct + total_value)
            * total_pct;
    float maxdamage = ((base_value + weapon_maxdamage) * base_pct + total_value)
            * total_pct;
            ...
            itr != mDummy.end(); ++itr)
    {
        switch ((*itr)->GetSpellProto()->Id)
        {
            case 61682:
            case 61683:
                mindamage = mindamage * (100.0f - float((*itr)->GetAmount()))
                        / 100.0f;
                maxdamage = maxdamage * (100.0f - float((*itr)->GetAmount()))
                        / 100.0f;
                break;
            default:
                break;
...
void Guardian::SetBonusDamage(int32 damage)
{
    ...
    if (GetOwner()->GetTypeId() == TYPEID_PLAYER) GetOwner()->SetUInt32Value(
            PLAYER_PET_SPELL_POWER, damage);
}
