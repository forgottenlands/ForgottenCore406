		...
		case SPELLFAMILY_SHAMAN:
		{
			...
			if (dummySpell->SpellFamilyFlags [1] & 0x00000400)
			{
				...
				if (AuraEffect* aur = GetAuraEffect(63279, 0))
				{
					...
					basepoints0 = int32(
							basepoints0 * (aur_mod + 100.0f) / 100.0f);
				}
		...
		uint32 procFlags, uint32 procEx, uint32 cooldown)
{
	...
	switch (auraSpellInfo->Id)
	{
        ...
        case 88994:
        case 88995:
            if (procSpell->Id != 32379 && damage < uint32(GetMaxHealth()*0.1))
                return false;
            break;
		default:
			break;
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
	...
	if (Player* plr = ToPlayer())
	{
		...
		if (pet)
		{
			...
			if (bg && bg->isArena()) pet->SetFlag(UNIT_FIELD_FLAGS,
					UNIT_FLAG_STUNNED);
			else plr->UnsummonPetTemporaryIfAny();
		}
