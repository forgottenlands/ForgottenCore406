...
#pragma warning(default:4355)
    ...
    m_ObjectSlot [0] = m_ObjectSlot [1] = m_ObjectSlot [2] = m_ObjectSlot [3] =
            0;
...
void Unit::Update(uint32 p_time)
{
    ...
    if (CanHaveThreatList() && getThreatManager().isNeedUpdateToClient(p_time)) SendThreatListUpdate();
                    ...
                    || (ToCreature()->isPet() && IsControlledByPlayer())))
    {
        ...
        if (m_HostileRefManager.isEmpty())
        {
            ...
            if (m_CombatTimer <= p_time) ClearInCombat();
            else m_CombatTimer -= p_time;
        }
    }
    ...
    {
        if (uint32 base_att = getAttackTimer(BASE_ATTACK)) setAttackTimer(
                BASE_ATTACK, (p_time >= base_att ? 0 : base_att - p_time));
        if (uint32 ranged_att = getAttackTimer(RANGED_ATTACK)) setAttackTimer(
                RANGED_ATTACK,
                (p_time >= ranged_att ? 0 : ranged_att - p_time));
        if (uint32 off_att = getAttackTimer(OFF_ATTACK)) setAttackTimer(
                OFF_ATTACK, (p_time >= off_att ? 0 : off_att - p_time));
    }
...
{
    if (GetTypeId() == TYPEID_PLAYER) return this->ToPlayer()->GetWeaponForAttack(
            OFF_ATTACK, true);
    else return m_canDualWield;
}

void Unit::SendMonsterMoveWithSpeedToCurrentDestination(Player* player)
{
    float x, y, z;
    if (GetMotionMaster()->GetDestination(x, y, z)) SendMonsterMoveWithSpeed(x,
            y, z, 0, player);
}
        ...
        uint32 Time, Player* player)
{
    ...
    if (player) player->GetSession()->SendPacket(&data);
...
{
    WorldPacket data(SMSG_MONSTER_MOVE,
            GetPackGUID().size() + 1 + 12 + 4 + 1 + 4 + 8 + 4 + 4 + 12);
    data.append(GetPackGUID());
...
void Unit::UpdateInterruptMask()
{
    ...
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL]) if (spell->getState()
            == SPELL_STATE_CASTING) m_interruptMask |=
        ...
        uint32 familyFlags) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end();
            ++itr)
        if (SpellEntry const *iterSpellProto = (*itr)->GetSpellProto()) if (iterSpellProto->SpellFamilyName
                == familyName
                && iterSpellProto->SpellFamilyFlags [0] & familyFlags) return true;
    return false;
}
...
void Unit::DealDamageMods(Unit *pVictim, uint32 &damage, uint32* absorb)
{
    ...
    if (absorb && originalDamage > damage) *absorb += (originalDamage - damage);
}
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage,
        ...
        bool durabilityLoss)
{
    if (pVictim->IsAIEnabled) pVictim->GetAI()->DamageTaken(this, damage);

    if (IsAIEnabled) GetAI()->DamageDealt(pVictim, damage, damagetype);
    ...
    if (damagetype != NODAMAGE)
    {
                ...
                i != vCopyDamageCopy.end(); ++i)
        {
            ...
            if (!((*i)->GetBase()->IsAppliedOnTarget(pVictim->GetGUID()))) continue;
            // check damage school mask
            if (((*i)->GetMiscValue() & damageSchoolMask) == 0) continue;

            Unit * shareDamageTarget = (*i)->GetCaster();
            if (!shareDamageTarget) continue;
    ...
    if (GetTypeId() == TYPEID_PLAYER && this != pVictim)
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER) if (Battleground *bg = killer->GetBattleground()) bg->UpdatePlayerScore(
                killer, SCORE_DAMAGE_DONE, damage);
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER) pVictim->ToPlayer()->UpdateAchievementCriteria(
            ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_RECEIVED, damage);
    else if (!pVictim->IsControlledByPlayer() || pVictim->IsVehicle())
    {
        if (!pVictim->ToCreature()->hasLootRecipient()) pVictim->ToCreature()->SetLootRecipient(
                this);

        if (IsControlledByPlayer()) pVictim->ToCreature()->LowerPlayerDamageReq(
    ...
    if (health <= damage)
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            killer->GetAchievementMgr().UpdateAchievementCriteria(
                    ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL, 1, 0, pVictim);
        }
        if (this->ToPlayer() && this->isAlive()) // Trinkets Heirloom
        if (this->ToPlayer()->isHonorOrXPTarget(pVictim))
        {
            AuraEffectList const& heirloom = GetAuraEffectsByType(
                    SPELL_AURA_DUMMY);
            for (AuraEffectList::const_iterator j = heirloom.begin();
                    j != heirloom.end(); ++j)
            {
                if ((*j)->GetId() == 59915
                        && this->getPowerType() == POWER_MANA) this->CastSpell(
                        this, 59914, true);
                if ((*j)->GetId() == 59906)
                {
                    int32 bonushealth = this->GetMaxHealth()
                            * this->GetAura(59906)->GetEffect(0)->GetAmount()
                            / 100;
                    this->CastCustomSpell(this, 59913, &bonushealth, 0, 0,
                            true);
                }
            }
        }
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            if (damagetype == SPELL_DIRECT_DAMAGE)
            {
                ...
                {
                    if (this->ToPlayer()->HasAura(44546)) if (roll_chance_f(
                            5.0f)) this->CastSpell(this, 57761, true);

                    if (this->ToPlayer()->HasAura(44548)) if (roll_chance_f(
                            10.0f)) this->CastSpell(this, 57761, true);

                    if (this->ToPlayer()->HasAura(44549)) if (roll_chance_f(
                            15.0f)) this->CastSpell(this, 57761, true);
                }
            }
        }
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
                if (this->ToPlayer()->HasAura(51528)) if (roll_chance_f(10.0f)) this->CastSpell(
                        this, 53817, true);

                if (this->ToPlayer()->HasAura(51529)) if (roll_chance_f(20.0f)) this->CastSpell(
                        this, 53817, true);

                if (this->ToPlayer()->HasAura(51530)) if (roll_chance_f(30.0f)) this->CastSpell(
                        this, 53817, true);
            }
        }
        ...
        if (damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE) pVictim->RemoveAurasWithInterruptFlags(
                AURA_INTERRUPT_FLAG_DIRECT_DAMAGE,
                spellProto ? spellProto->Id : 0);

        if (pVictim->GetTypeId() != TYPEID_PLAYER)
        {
            if (spellProto && IsDamageToThreatSpell(spellProto)) pVictim->AddThreat(
                    this, damage * 2.0f, damageSchoolMask, spellProto);
            else pVictim->AddThreat(this, (float) damage, damageSchoolMask,
                    spellProto);
        }
                ...
                if (damagetype != DOT)
                {
                    if (Spell* spell = pVictim->m_currentSpells[CURRENT_GENERIC_SPELL])
                    {
                        if (spell->getState() == SPELL_STATE_PREPARING)
                        {
                            ...
                            if (interruptFlags
                                    & SPELL_INTERRUPT_FLAG_ABORT_ON_DMG) pVictim->InterruptNonMeleeSpells(
                                    false);
                            else if (interruptFlags
                                    & SPELL_INTERRUPT_FLAG_PUSH_BACK) spell->Delayed();
                        }
                    }
                }
                ...
                if (Spell* spell = pVictim->m_currentSpells[CURRENT_CHANNELED_SPELL])
                {
                    if (spell->getState() == SPELL_STATE_CASTING)
                    {
                        ...
                        if (((channelInterruptFlags & CHANNEL_FLAG_DELAY) != 0)
                                && (damagetype != DOT)) spell->DelayedChannel();
                    }
                }
            }
        }
...
void Unit::CastStop(uint32 except_spellid)
{
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
        if (m_currentSpells [i]
                && m_currentSpells [i]->m_spellInfo->Id != except_spellid) InterruptSpell(
                CurrentSpellTypes(i), false);
}
        ...
        uint64 originalCaster)
{
    ...
    if (!originalCaster && GetTypeId() == TYPEID_UNIT
            && this->ToCreature()->isTotem() && IsControlledByPlayer()) if (Unit * owner = GetOwner()) originalCaster =
            owner->GetGUID();
    ...
    if (!originalCaster && triggeredByAura) originalCaster =
        ...
        uint64 originalCaster, Unit* OriginalVictim)
{
    ...
    if (OriginalVictim) targets.setUnitTarget(OriginalVictim);
    spell->m_CastItem = castItem;
    spell->prepare(&targets, triggeredByAura);
}
        ...
        bool crit)
{
    ...
    if (spellInfo->AttributesEx4 & SPELL_ATTR4_FIXED_DAMAGE)
    {
        ...
        if (!pVictim || !pVictim->isAlive()) return;
    ...
    if (IsDamageReducedByArmor(damageSchoolMask, spellInfo)) damage =
            CalcArmorReducedDamage(pVictim, damage, spellInfo, attackType);
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (crit)
            {
                ...
                if (attackType == RANGED_ATTACK) critPctDamageMod +=
                ...
                if (critPctDamageMod != 0) damage = int32(
            ...
            if (blocked)
            {
                ...
                if (pVictim->isBlockCritical()) damageInfo->blocked +=
                        damageInfo->blocked;
                if (damage < int32(damageInfo->blocked)) damageInfo->blocked =
                        uint32(damage);
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    if (!pVictim->isAlive() || pVictim->HasUnitState(UNIT_STAT_IN_FLIGHT)
            || (pVictim->HasUnitState(UNIT_STAT_ONVEHICLE)
                    && pVictim->GetVehicleBase() != this)
            || (pVictim->GetTypeId() == TYPEID_UNIT
                    && pVictim->ToCreature()->IsInEvadeMode())) return;
        ...
        CalcDamageInfo *damageInfo, WeaponAttackType attackType)
{
    ...
    if (!pVictim) return;
    if (!isAlive() || !pVictim->isAlive()) return;
    ...
    switch (damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_CRIT:
        {
            ...
            int32 mod = 0;
            // Apply SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_DAMAGE or SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_DAMAGE
            if (damageInfo->attackType == RANGED_ATTACK) mod +=
                    damageInfo->target->GetTotalAuraModifier(
                            SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_DAMAGE);
            else
            {
            ...
            mod += GetTotalAuraModifierByMiscMask(
                    SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, crTypeMask);
            if (mod != 0) damageInfo->damage = int32(
        ...
        case MELEE_HIT_BLOCK:
        {
            ...
            damageInfo->blocked_amount =
                    damageInfo->target->GetShieldBlockValue();
            //double blocked amount if block is critical
            if (damageInfo->target->isBlockCritical()) damageInfo->blocked_amount +=
                    damageInfo->blocked_amount;
            ...
            else damageInfo->procEx |= PROC_EX_NORMAL_HIT;
    ...
    if (int32(damageInfo->damage) > 0)
    {
        ...
        if (damageInfo->resist) damageInfo->HitInfo |= HITINFO_RESIST;
    }
    else
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->HitInfo & HITINFO_CRITICALHIT) pVictim->HandleEmoteCommand(
            EMOTE_ONESHOT_WOUNDCRITICAL);
    if (damageInfo->blocked_amount
            && damageInfo->TargetState != VICTIMSTATE_BLOCKS) pVictim->HandleEmoteCommand(
    ...
    if (damageInfo->TargetState == VICTIMSTATE_PARRY)
    {
        ...
        if (pVictim->haveOffhandWeapon() && offtime < basetime)
        {
            ...
            if (offtime > percent20 && offtime <= percent60) pVictim->setAttackTimer(
                    OFF_ATTACK, uint32(percent20));
        ...
        else
        {
            ...
            float percent60 = 3.0f * percent20;
            if (basetime > percent20 && basetime <= percent60) pVictim->setAttackTimer(
                    BASE_ATTACK, uint32(percent20));
                    ...
                    || !pVictim->ToCreature()->isWorldBoss()))
    {
        ...
        if (pVictim->getLevel() < 30) Probability = 0.65f * pVictim->getLevel()
                + 0.5f;
        ...
        if (Probability > 40.0f) Probability = 40.0f;

        if (roll_chance_f(Probability)) CastSpell(pVictim, 1604, true);
    }

    if (GetTypeId() == TYPEID_PLAYER) ToPlayer()->CastItemCombatSpell(pVictim,
            damageInfo->attackType, damageInfo->procVictim, damageInfo->procEx);
        ...
        SpellEntry const *spellInfo, uint8 effIndex)
{
    ...
    if (spellInfo)
    {
        ...
        if (sSpellMgr->GetSpellCustomAttr(spellInfo->Id)
                & SPELL_ATTR0_CU_IGNORE_ARMOR) return false;

        // bleeding effects are not reduced by armor
        if (effIndex != MAX_SPELL_EFFECTS
                && spellInfo->EffectApplyAuraName [effIndex]
                        == SPELL_AURA_PERIODIC_DAMAGE) if (GetSpellMechanicMask(
                spellInfo, effIndex) & (1 << MECHANIC_BLEED)) return false;
    }
    return true;
}
        ...
        SpellEntry const *spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    AuraEffectList const & reductionAuras = pVictim->GetAuraEffectsByType(
            SPELL_AURA_BYPASS_ARMOR_FOR_CASTER);
    for (AuraEffectList::const_iterator i = reductionAuras.begin();
            i != reductionAuras.end(); ++i)
        if ((*i)->GetCasterGUID() == GetGUID()) auraEffectivenessReduction +=
                (*i)->GetAmount();
    armor = CalculatePctN(armor,
            100 - std::min(auraEffectivenessReduction, 100));

    // Ignore enemy armor by SPELL_AURA_MOD_TARGET_RESISTANCE aura
    armor += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_TARGET_RESISTANCE,
            SPELL_SCHOOL_MASK_NORMAL);

    if (spellInfo) if (Player *modOwner = GetSpellModOwner()) modOwner->ApplySpellMod(
            ...
            j != ResIgnoreAurasAb.end(); ++j)
    {
        if ((*j)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL
                && (*j)->IsAffectedOnSpell(spellInfo)) armor = floor(
    ...
    {
        if ((*j)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) armor = floor(
                float(armor) * (float(100 - (*j)->GetAmount()) / 100.0f));
    }
    ...
    {
        float maxArmorPen = 0;
        if (getLevel() < 60) maxArmorPen = float(
                400 + 85 * pVictim->getLevel());
        else maxArmorPen = 400 + 85 * pVictim->getLevel()
    ...
    if (getLevel() > 59) armorReduction = armor
            / (armor + 467.5f * getLevel() - 22167.5f);
    if (getLevel() > 80) armorReduction = armor
            / (armor + 2167.5f * getLevel() - 158167.5f);

    if (armorReduction < 0.0f) armorReduction = 0.0f;
...
{
    if (!pVictim || !pVictim->isAlive() || !damage) return;
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
    {
        ...
        for (uint32 i = 0; i < 11; ++i)
        {
            ...
            if (discreteResistProbability [i] < 0.0f) discreteResistProbability [i] =
                    0.0f;
        }
        ...
        for (AuraEffectList::const_iterator j = ResIgnoreAurasAb.begin();
            ...
            if (((*j)->GetMiscValue() & schoolMask)
                    && (*j)->IsAffectedOnSpell(spellInfo)) AddPctN(
                    damageResisted, -(*j)->GetAmount());

        AuraEffectList const &ResIgnoreAuras = GetAuraEffectsByType(
                SPELL_AURA_MOD_IGNORE_TARGET_RESIST);
        for (AuraEffectList::const_iterator j = ResIgnoreAuras.begin();
                j != ResIgnoreAuras.end(); ++j)
            if ((*j)->GetMiscValue() & schoolMask) AddPctN(damageResisted,
                    -(*j)->GetAmount());
            ...
            itr != AbsIgnoreAurasA.end(); ++itr)
    {
        ...
        if ((*itr)->GetAmount() > auraAbsorbMod) auraAbsorbMod = float(
                (*itr)->GetAmount());
    }
            ...
            ++itr)
    {
        ...
        AuraApplication const * aurApp =
                absorbAurEff->GetBase()->GetApplicationOfTarget(
                        pVictim->GetGUID());
        if (!aurApp) continue;
        if (!(absorbAurEff->GetMiscValue() & schoolMask)) continue;

        // get amount which can be still absorbed by the aura
        int32 currentAbsorb = absorbAurEff->GetAmount();
        // aura with infinite absorb amount - let the scripts handle absorbtion amount, set here to 0 for safety
        if (currentAbsorb < 0) currentAbsorb = 0;
        ...
        if (absorbAurEff->GetAmount() >= 0)
        {
            ...
            if (absorbAurEff->GetAmount() <= 0) absorbAurEff->GetBase()->Remove(
    ...
    for (AuraEffectList::const_iterator itr = vManaShieldCopy.begin();
            (itr != vManaShieldCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        AuraEffect * absorbAurEff = (*itr);
        // Check if aura was removed during iteration - we don't need to work on such auras
        AuraApplication const * aurApp =
        ...
        if (float manaMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(absorbAurEff->GetSpellProto(), absorbAurEff->GetEffIndex(), absorbAurEff->GetCaster())) manaReduction =
                int32(float(manaReduction) * manaMultiplier);
        ...
        uint32 &healAmount, uint32 &absorb)
{
            ...
            i != vHealAbsorb.end() && RemainingHeal > 0; ++i)
    {
        ...
        if (RemainingHeal < currentAbsorb) currentAbsorb = RemainingHeal;
        ...
        if ((*i)->GetAmount() <= 0) existExpired = true;
    }
    ...
    if (existExpired)
    {
                ...
                i != vHealAbsorb.end();)
        {
            ...
            if (auraEff->GetAmount() <= 0)
            {
                ...
                if (removedAuras + 1 < pVictim->m_removedAurasCount) i =
                        vHealAbsorb.begin();
            }
        }
    }
        ...
        bool extra)
{
    if (HasUnitState(UNIT_STAT_CANNOT_AUTOATTACK)
            || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED)) return;

    if (!pVictim->isAlive()) return;

    if ((attType == BASE_ATTACK || attType == OFF_ATTACK)
            && !IsWithinLOSInMap(pVictim) && !isPet()) return;

    CombatStart(pVictim);
    RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_MELEE_ATTACK);

    uint32 hitInfo;
    if (attType == BASE_ATTACK) hitInfo = HITINFO_NORMALSWING2;
    else if (attType == OFF_ATTACK) hitInfo = HITINFO_LEFTSWING;
    else return; // ignore ranged case

    // melee attack spell casted at main hand attack only - no normal melee dmg dealt
    if (attType == BASE_ATTACK && m_currentSpells [CURRENT_MELEE_SPELL]) m_currentSpells [CURRENT_MELEE_SPELL]->cast();
    else
    {
        ...
        sLog->outStaticDebug(
                ...
                damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount,
                damageInfo.resist);
        else
        sLog->outStaticDebug(
    ...
    if (!extra && m_extraAttacks)
    {
        while (m_extraAttacks)
        {
            ...
            if (m_extraAttacks > 0) --m_extraAttacks;
        }
    }
}
        ...
        int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    if (pVictim->GetTypeId() == TYPEID_UNIT
            && pVictim->ToCreature()->IsInEvadeMode()) return MELEE_HIT_EVADE;
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER) dodge_chance -= int32(
                this->ToPlayer()->GetExpertiseDodgeOrParryReduction(attType)
                        * 100);
        else dodge_chance -= GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE)
                * 25;
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER) parry_chance -= int32(
                this->ToPlayer()->GetExpertiseDodgeOrParryReduction(attType)
                        * 100);
        else parry_chance -= GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE)
                * 25;
...
float Unit::CalculateLevelPenalty(SpellEntry const* spellProto) const
{
    if (spellProto->spellLevel <= 0
            || spellProto->spellLevel >= spellProto->maxLevel) return 1.0f;

    float LvlPenalty = 0.0f;

    if (spellProto->spellLevel < 20) LvlPenalty = 20.0f
            - spellProto->spellLevel * 3.75f;
        ...
        WeaponAttackType attackType)
{
            ...
            || pVictim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_UNIT
                && pVictim->ToCreature()->GetCreatureInfo()->flags_extra
                        & CREATURE_FLAG_EXTRA_NO_BLOCK) return false;

        // These spells shouldn't be blocked
        if (spellProto
                && spellProto->Attributes
                        & SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK) return false;
        ...
        if (roll_chance_f(blockChance)) return true;
    }
    return false;
}
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    if (spell->DmgClass == SPELL_DAMAGE_CLASS_RANGED) attType = RANGED_ATTACK;

    int32 attackerWeaponSkill;
    // skill value for these spells (for example judgements) is 5* level
    if (spell->DmgClass == SPELL_DAMAGE_CLASS_RANGED
            && !IsRangedWeaponSpell(spell)) attackerWeaponSkill = getLevel()
            * 5;
    // bonus from skills is 0.04% per skill Diff
    else attackerWeaponSkill = int32(GetWeaponSkillValue(attType, pVictim));
    ...
    for (uint8 eff = 0; eff < MAX_SPELL_EFFECTS; ++eff)
    {
        ...
        if (effect_mech)
        {
            int32 temp = pVictim->GetTotalAuraModifierByMiscValue(
                    SPELL_AURA_MOD_MECHANIC_RESISTANCE, effect_mech);
            if (resist_mech < temp * 100) resist_mech = temp * 100;
        }
    }
    // Roll chance
    tmp += resist_mech;
    ...
    if (spell->Attributes & SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK) return SPELL_MISS_NONE;
            ...
            && !pVictim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER) canDodge = false;
    ...
    if (pVictim->GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (flagEx & CREATURE_FLAG_EXTRA_NO_PARRY) canParry = false;
        // Check creatures flags_extra for disable block
        if (flagEx & CREATURE_FLAG_EXTRA_NO_BLOCK) canBlock = false;
    }
    ...
    for (AuraEffectList::const_iterator i = ignore.begin(); i != ignore.end();
            ++i)
    {
        if (!(*i)->IsAffectedOnSpell(spell)) continue;
    ...
    if (canDodge)
    {
        ...
        dodgeChance = int32(
                float(dodgeChance)
                        * GetTotalAuraMultiplier(SPELL_AURA_MOD_ENEMY_DODGE));
        // Reduce dodge chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER) dodgeChance -= int32(
                this->ToPlayer()->GetExpertiseDodgeOrParryReduction(attType)
                        * 100.0f);
        else dodgeChance -= GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE) * 25;
        if (dodgeChance < 0) dodgeChance = 0;

        if (roll < (tmp += dodgeChance)) return SPELL_MISS_DODGE;
    }
    ...
    if (canParry)
    {
        ...
        int32 parryChance = int32(pVictim->GetUnitParryChance() * 100.0f)
                - skillDiff * 4;
        // Reduce parry chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER) parryChance -= int32(
                this->ToPlayer()->GetExpertiseDodgeOrParryReduction(attType)
                        * 100.0f);
        else parryChance -= GetTotalAuraModifier(SPELL_AURA_MOD_EXPERTISE) * 25;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    if (!pVictim->isAlive() && pVictim->GetTypeId() != TYPEID_PLAYER) return SPELL_MISS_NONE;
    ...
    int32 modHitChance;
    if (leveldif < 3) modHitChance = 96 - leveldif;
    else modHitChance = 94 - (leveldif - 2) * lchance;

    // Spellmod from SPELLMOD_RESIST_MISS_CHANCE
    if (Player *modOwner = GetSpellModOwner()) modOwner->ApplySpellMod(
            spell->Id, SPELLMOD_RESIST_MISS_CHANCE, modHitChance);
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER) HitChance -= int32(
        ...
        bool CanReflect)
{
    ...
    if (pVictim->GetTypeId() == TYPEID_UNIT
            && pVictim->ToCreature()->IsInEvadeMode() && this != pVictim) return SPELL_MISS_EVADE;

    // Check for immune
    if (pVictim->IsImmunedToSpell(spell)) return SPELL_MISS_IMMUNE;
    ...
    return SPELL_MISS_NONE;
    // Check for immune
    if (pVictim->IsImmunedToDamage(spell)) return SPELL_MISS_IMMUNE;

    if (this == pVictim) return SPELL_MISS_NONE;
    ...
    if (CanReflect)
    {
        ...
        for (Unit::AuraEffectList::const_iterator i =
                ...
                ++i)
            if ((*i)->GetMiscValue() & GetSpellSchoolMask(spell)) reflectchance +=
                    (*i)->GetAmount();
...
uint32 Unit::GetDefenseSkillValue(Unit const* target) const
{
    ...
    else return GetUnitMeleeSkill(target);
}

float Unit::GetUnitDodgeChance() const
{
    if (IsNonMeleeSpellCasted(false) || HasUnitState(UNIT_STAT_STUNNED)) return 0.0f;

    if (GetTypeId() == TYPEID_PLAYER) return GetFloatValue(
            PLAYER_DODGE_PERCENTAGE);
    else
    {
        if (((Creature const*) this)->isTotem()) return 0.0f;
        else
        {
            float dodge = 5.0f;
...
float Unit::GetUnitParryChance() const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (player->CanParry())
        {
            ...
            if (!tmpitem) tmpitem = player->GetWeaponForAttack(OFF_ATTACK,
                    true);

            if (tmpitem) chance = GetFloatValue(PLAYER_PARRY_PERCENTAGE);
        }
    }
...
float Unit::GetUnitBlockChance() const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (player->CanBlock())
        {
            ...
            if (tmpitem && !tmpitem->IsBroken()) return GetFloatValue(
        ...
        const Unit *pVictim) const
{
    ...
    // flat aura mods
    if (attackType == RANGED_ATTACK) crit += pVictim->GetTotalAuraModifier(
            SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_CHANCE);
    else crit += pVictim->GetTotalAuraModifier(
    ...
    {
        // Glyph of barkskin
        if (pVictim->HasAura(63057) && pVictim->HasAura(22812)) crit -= 25.0f;
    }
        ...
        Unit const* target) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (attType != BASE_ATTACK && !item) return 0;

        if (IsInFeralForm()) return GetMaxSkillValueForLevel(); // always maximized SKILL_FERAL_COMBAT in fact

        // weapon skill or (unarmed for base attack and fist weapons)
        uint32 skill;
        if (item && item->GetSkill() != SKILL_FIST_WEAPONS) skill =
                item->GetSkill();
        else skill = SKILL_UNARMED;
    ...
    else value = GetUnitMeleeSkill(target);
    return value;
}
...
{
    if (m_currentSpells [CURRENT_AUTOREPEAT_SPELL]) _UpdateAutoRepeatSpell();
    ...
    {
        if (i->second->IsExpired()) RemoveOwnedAura(i, AURA_REMOVE_BY_EXPIRE);
        else ++i;
    }

    for (VisibleAuraMap::iterator itr = m_visibleAuras.begin();
            itr != m_visibleAuras.end(); ++itr)
        if (itr->second->IsNeedClientUpdate()) itr->second->ClientUpdate();
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    if (isAttackReady(RANGED_ATTACK))
    {
        ...
        spell->prepare(
                &(m_currentSpells [CURRENT_AUTOREPEAT_SPELL]->m_targets));
...
void Unit::SetCurrentCastedSpell(Spell * pSpell)
{
        ...
        case CURRENT_GENERIC_SPELL:
        {
            ...
            if (m_currentSpells [CURRENT_AUTOREPEAT_SPELL])
            {
                ...
                if (m_currentSpells [CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id
                        != 75) InterruptSpell(CURRENT_AUTOREPEAT_SPELL);
                m_AutoRepeatFirstCast = true;
            }
    ...
    if (m_currentSpells [CSpellType]) m_currentSpells [CSpellType]->SetReferencedFromCurrent(
            false);
...
void Unit::FinishSpell(CurrentSpellTypes spellType, bool ok /*= true*/)
{
    ...
    if (spellType == CURRENT_CHANNELED_SPELL) spell->SendChannelUpdate(0);
        ...
        bool skipAutorepeat, bool isAutoshoot, bool skipInstant) const
{
    ...
    if (!skipInstant && m_currentSpells [CURRENT_GENERIC_SPELL]
            && !m_currentSpells [CURRENT_GENERIC_SPELL]->GetCastTime()) return false;
                            ...
                            != SPELL_STATE_DELAYED))
    {
        if (!isAutoshoot
                || !(m_currentSpells [CURRENT_GENERIC_SPELL]->m_spellInfo->AttributesEx2
                        & SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS)) return (true);
    }
    ...
    else if (!skipAutorepeat && m_currentSpells [CURRENT_AUTOREPEAT_SPELL]) return (true);
    ...
    if (m_currentSpells [CURRENT_GENERIC_SPELL]
            && (!spell_id
                    || m_currentSpells [CURRENT_GENERIC_SPELL]->m_spellInfo->Id
                            == spell_id)) InterruptSpell(CURRENT_GENERIC_SPELL,
            withDelayed, withInstant);

    // autorepeat spells are interrupted if they are not finished or delayed
    if (m_currentSpells [CURRENT_AUTOREPEAT_SPELL]
            && (!spell_id
                    || m_currentSpells [CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id
                            == spell_id)) InterruptSpell(
            CURRENT_AUTOREPEAT_SPELL, withDelayed, withInstant);
    ...
    if (m_currentSpells [CURRENT_CHANNELED_SPELL]
            && (!spell_id
                    || m_currentSpells [CURRENT_CHANNELED_SPELL]->m_spellInfo->Id
                            == spell_id)) InterruptSpell(
            CURRENT_CHANNELED_SPELL, true, true);
}
...
bool Unit::CanCastWhileWalking(const SpellEntry * const sp)
{
    ...
    for (AuraEffectList::const_iterator i = alist.begin(); i != alist.end(); ++i)
    {
        ...
        if (!((*i)->GetSpellProto()->EffectSpellClassMask[(*i)->GetEffIndex()] & sp->SpellFamilyFlags)) continue;
        return true;
    }
    return false;
}
...
Spell* Unit::FindCurrentSpellBySpellId(uint32 spell_id) const
{
    for (uint32 i = 0; i < CURRENT_MAX_SPELL; i++)
        if (m_currentSpells [i]
                && m_currentSpells [i]->m_spellInfo->Id == spell_id) return m_currentSpells [i];
    return NULL;
}

int32 Unit::GetCurrentSpellCastTime(uint32 spell_id) const
{
    if (Spell const * spell = FindCurrentSpellBySpellId(spell_id)) return spell->GetCastTime();
    return 0;
}
...
bool Unit::isInAccessiblePlaceFor(Creature const* c) const
{
    ...
    else return c->canWalk() || c->canFly();
}
...
AuraApplication * Unit::_CreateAuraApplication(Aura * aura, uint8 effMask)
{
    ...
    if (!isAlive() && !IsDeathPersistentSpell(aurSpellInfo)
            && (GetTypeId() != TYPEID_PLAYER
                    || !this->ToPlayer()->GetSession()->PlayerLoading())) return NULL;
    ...
    if (AuraState aState = GetSpellAuraState(aura->GetSpellProto())) m_auraStateAuras.insert(
            AuraStateAurasMap::value_type(aState, aurApp));
...
void Unit::_ApplyAuraEffect(Aura * aura, uint8 effIndex)
{
    ...
    ASSERT(aurApp);
    if (!aurApp->GetEffectMask()) _ApplyAura(aurApp, 1 << effIndex);
    else aurApp->_HandleEffect(effIndex, true);
}
...
void Unit::_ApplyAura(AuraApplication * aurApp, uint8 effMask)
{
    ...
    if (aurApp->GetRemoveMode()) return;

    // Update target aura state flag
    if (AuraState aState = GetSpellAuraState(aura->GetSpellProto())) ModifyAuraState(
            aState, true);
    ...
    if (aura->GetSpellProto()->AuraInterruptFlags
            & AURA_INTERRUPT_FLAG_NOT_SEATED && !IsSitState()) SetStandState(
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (effMask & 1 << i && (!aurApp->GetRemoveMode())) aurApp->_HandleEffect(
                i, true);
    }
}
        ...
        AuraRemoveMode removeMode)
{
    ...
    {
        if (aurApp->HasEffect(itr)) aurApp->_HandleEffect(itr, false);
    }
            ...
            && this->ToTotem()->GetSummonerGUID() == aura->GetCasterGUID())
    {
        if (this->ToTotem()->GetSpell() == aura->GetId()
                && this->ToTotem()->GetTotemType() == TOTEM_PASSIVE) this->ToTotem()->setDeathState(
                JUST_DIED);
    }

    // Remove aurastates only if were not found
    if (!auraStateFound) ModifyAuraState(auraState, false);
...
void Unit::_RemoveNoStackAuraApplicationsDueToAura(Aura * aura)
{
    ...
    if (aura->GetType() == DYNOBJ_AURA_TYPE) return;
    ...
    if (IsPassiveSpell(spellId) && IsPassiveSpellStackableWithRanks(spellProto)) return;
            ...
            i != m_appliedAuras.end(); ++i)
    {
        ...
        if (!_IsNoStackAuraDueToAura(aura, i->second->GetBase())) continue;

        RemoveAura(i, AURA_REMOVE_BY_DEFAULT);
        if (i == m_appliedAuras.end()) break;
        remove = true;
    }
}
...
void Unit::_RemoveNoStackAurasDueToAura(Aura * aura)
{
            ...
            ++i)
    {
        ...
        if (!_IsNoStackAuraDueToAura(aura, i->second)) continue;

        RemoveOwnedAura(i, AURA_REMOVE_BY_DEFAULT);
        if (i == m_ownedAuras.end()) break;
        remove = true;
    }
}
        ...
        Aura * existingAura) const
{
    ...
    if (existingAura == appliedAura) return false;

    // Do not check dynobj auras for stacking
    if (existingAura->GetType() != UNIT_AURA_TYPE) return false;
    ...
    if (IsPassiveSpell(i_spellId))
    {
        ...
        if (!sSpellMgr->IsRankSpellDueToSpell(spellProto, i_spellId)) return false;
    }
    ...
    if (is_triggered_by_spell) return false;

    if (sSpellMgr->CanAurasStack(appliedAura, existingAura, sameCaster)) return false;
    return true;
}

void Unit::_RegisterAuraEffect(AuraEffect * aurEff, bool apply)
{
    if (apply) m_modAuras [aurEff->GetAuraType()].push_back(aurEff);
    else m_modAuras [aurEff->GetAuraType()].remove(aurEff);
}
...
void Unit::RemoveOwnedAura(AuraMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (m_auraUpdateIterator == i) ++m_auraUpdateIterator;

    m_ownedAuras.erase(i);
    m_removedAuras.push_back(aura);

    // Unregister single target aura
    if (aura->IsSingleTarget()) aura->UnregisterSingleTarget();
        ...
        uint64 itemCasterGUID, uint8 reqEffMask, Aura* except) const
{
    for (AuraMap::const_iterator itr = m_ownedAuras.lower_bound(spellId);
        ...
        if (((itr->second->GetEffectMask() & reqEffMask) == reqEffMask)
                ...
                && (!except || except != itr->second)) return itr->second;
    return NULL;
}
...
void Unit::RemoveAura(AuraApplication * aurApp, AuraRemoveMode mode)
{
    ASSERT(aurApp->GetBase()->GetApplicationOfTarget(GetGUID()) == aurApp);
    // no need to remove
    if (aurApp->GetRemoveMode() || aurApp->GetBase()->IsRemoved()) return;
    uint32 spellId = aurApp->GetBase()->GetId();
    for (AuraApplicationMap::iterator iter = m_appliedAuras.lower_bound(
            spellId); iter != m_appliedAuras.upper_bound(spellId);)
    {
...
{
    if (aura->IsRemoved()) return;
    if (AuraApplication * aurApp = aura->GetApplicationOfTarget(GetGUID())) RemoveAura(
            aurApp, mode);
    else
    ASSERT(false);
}
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
                    ...
                    && (aura->GetSpellProto()->SpellFamilyFlags [0] & 0x10000000))
            {
                ...
                if (caster)
                {
                    ...
                    if (triggeredSpellId) caster->CastSpell(caster,
                            triggeredSpellId, true);
                }
            }
            // Wyvern Sting
                    ...
                    && (aura->GetSpellProto()->SpellFamilyFlags [1] & 0x1000))
            {
                ...
                // Noxious Stings
                if (AuraEffect * auraEff = caster->GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS, SPELLFAMILY_HUNTER, 3521, 1)) if (Aura * newAura = caster->AddAura(aura->GetId(), dispeller)) newAura->SetDuration(
                        aura->GetDuration() / 100 * auraEff->GetAmount());
            }
            return;
        }
        else ++iter;
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                if (aura->GetEffect(i))
                {
                    ...
                    effMask |= (1 << i);
                    if (aura->GetEffect(i)->CanBeRecalculated()) recalculateMask |=
                            (1 << i);
                }
                else
                {
            ...
            if (Aura * newAura = (stealCharge || stealStack) ? stealer->GetAura(aura->GetId(), aura->GetCasterGUID()) : NULL)
            {
                if (stealCharge)
                {
                    ...
                    if (caster) if (Player* modOwner = caster->GetSpellModOwner()) modOwner->ApplySpellMod(
                            aura->GetId(), SPELLMOD_CHARGES, maxCharges);
                    newAura->SetCharges(
            ...
            if (stealCharge) aura->DropCharge();
...
void Unit::RemoveAurasWithAttribute(uint32 flags)
{
            ...
            iter != m_appliedAuras.end();)
    {
        ...
        if (spell->Attributes & flags) RemoveAura(iter);
        else ++iter;
    }
}
...
void Unit::RemoveNotOwnSingleTargetAuras(uint32 newPhase)
{
            ...
            iter != m_appliedAuras.end();)
    {
                ...
                && IsSingleTargetSpell(aura->GetSpellProto()))
        {
            ...
            {
                Unit* caster = aura->GetCaster();
                if (!caster || !caster->InSamePhase(newPhase)) RemoveAura(iter);
                else ++iter;
            }
        }
        else ++iter;
...
{
    if (!(m_interruptMask & flag)) return;
    ...
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL]) if (spell->getState()
            ...
            && spell->m_spellInfo->Id != except) InterruptNonMeleeSpells(false);
            ...
            iter != m_appliedAuras.end();)
    {
        ...
        if (!except || aura->GetId() != except)
        {
            if (GetAllSpellMechanicMask(aura->GetSpellProto()) & mechanic_mask)
            {
                RemoveAura(iter, removemode);
                auracount++;
                if (count && auracount == count) break;
                continue;
            }
        }
        ++iter;
...
void Unit::RemoveAllAurasOnDeath()
{
    ...
    {
        Aura const * aura = iter->second->GetBase();
        if (!aura->IsPassive() && !aura->IsDeathPersistent()) _UnapplyAura(iter,
    ...
    {
        Aura * aura = iter->second;
        if (!aura->IsPassive() && !aura->IsDeathPersistent()) RemoveOwnedAura(
                iter, AURA_REMOVE_BY_DEATH);
        else ++iter;
    }
}
...
void Unit::RemoveAllAurasRequiringDeadTarget()
{
            ...
            iter != m_appliedAuras.end();)
    {
        Aura const * aura = iter->second->GetBase();
        if (!aura->IsPassive()
                && IsRequiringDeadTargetSpell(aura->GetSpellProto())) _UnapplyAura(
                iter, AURA_REMOVE_BY_DEFAULT);
        else ++iter;
    }
            ...
            iter != m_ownedAuras.end();)
    {
        Aura * aura = iter->second;
        if (!aura->IsPassive()
                && IsRequiringDeadTargetSpell(aura->GetSpellProto())) RemoveOwnedAura(
                iter, AURA_REMOVE_BY_DEFAULT);
        else ++iter;
    }
}
...
void Unit::DelayOwnedAuras(uint32 spellId, uint64 caster, int32 delaytime)
{
            ...
            iter != m_ownedAuras.upper_bound(spellId); ++iter)
    {
        ...
        {
            if (aura->GetDuration() < delaytime) aura->SetDuration(0);
            else aura->SetDuration(aura->GetDuration() - delaytime);
        ...
        uint64 caster) const
{
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.lower_bound(
        ...
        if (itr->second->HasEffect(effIndex)
                && (!caster || itr->second->GetBase()->GetCasterGUID() == caster)) return itr->second->GetBase()->GetEffect(
                effIndex);
    return NULL;
}
        ...
        uint64 caster) const
{
    ...
    while (true)
    {
        if (AuraEffect * aurEff = GetAuraEffect(rankSpell, effIndex, caster)) return aurEff;
        SpellChainNode const * chainNode = sSpellMgr->GetSpellChainNode(
                rankSpell);
        if (!chainNode) break;
        else rankSpell = chainNode->next;
    }
    return NULL;
}
            ...
            ++i)
    {
        ...
        if (spell->SpellFamilyName == uint32(family)
                && spell->SpellFamilyFlags.HasFlag(familyFlag1, familyFlag2,
                        familyFlag3))
        {
            if (casterGUID && (*i)->GetCasterGUID() != casterGUID) continue;
            return (*i);
        }
    }
    return NULL;
...
AuraApplication * Unit::GetAuraApplication(uint32 spellId, uint64 casterGUID,
        uint64 itemCasterGUID, uint8 reqEffMask, AuraApplication * except) const
{
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.lower_bound(
            spellId); itr != m_appliedAuras.upper_bound(spellId); ++itr)
    {
        ...
        AuraApplication* except) const
{
    ...
    while (true)
    {
        if (AuraApplication * aurApp = GetAuraApplication(rankSpell, casterGUID, itemCasterGUID, reqEffMask, except)) return aurApp;
        SpellChainNode const * chainNode = sSpellMgr->GetSpellChainNode(
                rankSpell);
        if (!chainNode) break;
        else rankSpell = chainNode->next;
...
bool Unit::HasAuraEffect(uint32 spellId, uint8 effIndex, uint64 caster) const
{
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.lower_bound(
            spellId); itr != m_appliedAuras.upper_bound(spellId); ++itr)
        if (itr->second->HasEffect(effIndex)
                && (!caster || itr->second->GetBase()->GetCasterGUID() == caster)) return true;
    return false;
}
...
uint32 Unit::GetAuraCount(uint32 spellId) const
{
    ...
    for (AuraApplicationMap::const_iterator itr = m_appliedAuras.lower_bound(
            spellId); itr != m_appliedAuras.upper_bound(spellId); ++itr)
    {
        if (!itr->second->GetBase()->GetStackAmount()) count++;
        else count += (uint32) itr->second->GetBase()->GetStackAmount();
    }
    return count;
}
...
bool Unit::HasAuraTypeWithCaster(AuraType auratype, uint64 caster) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
        ...
        if (caster == (*i)->GetCasterGUID()) return true;
    return false;
}
...
bool Unit::HasAuraTypeWithMiscvalue(AuraType auratype, int32 miscvalue) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
        if (miscvalue == (*i)->GetMiscValue()) return true;
    return false;
}
        ...
        SpellEntry const * affectedSpell) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
        if ((*i)->IsAffectedOnSpell(affectedSpell)) return true;
    return false;
}
...
bool Unit::HasAuraTypeWithValue(AuraType auratype, int32 value) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
        if (value == (*i)->GetAmount()) return true;
    return false;
}

bool Unit::HasNegativeAuraWithInterruptFlag(uint32 flag, uint64 guid)
{
    if (!(m_interruptMask & flag)) return false;
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin();
            iter != m_interruptableAuras.end(); ++iter)
    {
        if (!(*iter)->IsPositive()
                && (*iter)->GetBase()->GetSpellProto()->AuraInterruptFlags
                        & flag
                && (!guid || (*iter)->GetBase()->GetCasterGUID() == guid)) return true;
    }
    return false;
}
...
bool Unit::HasNegativeAuraWithAttribute(uint32 flag, uint64 guid)
{
            ...
            iter != m_appliedAuras.end(); ++iter)
    {
        ...
        if (!iter->second->IsPositive()
                ...
                && (!guid || aura->GetCasterGUID() == guid)) return true;
    }
    return false;
}
        ...
        int32 script) const
{
    ...
    for (AuraEffectList::const_iterator i = auras.begin(); i != auras.end();
            ++i)
    {
        if ((*i)->GetMiscValue() == script) if ((*i)->IsAffectedOnSpell(spell)) return (*i);
    }
    return NULL;
}
...
uint32 Unit::GetDoTsByCaster(uint64 casterGUID) const
{
            ...
            itr && itr [0] != SPELL_AURA_NONE; ++itr)
    {
        ...
        {
            // Get auras by caster
            if ((*i)->GetCasterGUID() == casterGUID) ++dots;
        }
    }
    return dots;
}
...
int32 Unit::GetMaxPositiveAuraModifier(AuraType auratype)
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetAmount() > modifier) modifier = (*i)->GetAmount();
    }
        ...
        uint32 misc_mask) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() & misc_mask) modifier += (*i)->GetAmount();
    }
    return modifier;
}
        ...
        uint32 misc_mask) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() & misc_mask) multiplier *= (100.0f
                + (*i)->GetAmount()) / 100.0f;
    }
    return multiplier;
}
        ...
        uint32 misc_mask, const AuraEffect* except) const
{
            ...
            i != mTotalAuraList.end(); ++i)
    {
        if (except != (*i) && (*i)->GetMiscValue() & misc_mask
                && (*i)->GetAmount() > modifier) modifier = (*i)->GetAmount();
    }
        ...
        uint32 misc_mask) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() & misc_mask && (*i)->GetAmount() < modifier) modifier =
                (*i)->GetAmount();
    }
        ...
        int32 misc_value) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value) modifier += (*i)->GetAmount();
    }
    return modifier;
}
        ...
        int32 misc_value) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value) multiplier *= (100.0f
                + (*i)->GetAmount()) / 100.0f;
    }
    return multiplier;
}
        ...
        int32 misc_value) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value && (*i)->GetAmount() > modifier) modifier =
                (*i)->GetAmount();
    }
        ...
        SpellEntry const * affectedSpell) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->IsAffectedOnSpell(affectedSpell)) modifier +=
                (*i)->GetAmount();
    }
    return modifier;
}
        ...
        SpellEntry const * affectedSpell) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin();
            i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->IsAffectedOnSpell(affectedSpell)) multiplier *= (100.0f
                + (*i)->GetAmount()) / 100.0f;
    }
    return multiplier;
}
...
{
    if (m_dynObj.empty()) return NULL;
    for (DynObjectList::const_iterator i = m_dynObj.begin();
            i != m_dynObj.end(); ++i)
    {
        DynamicObject* dynObj = *i;

        if (dynObj->GetSpellId() == spellId) return dynObj;
    }
    return NULL;
}
...
GameObject* Unit::GetGameObject(uint32 spellId) const
{
    for (GameObjectList::const_iterator i = m_gameObj.begin();
            i != m_gameObj.end(); ++i)
        if ((*i)->GetSpellId() == spellId) return *i;
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && gameObj->GetSpellId())
    {
        ...
        this->ToPlayer()->AddSpellAndCategoryCooldowns(createBySpell, 0, NULL,
                true);
    }
}
        ...
        uint32 Resist, bool PhysicalDamage, uint32 Blocked, bool CriticalHit)
{
    ...
    log.HitInfo = SPELL_HIT_TYPE_UNK1 | SPELL_HIT_TYPE_UNK3
            | SPELL_HIT_TYPE_UNK6;
    if (CriticalHit) log.HitInfo |= SPELL_HIT_TYPE_CRIT;
        ...
        SpellEntry const * procAura)
{
    ...
    if (procAttacker) ProcDamageAndSpellFor(false, pVictim, procAttacker,
            procExtra, attType, procSpell, amount, procAura);
    // Now go on with a victim's events'n'auras
    // Not much to do if no flags are set or there is no victim
    if (pVictim && pVictim->isAlive() && procVictim) pVictim->ProcDamageAndSpellFor(
            true, this, procVictim, procExtra, attType, procSpell, amount,
            procAura);
}
...
void Unit::SendAttackStateUpdate(CalcDamageInfo *damageInfo)
{
    ...
    if (damageInfo->HitInfo & HITINFO_BLOCK) data
            << uint32(damageInfo->blocked_amount);

    if (damageInfo->HitInfo & HITINFO_UNK3) data << uint32(0);
        ...
        uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (hasteSpell->Id)
            {
                    ...
                    //case 33735:
                {
                    ...
                    if (!target || target == pVictim) return false;
                    basepoints0 = damage;
    ...
    if (cooldown && GetTypeId() == TYPEID_PLAYER
            && this->ToPlayer()->HasSpellCooldown(triggered_spell_id)) return false;
    ...
    if (cooldown && target->GetTypeId() == TYPEID_PLAYER) target->ToPlayer()->AddSpellCooldown(
            triggered_spell_id, 0, time(NULL) + cooldown);
        ...
        uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    // default case
    if (!target || (target != this && !target->isAlive())) return false;
    ...
    if (cooldown && GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->AddSpellCooldown(
            triggered_spell_id, 0, time(NULL) + cooldown);
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 24658:
                {
                    if (!procSpell || procSpell->Id == 24659) return false;
                ...
                case 28764:
                {
                    ...
                    if (!GetAuraEffect(SPELL_AURA_MOD_MANA_REGEN_INTERRUPT,
                            SPELLFAMILY_MAGE, 0x10000000, 0, 0)) return false;
                ...
                case 33493:
                {
                    ...
                    if (triggeredByAura->GetBase()->GetCharges() > 1) return false;
                ...
                case 39446:
                {
                    if (GetTypeId() != TYPEID_PLAYER || !this->isAlive()) return false;
                    ...
                    switch (getClass())
                    {
                        ...
                        case CLASS_DRUID: // 39511, 40997, 40998, 40999, 41002, 41005, 41009, 41011, 41409
                            triggered_spell_id = RAND(39511, 40997, 40998,
                                    40999, 41002, 41005, 41009, 41011, 41409);
                        ...
                        case CLASS_WARLOCK: // 40999, 41002, 41005, 41009, 41011, 41406, 41409
                            triggered_spell_id = RAND(40999, 41002, 41005,
                                    41009, 41011, 41406, 41409);
                            cooldown_spell_id = 40999;
                            break;
                        case CLASS_HUNTER: // 40997, 40999, 41002, 41005, 41009, 41011, 41406, 41409
                            triggered_spell_id = RAND(40997, 40999, 41002,
                                    41005, 41009, 41011, 41406, 41409);
                    ...
                    target = this;
                    if (roll_chance_i(10)) this->ToPlayer()->Say(
                            "This is Madness!", LANG_UNIVERSAL); // TODO: It should be moved to database, shouldn't it?
                    break;
                }
                ...
                case 45481:
                {
                    ...
                    if (ToPlayer()->GetReputationRank(934) == REP_EXALTED)
                    {
                        ...
                        if (IsFriendlyTo(target))
                        {
                            ...
                            if (!target)
                            {
                                uint64 selected_guid =
                                        ToPlayer()->GetSelection();
                                target = ObjectAccessor::GetUnit(*this,
                                        selected_guid);
                                if (!target) return false;
                            }
                ...
                {
                    Unit *owner = GetOwner();
                    if (!owner || owner->GetTypeId() != TYPEID_PLAYER) return false;
                    // Glyph of Shadowfiend (need cast as self cast for owner, no hidden cooldown)
                    owner->CastSpell(owner, 58227, true, castItem,
                            triggeredByAura);
                    return true;
                }
                    // Glyph of Life Tap
                ...
                case 71519: // Deathbringer's Will Normal
                {
                    ...
                    CastSpell(target, RandomSpells [rand_spell], true, castItem,
                            triggeredByAura, originalCaster);
                    for (std::vector <uint32>::iterator itr =
                            RandomSpells.begin(); itr != RandomSpells.end();
                            ++itr)
                    {
                        if (!ToPlayer()->HasSpellCooldown(*itr)) ToPlayer()->AddSpellCooldown(
                                *itr, 0, time(NULL) + cooldown);
                    }
                    break;
                }
                ...
                case 71903:
                {
                    ...
                    if (!dummy || dummy->GetStackAmount() < 10) return false;
                ...
                case 71169:
                {
                    ...
                    if (player->GetQuestStatus(24749)
                            == QUEST_STATUS_INCOMPLETE) // Unholy Infusion
                    {
                    ...
                    else if (player->GetQuestStatus(24756)
                            == QUEST_STATUS_INCOMPLETE) // Blood Infusion
                    {
                    ...
                    else if (player->GetQuestStatus(24757)
                            == QUEST_STATUS_INCOMPLETE) // Frost Infusion
                    {
                    ...
                    else if (player->GetQuestStatus(24547)
                            == QUEST_STATUS_INCOMPLETE) // A Feast of Souls
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            {
                if (getPowerType() != POWER_MANA) return false;
            ...
            if (dummySpell->SpellIconID == 2999)
            {
                ...
                if (procEx & PROC_EX_CRITICAL_HIT)
                {
                    ...
                    if (roll_chance_i(triggerAmount)) CastSpell(this, 48108,
            ...
            switch (dummySpell->Id)
            {
                ...
                case 56372:
                {
                    ...
                    {
                        SpellEntry const* cdSpell = sSpellStore.LookupEntry(
                                itr->first);
                        // Frost Nova
                        if (cdSpell
                                && cdSpell->SpellFamilyName == SPELLFAMILY_MAGE
                                && cdSpell->SpellFamilyFlags [0] & 0x00000040) this->ToPlayer()->RemoveSpellCooldown(
                                cdSpell->Id, true);
                    }
                    break;
                }
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (dummySpell->SpellFamilyFlags [1] & 0x8)
            {
                ...
                if (!HasInArc(M_PI, pVictim)) return false;
            ...
            if (dummySpell->SpellIconID == 1697)
            {
                ...
                if (procSpell == 0
                        || !(procEx
                                & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT))
                        || this == pVictim) return false;
                // Need stun or root mechanic
                if (!(GetAllSpellMechanicMask(procSpell)
                        & ((1 << MECHANIC_ROOT) | (1 << MECHANIC_STUN)))) return false;
            ...
            {
                if (!pVictim || !pVictim->isAlive() || !procSpell) return false;
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (dummySpell->SpellFamilyFlags [1] & 0x00000400)
            {
                ...
                if (triggeredByAura->GetCasterGUID() != pVictim->GetGUID()) return false;

                // Energize 0.25% of max. mana
                pVictim->CastSpell(pVictim, 57669, true, castItem,
                        triggeredByAura);
                return true; // no hidden cooldown
            }
            // Divine Aegis
            ...
            switch (dummySpell->Id)
            {
                ...
                {
                    // Shadow Word: Pain
                    if (procSpell->SpellFamilyFlags [0] & 0x8000) triggered_spell_id =
                            40441;
                    // Renew
                    else if (procSpell->SpellFamilyFlags [0] & 0x40) triggered_spell_id =
                            40440;
                    else return false;
                ...
                case 55677:
                {
                    ...
                    if (procSpell->SpellIconID != 74) return false;
                    if (!target || !target->IsFriendlyTo(this)) return false;

                    basepoints0 = int32(
                            target->CountPctFromMaxHealth(triggerAmount));
                    ...
                    if (!procSpell
                            || (GetSpellSchoolMask(procSpell)
                                    & (SPELL_SCHOOL_MASK_FROST
                                            | SPELL_SCHOOL_MASK_SHADOW)) == 0) return false;
                ...
                case 70770:
                {
                    ...
                    if (procSpell->SpellFamilyFlags [0] & 0x800)
                    {
                        ...
                        SpellEntry const* blessHealing =
                                sSpellStore.LookupEntry(triggered_spell_id);
                        if (!blessHealing) return false;
                        basepoints0 =
                                int32(
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 54815:
                {
                    ...
                    if (AuraEffect const *AurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, 0x00800000, 0x0, 0x0, GetGUID()))
                    {
                        ...
                        uint32 CountMax = GetSpellMaxDuration(
                                AurEff->GetSpellProto());
                ...
                case 24932:
                {
                    if (triggerAmount <= 0) return false;
                    basepoints0 = int32(CountPctFromMaxHealth(triggerAmount));
                    target = this;
                    triggered_spell_id = 34299;
                    if (triggeredByAura->GetCasterGUID() != GetGUID()) break;
                ...
                {
                    if (!pVictim
                            || !pVictim->HealthBelowPct(uint32(triggerAmount))) return false;
                    basepoints0 = int32(triggerAmount * damage / 100);
                ...
                case 40442:
                {
                    ...
                    if (!roll_chance_f(chance)) return false;
                ...
                case 70723:
                {
                            ...
                            && (procEx & PROC_EX_CRITICAL_HIT))
                    {
                        ...
                        SpellEntry const* triggeredSpell =
                                sSpellStore.LookupEntry(triggered_spell_id);
                        if (!triggeredSpell) return false;
                        basepoints0 =
                                int32(
                                        triggerAmount * damage / 100
                                                / (GetSpellMaxDuration(
                ...
                case 70664:
                {
                    ...
                    if (procSpell->SpellVisual [0] != 32) return false;

                    Player* caster = ToPlayer();
                    if (!caster) return false;
                    if (!caster->GetGroup() && pVictim == this) return false;

                    CastCustomSpell(70691, SPELLVALUE_BASE_POINT0, damage,
                            pVictim, true);
                    return true;
                }
            }
            ...
            else if (dummySpell->SpellIconID == 2850)
            {
                ...
                if (effIndex == 0)
                {
                    if (!(procSpell->SpellFamilyFlags [0] & 0x00080000)) return false;
                ...
                {
                    if (!(procSpell->SpellFamilyFlags [2] & 0x00000800)) return false;
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellIconID == 2236)
            {
                ...
                {
                    if (!pVictim) return false;
                    if (AuraEffect const* pEff = pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x0, 0x80000000, 0x0, GetGUID())) basepoints0 =
                            CalculatePowerCost(
                                    pEff->GetSpellProto(),
                                    this,
                                    SpellSchoolMask(
                                            pEff->GetSpellProto()->SchoolMask))
                                    * 4 / 10 / 3;
                }
                else basepoints0 = CalculatePowerCost(procSpell, this,
            ...
            if (dummySpell->SpellIconID == 3560)
            {
                ...
                if (!(procSpell->SpellFamilyFlags [1] & 0x01000000)) return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->Id == 53651)
            {
                ...
                if (Unit * beaconTarget = triggeredByAura->GetBase()->GetCaster())
                {
                    ...
                    if (beaconTarget == this) return false;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 86674:
                {
                    ...
                    if (!bp0 || !bp1) return false;

                    if (pVictim && pVictim->IsFriendlyTo(this)) CastCustomSpell(
                            pVictim, 86678, &bp0, &bp1, 0, true, NULL,
                            triggeredByAura, 0);
                    else CastCustomSpell(this, 86678, &bp0, &bp1, 0, true, NULL,
                            triggeredByAura, 0);
                    return true;
                }
                ...
                case 31801:
                {
                    ...
                    bool damager = procSpell
                            && procSpell->EquippedItemClass != -1;

                    if (!stacker && !damager) return false;
                    ...
                    if (Aura * aur = pVictim->GetAura(triggered_spell_id, GetGUID()))
                    {
                        ...
                        {
                            if (stacker) aur->RefreshDuration();
                            CastSpell(pVictim, 42463, true);
                            return true;
                        }
                    }

                    if (!stacker) return false;
                    break;
                }
                    // Seal of Corruption
                case 53736:
                ...
                case 33776:
                {
                    ...
                    // heal amount
                    basepoints0 = triggerAmount
                            * (std::min(damage, GetMaxHealth() - GetHealth()))
                            / 100;
                    target = this;

                    if (basepoints0) triggered_spell_id = 31786;
                    break;
                }
                ...
                case 40470:
                {
                    ...
                    // Judgement (any)
                    else if (GetSpellSpecific(procSpell)
                ...
                case 71406: // Tiny Abomination in a Jar
                {
                    ...
                    if (!dummy || dummy->GetStackAmount() < 8) return false;

                    RemoveAurasDueToSpell(71432);
                    triggered_spell_id = 71433; // default main hand attack
                    // roll if offhand
                    if (Player const* player = ToPlayer()) if (player->GetWeaponForAttack(
                            OFF_ATTACK, true) && urand(0, 1)) triggered_spell_id =
                            71434;
                    target = pVictim;
                    break;
                }
                case 71545: // Tiny Abomination in a Jar (Heroic)
                {
                    ...
                    Aura const* dummy = GetAura(71432);
                    if (!dummy || dummy->GetStackAmount() < 7) return false;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 51524:
                {
                    ...
                    caster->CastSpell(caster, 59566, true, castItem,
                            triggeredByAura, originalCaster);
                    return true;
                }
                    // Tidal Force
                ...
                case 70811:
                {
                    ...
                    if (procSpell->SpellFamilyFlags [0] & 0x3)
                    {
                        ...
                        {
                            uint32 newCooldownDelay =
                                    ToPlayer()->GetSpellCooldownDelay(16166);
                            if (newCooldownDelay < 3) newCooldownDelay = 0;
                            else newCooldownDelay -= 2;
                ...
                case 70817:
                {
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0x0, 0x0, GetGUID()))
                    {
                        ...
                        if (newDuration > maxDuration) flameShock->SetMaxDuration(
                                newDuration);
                ...
                {
                    if (procSpell->SpellIconID != 2019) return false;
                            ...
                            i != auras.end(); ++i)
                    {
                        ...
                        if (spell->SpellFamilyName == uint32(SPELLFAMILY_SHAMAN)
                                && spell->SpellFamilyFlags.HasFlag(0,
                                        0x02000000, 0))
                        {
                            if ((*i)->GetCasterGUID() != GetGUID()) continue;
                            if (spell->Id == 63283) continue;
                            aurEffA = (*i);
                            break;
                        }
                    }
                    if (aurEffA)
                    {
                        ...
                        if (AuraEffect * aurEffB = aurEffA->GetBase()->GetEffect(EFFECT_1)) bp1 =
                                aurEffB->GetAmount() * triggerAmount / 100;
                        CastCustomSpell(this, 63283, &bp0, &bp1, NULL, true,
                                NULL, triggeredByAura);
                        return true;
                    }
                    return false;
                }
            ...
            {
                if (this->GetDistance(target) < 15.0f) return false;
            ...
            if (dummySpell->SpellIconID == 3063)
            {
                ...
                if (procSpell->Id != 2484) return false;
            ...
            if (dummySpell->SpellFamilyFlags [1] & 0x00000400)
            {
                ...
                if (AuraEffect* aurEff = GetDummyAuraEffect(SPELLFAMILY_SHAMAN, 2015, 0))
                    basepoints0 += int32(basepoints0 * aurEff->GetAmount() / 100.0f);
            ...
            {
                if (!procSpell || GetTypeId() != TYPEID_PLAYER || !pVictim) return false;
                ...
                if (procSpell->SpellFamilyFlags [0] & 0x2)
                {
                    ...
                    float chance = 100.0f
                            / procSpell->EffectChainTarget [effIndex];
                    if (!roll_chance_f(chance)) return false;
                ...
                if (cooldown && GetTypeId() == TYPEID_PLAYER) ToPlayer()->AddSpellCooldown(
                        dummySpell->Id, 0, time(NULL) + cooldown);
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49194)
            {
                ...
                if (AuraEffect *glyph=GetAuraEffect(63332, 0)) AddPctN(
                        basepoints0, glyph->GetAmount());
            ...
            if (dummySpell->Id == 61257)
            {
                ...
                if (!(GetAllSpellMechanicMask(procSpell)
                        & ((1 << MECHANIC_ROOT) | (1 << MECHANIC_SNARE)))) return false;
            ...
            {
                if (!roll_chance_f(GetUnitCriticalChance(BASE_ATTACK, pVictim))) return false;
                basepoints0 = triggerAmount * damage / 100;
            ...
            if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPEID_PLAYER)
            {
                        ...
                        itr != sp_list.end(); ++itr)
                {
                    ...
                    if (!itr->second->active || itr->second->disabled
                            || itr->second->state == PLAYERSPELL_REMOVED) continue;
                            ...
                            && spellProto->SpellFamilyFlags [0] & 0x2000)
                    {
                        ...
                        if (chain && chain->last == itr->first) break;
                    }
                }
            }
            ...
            if (dummySpell->Id == 17619)
            {
                if (procSpell->SpellFamilyName == SPELLFAMILY_POTION)
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                    {
                        ...
                        basepoints0 = int32(
                                CalculateSpellDamage(this, procSpell, i)
                                        * 0.4f);
        ...
        case SPELLFAMILY_PET:
        {
            switch (dummySpell->SpellIconID)
            {
                ...
                case 201:
                {
                    ...
                    target = this;
                    float addThreat = SpellMgr::CalculateSpellEffectAmount(
                            procSpell, 0, this) * triggerAmount / 100.0f;
                ...
                case 1582:
                    triggered_spell_id =
                            dummySpell->Id == 62765 ? 62801 : 62800;
                    target = this;
                    break;
            }
            break;
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellFamilyFlags [1] & 0x40000)
            {
                ...
                basepoints0 = uint32(
                        maxmana * GetAttackTime(RANGED_ATTACK) / 1000.0f
                                / 100.0f);
                target = this;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 49222:
                {
                    *handled = true;
                    if (cooldown && GetTypeId() == TYPEID_PLAYER)
                    {
                        if (ToPlayer()->HasSpellCooldown(100000)) return false;
                        ToPlayer()->AddSpellCooldown(100000, 0,
                                time(NULL) + cooldown);
                    }
                    return true;
                }
                ...
                case 59906:
                {
                    int32 bp0 = CalculatePctN(
                            GetMaxHealth(),
                            SpellMgr::CalculateSpellEffectAmount(dummySpell,
                                    0));
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            else if (dummySpell->Id == 31834)
            {
                *handled = true;
                ...
                if (AuraEffect* aurEff = GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_PALADIN, 2141, 0)) removeChance =
                        aurEff->GetSpellProto()->procChance;
                if (!roll_chance_i(removeChance)) return true;
                break;
            }
            // Glyph of Divinity
                ...
                *handled = true;
                ...
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                {
                    if (procSpell->Effect [i] == SPELL_EFFECT_ENERGIZE)
                    {
                        ...
                        int32 mana = SpellMgr::CalculateSpellEffectAmount(
                                procSpell, i) * 2;
            ...
            switch (dummySpell->Id)
            {
                case 11129:
                {
                    *handled = true;
                    ...
                    if (HasAura(28682)) this->CastSpell(this, 28682, true);
                ...
                case 31658:
                {
                    *handled = true;
                    ...
                    int32 bp0 = this->GetCreateMana()
                            * SpellMgr::CalculateSpellEffectAmount(spInfo, 0)
                            / 100;
                    this->CastCustomSpell(this, 67545, &bp0, NULL, NULL, true,
                            NULL, triggeredByAura->GetEffect(0),
                            this->GetGUID());
                    return true;
                }
            }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
                    ...
                    || dummySpell->SpellIconID == 2622)
            {
                *handled = true;
                ...
                if (GetTypeId() == TYPEID_PLAYER)
                {
                    if (this->ToPlayer()->getClass() != CLASS_DEATH_KNIGHT) return false;
                    RuneType rune = this->ToPlayer()->GetLastUsedRune();
                    AuraEffect * aurEff = triggeredByAura->GetEffect(0);
                    if (!aurEff) return false;
                    // Reset amplitude - set death rune remove timer to 30s
                    aurEff->ResetPeriodic(true);
                    uint32 runesLeft;

                    if (dummySpell->SpellIconID == 2622) runesLeft = 2;
                    else runesLeft = 1;
                    ...
                    for (uint8 i = 0; i < MAX_RUNES && runesLeft; ++i)
                    {
                        ...
                        {
                            if (((Player*) this)->GetCurrentRune(i)
                                    == RUNE_DEATH
                        ...
                        if (((Player*) this)->GetRuneCooldown(i)
                                != ((Player*) this)->GetRuneBaseCooldown(i)) continue;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 51209:
                    *handled = true;
                    ...
                    if (procSpell && procSpell->Dispel == DISPEL_DISEASE) return false;
                    return true;
            }
            break;
        }
        ...
        uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    if (sSpellStore.LookupEntry(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 40336:
                    {
                        ...
                        if (this && this->isAlive()) CastSpell(this, 29471,
                                true, castItem, triggeredByAura);
                        if (pVictim && pVictim->isAlive()) CastSpell(pVictim,
                                27526, true, castItem, triggeredByAura);
                        return true;
                    }
                }
                break;
            ...
            {
                // Greater Heal Refund
                if (auraSpellInfo->Id == 37594) trigger_spell_id = 37595;
                // Blessed Recovery
                else if (auraSpellInfo->SpellIconID == 1875)
                {
                    ...
                    basepoints0 = CalculatePctN(int32(damage), triggerAmount)
                            / 3;
                    target = this;
                    if (AuraEffect * aurEff = target->GetAuraEffect(trigger_spell_id, 0)) basepoints0 +=
                            aurEff->GetAmount();
                }
                break;
            }
            ...
            case SPELLFAMILY_HUNTER:
            {
                ...
                {
                    if (!(auraSpellInfo->procFlags == 0x1140)) return false;
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    default:
                    {
                        ...
                        else if (auraSpellInfo->SpellIconID == 2013)
                        {
                            ...
                            if (!HealthBelowPctDamaged(30, damage)) return false;

                            if (pVictim && pVictim->isAlive()) pVictim->getThreatManager().modifyThreatPercent(
                                    this, -10);

                            basepoints0 = int32(
                                    CountPctFromMaxHealth(triggerAmount));
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 28845:
        {
            ...
            if (HealthBelowPctDamaged(20, damage) || HealthBelowPct(20)) return false;
            break;
        }
        ...
        case 31255:
        {
            ...
            if (!pVictim || !pVictim->isAlive() || pVictim->HealthAbovePct(20)) return false;
        ...
        {
            // Shadow Word: Pain
            if (!(procSpell->SpellFamilyFlags [0] & 0x8000)) return false;
            break;
        }
        ...
        case 37594:
        {
            ...
            if (pVictim->IsFullHealth()) return false;
            // If your Greater Heal brings the target to full health, you gain $37595s1 mana.
            if (pVictim->GetHealth() + damage < pVictim->GetMaxHealth()) return false;
            break;
        }
        ...
        case 40971:
        {
            ...
            if (!pVictim || !pVictim->isAlive()
                    || pVictim->HealthAbovePct(triggerAmount)) return false;
            break;
        }
        ...
        case 45057:
        {
            ...
            if (GetHealth() - damage > GetMaxHealth() * triggerAmount / 100) return false;
            break;
        }
            // Rapid Recuperation
        case 53228:
        ...
        {
            if (procSpell->SpellFamilyFlags [0] & 0x800000) if ((procSpell->Id
                    != 58381) || !roll_chance_i(50)) return false;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 71639: // Thick Skin
            if (HealthBelowPct(34) || (!HealthBelowPctDamaged(35, damage))) return false;
            else
            {
                if (!ToPlayer()->HasSpellCooldown(trigger_spell_id))
                {
        ...
        {
            if (!pVictim || pVictim == this) return false;
        ...
        {
            basepoints0 = int32(
                    GetTotalAttackPowerValue(BASE_ATTACK) * triggerAmount
                            / 100);
            break;
        }
        ...
        case 35095:
        {
            if (!procSpell || procSpell->powerType != POWER_MANA
                    || (procSpell->manaCost == 0
                            ...
                            && procSpell->manaCostPerlevel == 0)) return false;
            break;
        }
            // Demonic Pact
        case 48090:
        {
            ...
            if (isPet()) if (Unit * owner = GetOwner())
            {
        ...
        case 54748:
        {
            ...
            if (!(GetAllSpellMechanicMask(procSpell)
                    & ((1 << MECHANIC_INTERRUPT) | (1 << MECHANIC_SILENCE)))) return false;
            break;
        }
            // Lock and Load
        case 56453:
        ...
        case 70893:
        {
            ...
            if (!(procSpell->SpellFamilyFlags [1] & 0x10000000)
                    && (procEx != PROC_EX_CRITICAL_HIT)) return false;
            // check if we're procced by Claw, Bite or Smack (need to use the spell icon ID to detect it)
            if (!(procSpell->SpellIconID == 262
                    || procSpell->SpellIconID == 1680
                    || procSpell->SpellIconID == 473)) return false;
            break;
        }
        ...
        case 71169:
        {
            ...
            if (pVictim->GetTypeId() != TYPEID_UNIT) return false;
            // critters are not allowed
            if (pVictim->GetCreatureType() == CREATURE_TYPE_CRITTER) return false;
            break;
        }
        ...
        uint32 cooldown)
{
    ...
    switch (scriptId)
    {
        ...
        {
            if (!procSpell || procSpell->SpellVisual [0] != 9487) return false;
        ...
        {
            // Chance 50%
            if (!roll_chance_i(50)) return false;
        ...
        case 7012:
        {
            if (!roll_chance_i(triggeredByAura->GetAmount())) return false;
            switch (pVictim->getPowerType())
            {
                case POWER_MANA:
    ...
    // not processed
    if (!triggered_spell_id) return false;
...
void Unit::setPowerType(Powers new_powertype)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        if (this->ToPlayer()->GetGroup()) this->ToPlayer()->SetGroupUpdateFlag(
    ...
    else if (this->ToCreature()->isPet())
    {
        ...
        if (pet->isControlled())
        {
            ...
            if (owner && (owner->GetTypeId() == TYPEID_PLAYER)
                    && owner->ToPlayer()->GetGroup()) owner->ToPlayer()->SetGroupUpdateFlag(
...
ReputationRank Unit::GetReactionTo(Unit const* target) const
{
    ...
    if (GetCharmerOrOwnerOrSelf() == target->GetCharmerOrOwnerOrSelf()) return REP_FRIENDLY;
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
    {
        if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
        {
            ...
            if (selfPlayerOwner && targetPlayerOwner)
            {
                ...
                if (selfPlayerOwner == targetPlayerOwner) return REP_FRIENDLY;

                // duel - always hostile to opponent
                if (selfPlayerOwner->duel
                        && selfPlayerOwner->duel->opponent == targetPlayerOwner
                        && selfPlayerOwner->duel->startTime != 0) return REP_HOSTILE;

                // same group - checks dependant only on our faction - skip FFA_PVP for example
                if (selfPlayerOwner->IsInRaidWith(targetPlayerOwner)) return REP_FRIENDLY; // return true to allow config option AllowTwoSide.Interaction.Group to work
            ...
            if (GetByteValue(UNIT_FIELD_BYTES_2, 1) & UNIT_BYTE2_FLAG_FFA_PVP
                    && target->GetByteValue(UNIT_FIELD_BYTES_2, 1)
                            & UNIT_BYTE2_FLAG_FFA_PVP) return REP_HOSTILE;

            if (selfPlayerOwner)
            {
                if (FactionTemplateEntry const* targetFactionTemplateEntry = target->getFactionTemplateEntry())
                {
                    if (ReputationRank const* repRank = selfPlayerOwner->GetReputationMgr().GetForcedRankIfAny(targetFactionTemplateEntry)) return *repRank;
                    if (!selfPlayerOwner->HasFlag(UNIT_FIELD_FLAGS_2,
                        ...
                        if (FactionEntry const* targetFactionEntry = sFactionStore.LookupEntry(targetFactionTemplateEntry->faction))
                        {
                            if (targetFactionEntry->CanHaveReputation())
                            {
                                ...
                                if (targetFactionTemplateEntry->factionFlags
                                        ...
                                        && selfPlayerOwner->HasFlag(
                                                PLAYER_FLAGS,
                                                PLAYER_FLAGS_CONTESTED_PVP)) return REP_HOSTILE;
                                ...
                                if (selfPlayerOwner->GetReputationMgr().IsAtWar(
                                        targetFactionEntry)) return REP_HOSTILE;
                                return REP_FRIENDLY;
                            }
                        }
                    }
...
ReputationRank Unit::GetFactionReactionTo(
        FactionTemplateEntry const* factionTemplateEntry, Unit const* target)
{
    // always neutral when no template entry found
    if (!factionTemplateEntry) return REP_NEUTRAL;

    FactionTemplateEntry const* targetFactionTemplateEntry =
            target->getFactionTemplateEntry();
    if (!targetFactionTemplateEntry) return REP_NEUTRAL;
    ...
    if (Player const* targetPlayerOwner = target->GetAffectingPlayer())
    {
        ...
        if (factionTemplateEntry->factionFlags
                & FACTION_TEMPLATE_FLAG_CONTESTED_GUARD
                && targetPlayerOwner->HasFlag(PLAYER_FLAGS,
                        PLAYER_FLAGS_CONTESTED_PVP)) return REP_HOSTILE;
        if (ReputationRank const* repRank = targetPlayerOwner->GetReputationMgr().GetForcedRankIfAny(factionTemplateEntry)) return *repRank;
            ...
            if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(factionTemplateEntry->faction))
            {
                if (factionEntry->CanHaveReputation())
                {
                    ...
                    if (targetPlayerOwner->GetReputationMgr().IsAtWar(
                            factionEntry)) repRank = std::min(REP_NEUTRAL,
                            repRank);
                    return repRank;
                }
            }
    ...
    if (factionTemplateEntry->IsHostileTo(*targetFactionTemplateEntry)) return REP_HOSTILE;
    if (factionTemplateEntry->IsFriendlyTo(*targetFactionTemplateEntry)) return REP_FRIENDLY;
    if (targetFactionTemplateEntry->IsFriendlyTo(*factionTemplateEntry)) return REP_FRIENDLY;
    if (factionTemplateEntry->factionFlags
            & FACTION_TEMPLATE_FLAG_HOSTILE_BY_DEFAULT) return REP_HOSTILE;
    // neutral by default
    return REP_NEUTRAL;
}
...
bool Unit::IsHostileTo(Unit const* unit) const
{
    ...
    if (unit->GetTypeId() == TYPEID_PLAYER
            && ((Player const*) unit)->isGameMaster()) return false;

    // always hostile to enemy
    if (getVictim() == unit || unit->getVictim() == this) return true;
    ...
    if (testerOwner
            && (testerOwner->getVictim() == unit
                    || unit->getVictim() == testerOwner)) return true;

    // always hostile to enemy owner
    if (targetOwner
            && (getVictim() == targetOwner || targetOwner->getVictim() == this)) return true;

    // always hostile to owner of owner's enemy
    if (testerOwner && targetOwner
            && (testerOwner->getVictim() == targetOwner
                    || targetOwner->getVictim() == testerOwner)) return true;

    Unit const* tester = testerOwner ? testerOwner : this;
    Unit const* target = targetOwner ? targetOwner : unit;

    // always non-hostile to target with common owner, or to owner/pet
    if (tester == target) return false;
            ...
            && target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            if (pTester->GetTeam() == pTarget->GetTeam()) return false;
    ...
    FactionTemplateEntry const*target_faction =
            target->getFactionTemplateEntry();
    if (!tester_faction || !target_faction) return false;

    if (target->isAttackingPlayer() && tester->IsContestedGuard()) return true;
            ...
            && !tester->HasAuraType(SPELL_AURA_MOD_FACTION))
    {
        ...
        {
            if (ReputationRank const* force =tester->ToPlayer()->GetReputationMgr().GetForcedRankIfAny(target_faction)) return *force
                    <= REP_HOSTILE;

            // if faction have reputation then hostile state for tester at 100% dependent from at_war state
            if (FactionEntry const* raw_target_faction = sFactionStore.LookupEntry(target_faction->faction)) if (FactionState const* factionState = tester->ToPlayer()->GetReputationMgr().GetState(raw_target_faction)) return (factionState->Flags
                    & FACTION_FLAG_AT_WAR);
        }
    }
            ...
            && !target->HasAuraType(SPELL_AURA_MOD_FACTION))
    {
        ...
        {
            if (ReputationRank const* force = target->ToPlayer()->GetReputationMgr().GetForcedRankIfAny(tester_faction)) return *force
                    <= REP_HOSTILE;

            // apply reputation state
            FactionEntry const* raw_tester_faction = sFactionStore.LookupEntry(
                    tester_faction->faction);
            if (raw_tester_faction && raw_tester_faction->reputationListID >= 0) return ((Player const*) target)->GetReputationMgr().GetRank(
...
bool Unit::IsFriendlyTo(Unit const* unit) const
{
    ...
    if (unit->GetTypeId() == TYPEID_PLAYER
            && ((Player const*) unit)->isGameMaster()) return true;

    // always non-friendly to enemy
    if (getVictim() == unit || unit->getVictim() == this) return false;
    ...
    if (testerOwner
            && (testerOwner->getVictim() == unit
                    || unit->getVictim() == testerOwner)) return false;

    // always non-friendly to enemy owner
    if (targetOwner
            && (getVictim() == targetOwner || targetOwner->getVictim() == this)) return false;

    // always non-friendly to owner of owner's enemy
    if (testerOwner && targetOwner
            && (testerOwner->getVictim() == targetOwner
                    || targetOwner->getVictim() == testerOwner)) return false;

    Unit const* tester = testerOwner ? testerOwner : this;
    Unit const* target = targetOwner ? targetOwner : unit;

    // always friendly to target with common owner, or to owner/pet
    if (tester == target) return true;
            ...
            && target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (pTester->duel && pTester->duel->opponent == target
                && pTester->duel->startTime != 0) return false;

        // Group
        if (pTester->GetGroup() && pTester->GetGroup() == pTarget->GetGroup()) return true;

        // Sanctuary
        if (pTarget->HasByteFlag(UNIT_FIELD_BYTES_2, 1,
                UNIT_BYTE2_FLAG_SANCTUARY)
                && pTester->HasByteFlag(UNIT_FIELD_BYTES_2, 1,
                        UNIT_BYTE2_FLAG_SANCTUARY)) return true;

        // PvP FFA state
        if (pTester->HasByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP)
                && pTarget->HasByteFlag(UNIT_FIELD_BYTES_2, 1,
                        UNIT_BYTE2_FLAG_FFA_PVP)) return false;
        ...
        if (!pTester->HasAuraType(SPELL_AURA_MOD_FACTION)
                && !pTarget->HasAuraType(SPELL_AURA_MOD_FACTION))
        {
            if (pTester->GetTeam() == pTarget->GetTeam()) return true;
    ...
    if (target->isAttackingPlayer() && tester->IsContestedGuard()) return false;
            ...
            && !target->HasAuraType(SPELL_AURA_MOD_FACTION))
    {
        ...
        if (tester_faction->faction)
        {
            ...
            // apply reputation state
            if (FactionEntry const *raw_tester_faction = sFactionStore.LookupEntry(tester_faction->faction)) if (raw_tester_faction->reputationListID
                    >= 0) return ((Player const*) target)->GetReputationMgr().GetRank(
...
bool Unit::IsHostileToPlayers() const
{
    ...
    if (!my_faction || !my_faction->faction) return false;

    FactionEntry const *raw_faction = sFactionStore.LookupEntry(
            my_faction->faction);
    if (raw_faction && raw_faction->reputationListID >= 0) return false;
...
{
    FactionTemplateEntry const *my_faction = getFactionTemplateEntry();
    if (!my_faction || !my_faction->faction) return true;
...
bool Unit::Attack(Unit *victim, bool meleeAttack)
{
    ...
    if (!isAlive() || !victim->IsInWorld() || !victim->isAlive()) return false;

    // player cannot attack in mount state
    if (GetTypeId() == TYPEID_PLAYER && IsMounted()) return false;

    // nobody can attack GM in GM-mode
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (victim->ToPlayer()->isGameMaster()) return false;
    }
    else
    {
        //!creature -> WHO ARE U??
        if (!victim->ToCreature() || victim->ToCreature()->IsInEvadeMode()) return false;
    }

    // remove SPELL_AURA_MOD_UNATTACKABLE at attack (in case non-interruptible spells stun aura applied also that not let attack)
    if (HasAuraType(SPELL_AURA_MOD_UNATTACKABLE)) RemoveAurasByType(
    ...
    if (m_attacking)
    {
        ...
        if (!meleeAttack) ClearUnitState(UNIT_STAT_MELEE_ATTACKING);
    }

    if (m_attacking) m_attacking->_removeAttacker(this);
    ...
    if (meleeAttack) AddUnitState(UNIT_STAT_MELEE_ATTACKING);
    ...
    if (GetTypeId() == TYPEID_UNIT && !this->ToCreature()->isPet())
    {
        ...
        if (victim->GetTypeId() == TYPEID_PLAYER) victim->SetInCombatWith(this);
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetEmoteState() != 0) ToPlayer()->SetEmoteState(
            0);

    // delay offhand weapon attack to next attack time
    if (haveOffhandWeapon()) resetAttackTimer(OFF_ATTACK);

    if (meleeAttack) SendMeleeAttackStart(victim);
...
{
    if (includingCast && IsNonMeleeSpellCasted(false)) InterruptNonMeleeSpells(
            false);

    AttackStop();
    RemoveAllAttackers();
    if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->SendAttackSwingCancelAttack(); // melee and ranged forced attack cancel
    ClearInCombat();
}
...
{
    if (HasUnitState(UNIT_STAT_ATTACK_PLAYER)) return true;

    for (ControlList::const_iterator itr = m_Controlled.begin();
            itr != m_Controlled.end(); ++itr)
        if ((*itr)->isAttackingPlayer()) return true;

    for (uint8 i = 0; i < MAX_SUMMON_SLOT; ++i)
        if (m_SummonSlot [i]) if (Creature *summon = GetMap()->GetCreature(m_SummonSlot[i])) if (summon->isAttackingPlayer()) return true;
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                        ...
                        itr != sp_list.end(); ++itr)
                {
                    if (itr->second->state == PLAYERSPELL_REMOVED
                            || itr->second->disabled) continue;
                    SpellEntry const *spellInfo = sSpellStore.LookupEntry(
                            itr->first);
                    if (!spellInfo || !IsPassiveSpell(itr->first)) continue;
                    if (spellInfo->CasterAuraState == uint32(flag)) CastSpell(
                            this, itr->first, true, NULL);
                }
            }
            else if (this->ToCreature()->isPet())
            {
        ...
        if (HasFlag(UNIT_FIELD_AURASTATE, 1 << (flag - 1)))
        {
            ...
            if (flag != AURA_STATE_ENRAGE) // enrage aura state triggering continues auras
            {
                        ...
                        itr != tAuras.end();)
                {
                    ...
                    if (spellProto->CasterAuraState == uint32(flag)) RemoveAura(
                            itr);
                    else ++itr;
                }
            }
...
uint32 Unit::BuildAuraStateUpdateForTarget(Unit * target) const
{
    ...
    for (AuraStateAurasMap::const_iterator itr = m_auraStateAuras.begin();
        ...
        if ((1 << (itr->first - 1)) & PER_CASTER_AURA_STATE_MASK) if (itr->second->GetBase()->GetCasterGUID()
                == target->GetGUID()) auraStates |= (1 << (itr->first - 1));
    ...
    if (Caster)
    {
        if (spellProto)
        {
            ...
            for (AuraEffectList::const_iterator j = stateAuras.begin();
                    j != stateAuras.end(); ++j)
                if ((*j)->IsAffectedOnSpell(spellProto)) return true;
        }
...
{
    if (uint64 charmerid = GetCharmerGUID()) return ObjectAccessor::GetUnit(
            *this, charmerid);
    return NULL;
}
...
Player* Unit::GetCharmerOrOwnerPlayerOrPlayerItself() const
{
    ...
    if (IS_PLAYER_GUID(guid)) return ObjectAccessor::GetPlayer(*this, guid);
...
{
    if (!GetCharmerOrOwnerGUID()) return
            GetTypeId() == TYPEID_PLAYER ? (Player*) this : NULL;

    if (Unit* owner = GetCharmerOrOwner()) return owner->GetCharmerOrOwnerPlayerOrPlayerItself();
    return NULL;
}
...
Minion *Unit::GetFirstMinion() const
{
    ...
    {
        if (Creature* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*this, pet_guid)) if (pet->HasUnitTypeMask(
                UNIT_MASK_MINION)) return (Minion*) pet;
...
Guardian* Unit::GetGuardianPet() const
{
    ...
    {
        if (Creature* pet = ObjectAccessor::GetCreatureOrPetOrVehicle(*this, pet_guid)) if (pet->HasUnitTypeMask(
                UNIT_MASK_GUARDIAN)) return (Guardian*) pet;
...
Unit* Unit::GetCharm() const
{
    ...
    {
        if (Unit* pet = ObjectAccessor::GetUnit(*this, charm_guid)) return pet;
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (spellInfo
                    && (spellInfo->Attributes
                            & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)) this->ToPlayer()->AddSpellAndCategoryCooldowns(
                    spellInfo, 0, NULL, true);
        }
    }
    else
    {
        ...
        {
            if (GetCritterGUID() == minion->GetGUID()) SetCritterGUID(0);
        }

        if (minion->IsGuardianPet())
        {
            if (GetPetGUID() == minion->GetGUID()) SetPetGUID(0);
        }
        else if (minion->isTotem())
        {
            // All summoned by totem minions must disappear when it is removed.
            if (const SpellEntry* spInfo = sSpellStore.LookupEntry(minion->ToTotem()->GetSpell())) for (int i =
                    0; i < MAX_SPELL_EFFECTS; ++i)
            {
                if (spInfo->Effect [i] != SPELL_EFFECT_SUMMON) continue;
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (spellInfo
                    && (spellInfo->Attributes
                            & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)) this->ToPlayer()->SendCooldownEvent(
                    spellInfo);
        }
        ...
        //if (minion->HasUnitTypeMask(UNIT_MASK_GUARDIAN))
        {
            if (RemoveUInt64Value(UNIT_FIELD_SUMMON, minion->GetGUID()))
            {
                        ...
                        itr != m_Controlled.end(); ++itr)
                {
                    ...
                    if (GetGUID() == (*itr)->GetCharmerGUID()) continue;
                    ...
                    if (!(*itr)->HasUnitTypeMask(
                            UNIT_MASK_CONTROLABLE_GUARDIAN)) continue;
                    ...
                    if (AddUInt64Value(UNIT_FIELD_SUMMON, (*itr)->GetGUID()))
                    {
                        ...
                        {
                            if ((*itr)->isPet()) this->ToPlayer()->PetSpellInitialize();
                            else this->ToPlayer()->CharmSpellInitialize();
                        }
                    }
                    break;
                }
...
void Unit::SetCharm(Unit* charm, bool apply)
{
    if (apply)
    {
        ...
        else charm->m_ControlledByPlayer = false;
    ...
    else
    {
        ...
        if (charm->GetTypeId() == TYPEID_PLAYER
                ...
                || charm->GetOwnerGUID() != GetGUID()) m_Controlled.erase(
                charm);
    }
}
...
int32 Unit::DealHeal(Unit *pVictim, uint32 addhealth)
{
    ...
    if (pVictim->IsAIEnabled) pVictim->GetAI()->HealReceived(this, addhealth);

    if (IsAIEnabled) GetAI()->HealDone(pVictim, addhealth);

    if (addhealth) gain = pVictim->ModifyHealth(int32(addhealth));

    Unit* unit = this;

    m_heal_done [0] += addhealth;

    if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->isTotem()) unit =
            GetOwner();

    if (unit->GetTypeId() == TYPEID_PLAYER)
    {
        if (Battleground *bg = unit->ToPlayer()->GetBattleground()) bg->UpdatePlayerScore(
                (Player*) unit, SCORE_HEALING_DONE, gain);

        // use the actual gain, as the overheal shall not be counted, skip gain 0 (it ignored anyway in to criteria)
        if (gain) unit->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(
...
Unit* Unit::SelectMagnetTarget(Unit *victim, SpellEntry const *spellInfo)
{
                    ...
                    || spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC))
    {
        ...
        if (spellInfo->DmgClass == SPELL_DAMAGE_CLASS_NONE) return victim;
        ...
        for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin();
            ...
            if (Unit* magnet = (*itr)->GetBase()->GetUnitOwner()) if (magnet->isAlive()) if (Spell* spell = FindCurrentSpellBySpellId(spellInfo->Id))
            {
    ...
    else
    {
        ...
        for (AuraEffectList::const_iterator i = hitTriggerAuras.begin();
            ...
            if (Unit* magnet = (*i)->GetBase()->GetCaster()) if (magnet->isAlive()
                    && magnet->IsWithinLOSInMap(this)) if (roll_chance_i(
                    (*i)->GetAmount()))
            {
...
Unit* Unit::GetFirstControlled() const
{
    ...
    Unit *unit = GetCharm();
    if (!unit) if (uint64 guid = GetUInt64Value(UNIT_FIELD_SUMMON)) unit =
            ObjectAccessor::GetUnit(*this, guid);
...
void Unit::RemoveAllControlled()
{
    ...
    if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->StopCastingCharm();

    while (!m_Controlled.empty())
    {
        Unit *target = *m_Controlled.begin();
        m_Controlled.erase(m_Controlled.begin());
        if (target->GetCharmerGUID() == GetGUID()) target->RemoveCharmAuras();
        else if (target->GetOwnerGUID() == GetGUID() && target->isSummon()) target->ToTempSummon()->UnSummon();
        else
        sLog->outError(
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    else if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->isPet()) player =
            (Player*) GetOwner();
    ...
    if (!pGroup)
    {
        ...
        if (player != this) return
                IsWithinDistInMap(player, radius) ? player : NULL;
    ...
    for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL;
        ...
        if (Player *Target = itr->getSource())
        {
            ...
            if (Target != this && Target->isAlive()
                    ...
                    && !IsHostileTo(Target)) nearMembers.push_back(Target);

            // Push player's pet to vector
            if (Unit *pet = Target->GetGuardianPet()) if (pet != this
                    && pet->isAlive() && IsWithinDistInMap(pet, radius)
                    && !IsHostileTo(pet)) nearMembers.push_back(pet);
        }

    if (nearMembers.empty()) return NULL;
...
void Unit::UnsummonAllTotems()
{
    for (uint8 i = 0; i < MAX_SUMMON_SLOT; ++i)
    {
        if (!m_SummonSlot [i]) continue;

        if (Creature *OldTotem = GetMap()->GetCreature(m_SummonSlot[i])) if (OldTotem->isSummon()) OldTotem->ToTempSummon()->UnSummon();
    }
}
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellEntry const *spellProto,
        ...
        uint32 stack)
{
    if (!spellProto || !pVictim || damagetype == DIRECT_DAMAGE) return pdamage;

    // For totems get damage bonus from owner
    if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->isTotem()) if (Unit *owner = GetOwner()) return owner->SpellDamageBonus(
            pVictim, spellProto, effIndex, pdamage, damagetype);
    ...
    // ..done
    // Pet damage
    if (GetTypeId() == TYPEID_UNIT && !this->ToCreature()->isPet()) DoneTotalMod *=
            this->ToCreature()->GetSpellDamageMod(
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneVersus.begin();
        ...
        if (creatureTypeMask & uint32((*i)->GetMiscValue())) DoneTotalMod *=
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneVersusAurastate.begin();
        ...
        if (pVictim->HasAuraState(AuraState((*i)->GetMiscValue()))) DoneTotalMod *=
                ((*i)->GetAmount() + 100.0f) / 100.0f;
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 6928:
            {
                if (pVictim->HasAuraState(AURA_STATE_HEALTHLESS_35_PERCENT,
                        spellProto, this)) DoneTotalMod *= (100.0f
                        + (*i)->GetAmount()) / 100.0f;
                break;
            }
                // Soul Siphon
            ...
            case 6927:
                if (HasAuraState(AURA_STATE_HEALTHLESS_20_PERCENT, spellProto,
                        this)) DoneTotalMod *= (100.0f + (*i)->GetAmount())
                        / 100.0f;
                break;
            case 5481: // Starfire Bonus
            {
                if (pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                        SPELLFAMILY_DRUID, 0x200002, 0, 0)) DoneTotalMod *=
                        ((*i)->GetAmount() + 100.0f) / 100.0f;
                break;
            }
            ...
            case 7277:
            {
                ...
                {
                    if (!pVictim->HealthAbovePct(35)) DoneTotalMod *= (100.0f
                            + (*i)->GetAmount()) / 100.0f;
                }
                // Tundra Stalker
                else
                {
                    ...
                    if (pVictim->HasAura(55095)) DoneTotalMod *=
                            ((*i)->GetAmount() + 100.0f) / 100.0f;
                    break;
                }
                break;
            ...
            case 7293:
            {
                ...
                {
                    if (SpellChainNode const *chain = sSpellMgr->GetSpellChainNode((*i)->GetId())) DoneTotalMod *=
                            (chain->rank * 2.0f + 100.0f) / 100.0f;
                }
                break;
            }
                // Twisted Faith
            ...
            {
                if (pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                        SPELLFAMILY_PRIEST, 0x8000, 0, 0, GetGUID())) DoneTotalMod *=
                        ((*i)->GetAmount() + 100.0f) / 100.0f;
                break;
            }
        }
    ...
    switch (spellProto->SpellFamilyName)
    {
            ...
            if (spellProto->SpellIconID == 186)
            {
                if (pVictim->HasAuraState(AURA_STATE_FROZEN, spellProto, this))
                {
                    ...
                    if (owner->HasAura(56377)
                            && pVictim->getLevel() > owner->getLevel()) DoneTotalMod *=
                            1.05f;
        ...
        case SPELLFAMILY_PRIEST:
            ...
            {
                // Glyph of Smite
                if (AuraEffect * aurEff = GetAuraEffect(55692, 0)) if (pVictim->GetAuraEffect(
                        SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST,
                        0x100000, 0, 0, GetGUID())) AddPctN(DoneTotalMod,
                        aurEff->GetAmount());
            }
            break;
        case SPELLFAMILY_PALADIN:
                    ...
                    && spellProto->SpellIconID == 2292)
            {
                ...
                if (stacks) DoneTotalMod *= (10.0f + float(stacks)) / 10.0f;
            }
            break;
        case SPELLFAMILY_WARLOCK:
            //Fire and Brimstone
            if (spellProto->SpellFamilyFlags [1] & 0x00020040) if (pVictim->HasAuraState(
            ...
            if (spellProto->SpellFamilyFlags [0] & 0x00004000) if (HasAura(
                    200000)) DoneTotalMod *= 2;
            // Shadow Bite (15% increase from each dot)
            if (spellProto->SpellFamilyFlags [1] & 0x00400000 && isPet()
                    && GetOwner()) if (uint8 count = pVictim->GetDoTsByCaster(GetOwnerGUID())) AddPctN(
                    DoneTotalMod, 15 * count);
            break;
        case SPELLFAMILY_HUNTER:
            // Steady Shot
            if (spellProto->SpellFamilyFlags [1] & 0x1) if (AuraEffect * aurEff = GetAuraEffect(56826, 0)) // Glyph of Steady Shot
            if (pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                    SPELLFAMILY_HUNTER, 0x00004000, 0, 0, GetGUID())) AddPctN(
                    DoneTotalMod, aurEff->GetAmount());
            break;
        case SPELLFAMILY_DEATHKNIGHT:
            // Improved Icy Touch
            if (spellProto->SpellFamilyFlags [0] & 0x2) if (AuraEffect * aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 2721, 0)) DoneTotalMod *=
                    (100.0f + aurEff->GetAmount()) / 100.0f;

            // Sigil of the Vengeful Heart
            if (spellProto->SpellFamilyFlags [0] & 0x2000) if (AuraEffect* aurEff = GetAuraEffect(64962, EFFECT_1)) AddPctN(
                    DoneTotal, aurEff->GetAmount());

            // Glacier Rot
            if (spellProto->SpellFamilyFlags [0] & 0x2
                    || spellProto->SpellFamilyFlags [1] & 0x6) if (AuraEffect * aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 196, 0)) if (pVictim->GetDiseasesByCaster(
                    owner->GetGUID()) > 0) DoneTotalMod *= (100.0f
                    + aurEff->GetAmount()) / 100.0f;
            ...
            if (spellProto->SpellFamilyFlags [1] & 0x20000000)
            {
                ...
                if (GetTypeId() == TYPEID_PLAYER)
                {
                    PlayerSpellMap playerSpells =
                            this->ToPlayer()->GetSpellMap();
                    for (PlayerSpellMap::const_iterator itr =
                            playerSpells.begin(); itr != playerSpells.end();
                            ++itr)
                    {
                        if (itr->second->state == PLAYERSPELL_REMOVED
                                || itr->second->disabled) continue;
                        switch (itr->first)
                        {
                            ...
                            case 49638:
                            {
                                if (const SpellEntry *proto=sSpellStore.LookupEntry(itr->first)) AddPctN(
                                        ApCoeffMod,
                                        SpellMgr::CalculateSpellEffectAmount(
                                                proto, 0));
                            }
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                        ...
                        itr != playerSpells.end(); ++itr)
                {
                    ...
                    switch (itr->first)
                    {
                        ...
                        case 49638:
                        {
                            if (const SpellEntry *proto=sSpellStore.LookupEntry(itr->first)) ApCoeffMod *=
                                    (100.0f
                                            + SpellMgr::CalculateSpellEffectAmount(
                                                    proto, 0)) / 100.0f;
                        }
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentTaken.begin();
        ...
        if ((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
        {
            if ((*i)->GetAmount() > 0)
            {
                if ((*i)->GetAmount() > maxPositiveMod) maxPositiveMod =
                        (*i)->GetAmount();
            }
            else sumNegativeMod += (*i)->GetAmount();
        }
    ...
    for (AuraEffectList::const_iterator i = mOwnerTaken.begin();
        ...
        if ((*i)->GetCasterGUID() == GetGUID()
                && (*i)->IsAffectedOnSpell(spellProto)) sumNegativeMod +=
                (*i)->GetAmount();
    ...
    if (uint32 mechanicMask = GetAllSpellMechanicMask(spellProto))
    {
        ...
        for (AuraEffectList::const_iterator i = mDamageDoneMechanic.begin();
            ...
            if (mechanicMask & uint32(1 << ((*i)->GetMiscValue()))) sumNegativeMod +=
                    (*i)->GetAmount();
    }
    ...
    if (HasUnitTypeMask(UNIT_MASK_GUARDIAN)) if (!(GetSpellSchoolMask(
            spellProto) & SPELL_SCHOOL_MASK_NORMAL)) // Do not add Spellpower to melee abilities
    ...
    if (DoneAdvertisedBenefit || TakenAdvertisedBenefit)
    {
        ...
        if (Player* modOwner = GetSpellModOwner())
        {
            ...
            if (isPet()) modOwner->ApplySpellMod(spellProto->Id,
                    SPELLMOD_ALL_EFFECTS, DoneAdvertisedBenefit);
        }
    ...
    if (Player* modOwner = GetSpellModOwner()) modOwner->ApplySpellMod(
            spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE,
            tmpDamage);
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator i = mDamageDonebyAP.begin();
            ...
            if ((*i)->GetMiscValue() & schoolMask) DoneAdvertisedBenefit +=
                    int32(
        ...
        Unit *pVictim)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneCreature.begin();
        ...
        if (creatureTypeMask & uint32((*i)->GetMiscValue())) TakenAdvertisedBenefit +=
                (*i)->GetAmount();
    ...
    for (AuraEffectList::const_iterator i = mDamageTaken.begin();
        ...
        if (((*i)->GetMiscValue() & schoolMask) != 0) TakenAdvertisedBenefit +=
                (*i)->GetAmount();
        ...
        uint32 damage, Unit *pVictim)
{
    ...
    if (modOwner) modOwner->ApplySpellMod(spellProto->Id,
        ...
        uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->isTotem()) if (Unit* owner = GetOwner()) return owner->SpellHealingBonus(
            pVictim, spellProto, effIndex, healamount, damagetype, stack);

    // no bonus for heal potions/bandages
    if (spellProto->SpellFamilyName == SPELLFAMILY_POTION) return healamount;
            ...
            i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 7997: // Renewed Hope
            case 7998:
                if (pVictim->HasAura(6788)) DoneTotalMod *= ((*i)->GetAmount()
                        + 100.0f) / 100.0f;
                break;
            case 21: // Test of Faith
            case 6935:
            ...
            case 8477: // Nourish Heal Boost
            {
                ...
                {
                    Aura const * aura = itr->second->GetBase();
                    if (aura->GetCasterGUID() != GetGUID()) continue;
                    SpellEntry const* m_spell = aura->GetSpellProto();
                    if (m_spell->SpellFamilyName != SPELLFAMILY_DRUID
                            || !(m_spell->SpellFamilyFlags [1] & 0x00000010
                                    || m_spell->SpellFamilyFlags [0] & 0x50)) continue;
                    modPercent += stepPercent * aura->GetStackAmount();
                }
                DoneTotalMod *= (modPercent + 100.0f) / 100.0f;
                break;
            ...
            {
                if (pVictim->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN,
                        0, 0x00000400, 0, GetGUID())) DoneTotalMod *=
                        ((*i)->GetAmount() + 100.0f) / 100.0f;
                break;
            }
            default:
    ...
    if (bonus)
    {
        ...
        {
            coeff = bonus->dot_damage;
            if (bonus->ap_dot_bonus > 0) DoneTotal +=
                    int32(
    ...
    else // scripted bonus
    {
                ...
                && spellProto->SpellIconID == 329)
        {
            ...
            if (apBonus > DoneAdvertisedBenefit) DoneTotal += int32(
                    apBonus * 0.22f); // 22% of AP per tick
            else DoneTotal += int32(DoneAdvertisedBenefit * 0.377f); //37.7% of BH per tick
        }
        // Earthliving - 0.45% of normal hot coeff
        else if (spellProto->SpellFamilyName == SPELLFAMILY_SHAMAN
                && spellProto->SpellFamilyFlags [1] & 0x80000) factorMod *=
                0.45f;
            ...
            && spellProto->SpellFamilyFlags [0] & 0x40)
    {
        ...
        if (AuraEffect const* HealingWay = pVictim->GetAuraEffect(29203, 0)) TakenTotalMod *=
                (HealingWay->GetAmount() + 100.0f) / 100.0f;
    }

    // Tenacity increase healing % taken
    if (AuraEffect const* Tenacity = pVictim->GetAuraEffect(58549, 0)) TakenTotalMod *=
            (Tenacity->GetAmount() + 100.0f) / 100.0f;

    // Healing taken percent
    float minval = (float) pVictim->GetMaxNegativeAuraModifier(
            SPELL_AURA_MOD_HEALING_PCT);
    if (minval) TakenTotalMod *= (100.0f + minval) / 100.0f;

    float maxval = (float) pVictim->GetMaxPositiveAuraModifier(
            SPELL_AURA_MOD_HEALING_PCT);
    if (maxval) TakenTotalMod *= (100.0f + maxval) / 100.0f;
    ...
    if (damagetype == DOT)
    {
        ...
        if (minval_hot) TakenTotalMod *= (100.0f + minval_hot) / 100.0f;
    ...
    for (AuraEffectList::const_iterator i = mHealingGet.begin();
        ...
        if (GetGUID() == (*i)->GetCasterGUID()
                && (*i)->IsAffectedOnSpell(spellProto)) TakenTotalMod *=
                ((*i)->GetAmount() + 100.0f) / 100.0f;
...
int32 Unit::SpellBaseHealingBonus(SpellSchoolMask schoolMask)
{
    ...
    for (AuraEffectList::const_iterator i = mHealingDone.begin();
        ...
        if (!(*i)->GetMiscValue() || ((*i)->GetMiscValue() & schoolMask) != 0) AdvertisedBenefit +=
                (*i)->GetAmount();
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator i = mHealingDonebyAP.begin();
            ...
            if ((*i)->GetMiscValue() & schoolMask) AdvertisedBenefit += int32(
        ...
        Unit *pVictim)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageTaken.begin();
            i != mDamageTaken.end(); ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0) AdvertisedBenefit +=
                (*i)->GetAmount();
    return AdvertisedBenefit;
}
...
bool Unit::IsImmunedToDamage(SpellSchoolMask shoolMask)
{
    ...
    for (SpellImmuneList::const_iterator itr = schoolList.begin();
        ...
        if (itr->type & shoolMask) return true;
...
{
    if (spellInfo->Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) return false;
        ...
        for (SpellImmuneList::const_iterator itr = schoolList.begin();
            ...
            if (itr->type & shoolMask
                    && !CanSpellPierceImmuneAura(spellInfo,
                            sSpellStore.LookupEntry(itr->spellId))) return true;
    }
...
bool Unit::IsImmunedToSpell(SpellEntry const* spellInfo)
{
    ...
    for (SpellImmuneList::const_iterator itr = idList.begin();
            itr != idList.end(); ++itr)
        if (itr->type == spellInfo->Id) return true;
    ...
    if (spellInfo->Dispel)
    {
        ...
        for (SpellImmuneList::const_iterator itr = dispelList.begin();
            ...
            if (itr->type == spellInfo->Dispel) return true;
    }
    ...
    if (spellInfo->Mechanic)
    {
        ...
        for (SpellImmuneList::const_iterator itr = mechanicList.begin();
            ...
            if (itr->type == spellInfo->Mechanic) return true;
    }
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (!spellInfo->EffectMechanic [i]
                && spellInfo->Effect [i] != SPELL_EFFECT_KNOCK_BACK) if (IsImmunedToSpellEffect(
                spellInfo, i)) return true;
    }
        ...
        uint32 index) const
{
    ...
    for (SpellImmuneList::const_iterator itr = effectList.begin();
        ...
        if (itr->type == effect) return true;
    ...
    if (uint32 mechanic = spellInfo->EffectMechanic[index])
    {
        ...
        for (SpellImmuneList::const_iterator itr = mechanicList.begin();
                itr != mechanicList.end(); ++itr)
            if (itr->type == mechanic) return true;
    }
    ...
    switch (spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
            if (spellInfo->SpellFamilyFlags [0]
                    == SPELLFAMILYFLAG_SHAMAN_FROST_SHOCK) return true;
            break;
        case SPELLFAMILY_DEATHKNIGHT:
        ...
        WeaponAttackType attType, SpellEntry const *spellProto)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageDoneCreature.begin();
            i != mDamageDoneCreature.end(); ++i)
        if (creatureTypeMask & uint32((*i)->GetMiscValue())) DoneFlatBenefit +=
                (*i)->GetAmount();
    ...
    if (attType == RANGED_ATTACK)
    {
        ...
        for (AuraEffectList::const_iterator i = mCreatureAttackPower.begin();
            ...
            if (creatureTypeMask & uint32((*i)->GetMiscValue())) APbonus +=
                    (*i)->GetAmount();
    }
    else
    {
    ...
    if (APbonus != 0) // Can be negative
    {
        ...
        if (spellProto) for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    ...
    for (AuraEffectList::const_iterator i = mDamageTaken.begin();
            i != mDamageTaken.end(); ++i)
        if ((*i)->GetMiscValue() & GetMeleeDamageSchoolMask()) TakenFlatBenefit +=
                (*i)->GetAmount();

    if (attType != RANGED_ATTACK) TakenFlatBenefit +=
            pVictim->GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_DAMAGE_TAKEN);
    else TakenFlatBenefit += pVictim->GetTotalAuraModifier(
    ...
    if (spellProto) if (GetSpellSchoolMask(spellProto)
            != SPELL_SCHOOL_MASK_NORMAL)
    {
        AuraEffectList const &mModDamagePercentDone = GetAuraEffectsByType(
                SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin();
                i != mModDamagePercentDone.end(); ++i)
            if (((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
                    && !((*i)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL)) DoneTotalMod *=
                    ((*i)->GetAmount() + 100.0f) / 100.0f;
    }
            ...
            i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            {
                if (pVictim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                        SPELLFAMILY_DEATHKNIGHT, 0, 0x02000000, 0)) if (SpellChainNode const *chain = sSpellMgr->GetSpellChainNode((*i)->GetId())) DoneTotalMod *=
                        (chain->rank * 2.0f + 100.0f) / 100.0f;
                break;
            }
        }
    ...
    // Custom scripted damage
    if (spellProto) switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_DEATHKNIGHT:
            // Glacier Rot
            if (spellProto->SpellFamilyFlags [0] & 0x2
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentTaken.begin();
            i != mModDamagePercentTaken.end(); ++i)
        if ((*i)->GetMiscValue() & GetMeleeDamageSchoolMask()) TakenTotalMod *=
                ((*i)->GetAmount() + 100.0f) / 100.0f;
    ...
    if (spellProto)
    {
        ...
        if (spellProto->SpellFamilyName == SPELLFAMILY_DRUID
                && spellProto->SpellFamilyFlags [0] & 0x00008800) mechanicMask |=
                (1 << MECHANIC_BLEED);
        ...
        if (mechanicMask)
        {
            ...
            for (AuraEffectList::const_iterator i = mDamageDoneMechanic.begin();
                    i != mDamageDoneMechanic.end(); ++i)
                if (mechanicMask & uint32(1 << ((*i)->GetMiscValue()))) TakenTotalMod *=
                        ((*i)->GetAmount() + 100.0f) / 100.0f;
        }
    }
        ...
        switch ((*i)->GetSpellProto()->SpellIconID)
        {
            ...
            case 1804:
            {
                if ((*i)->GetSpellProto()->SpellFamilyName
                        != SPELLFAMILY_PALADIN) continue;

                if ((*i)->GetMiscValue()
                        & (spellProto ? GetSpellSchoolMask(spellProto) : 0)) TakenTotalMod *=
                        ((*i)->GetAmount() + 100.0f) / 100.0f;
                break;
            }
                // Ebon Plague
...
float Unit::GetWeaponProcChance() const
{
    ...
    // (odd formula...)
    if (isAttackReady(BASE_ATTACK)) return (GetAttackTime(BASE_ATTACK) * 1.8f
            / 1000.0f);
    else if (haveOffhandWeapon() && isAttackReady(OFF_ATTACK)) return (GetAttackTime(
            OFF_ATTACK) * 1.6f / 1000.0f);
    return 0;
}
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    if (Player* plr = ToPlayer())
    {
        ...
        if (pet)
        {
            ...
            // don't unsummon pet in bg or arena but SetFlag UNIT_FLAG_STUNNED to disable pet's interface
            if (bg)
        ...
        if (plr->GetEmoteState()) plr->SetEmoteState(0);
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        if (Pet *pPet = this->ToPlayer()->GetPet())
        {
            if (pPet && pPet->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED)
                    && !pPet->HasUnitState(UNIT_STAT_STUNNED)) pPet->RemoveFlag(
                    UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED);
        }
        else this->ToPlayer()->ResummonPetTemporaryUnSummonedIfAny();
    }
...
void Unit::CombatStart(Unit* target, bool initialAggro)
{
    if (initialAggro)
    {
        if (!target->IsStandState()) target->SetStandState(
    ...
    if (who->GetTypeId() == TYPEID_PLAYER) SetContestedPvP(who->ToPlayer());
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (PvP) m_CombatTimer = 5000;

    if (isInCombat() || HasUnitState(UNIT_STAT_EVADE)) return;
    ...
    if (GetTypeId() != TYPEID_PLAYER)
    {
        ...
        if ((IsAIEnabled && this->ToCreature()->AI()->IsEscorted())
                ...
                || GetMotionMaster()->GetCurrentMovementGeneratorType()
                        == POINT_MOTION_TYPE) this->ToCreature()->SetHomePosition(
        ...
        if (enemy)
        {
            ...
            if (this->ToCreature()->GetFormation()) this->ToCreature()->GetFormation()->MemberAttackStart(
        ...
        if (!(ToCreature()->GetCreatureInfo()->type_flags
                & CREATURE_TYPEFLAGS_MOUNTED_COMBAT)) Unmount();
    }
    ...
    if (GetTypeId() != TYPEID_PLAYER)
    {
        ...
        if (creature->GetCreatureInfo()
                && creature->GetCreatureInfo()->unit_flags
                        & UNIT_FLAG_OOC_NOT_ATTACKABLE) SetFlag(
                UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE); //re-apply Out of Combat Non Attackable flag if we leave combat, can be overriden in scripts in EnterEvadeMode()

        ClearUnitState(UNIT_STAT_ATTACK_PLAYER);
        if (HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_TAPPED)) SetUInt32Value(
                UNIT_DYNAMIC_FLAGS,
                ((Creature*) this)->GetCreatureInfo()->dynamicflags);
    }
    else this->ToPlayer()->UpdatePotionCooldown();

    if (GetTypeId() != TYPEID_PLAYER && ((Creature*) this)->isPet())
    {
        if (Unit *owner = GetOwner()) for (uint8 i = 0; i < MAX_MOVE_TYPE; ++i)
            if (owner->GetSpeedRate(UnitMoveType(i))
                    > GetSpeedRate(UnitMoveType(i))) SetSpeed(UnitMoveType(i),
                    owner->GetSpeedRate(UnitMoveType(i)), true);
    }
    else if (!isCharmed()) return;
...
bool Unit::isTargetableForAttack(bool checkFakeDeath) const
{
    ...
    if (HasFlag(
            ...
            UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE
                    | UNIT_FLAG_OOC_NOT_ATTACKABLE)) return false;

    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->isGameMaster()) return false;

    return !HasUnitState(UNIT_STAT_UNATTACKABLE)
            && (!checkFakeDeath || !HasUnitState(UNIT_STAT_DIED));
}
...
bool Unit::canAttack(Unit const* target, bool force) const
{
    ...
    if (force)
    {
        ...
        if (GetTypeId() != TYPEID_PLAYER)
        {
            if (isPet())
            {
                if (Unit *owner = GetOwner()) if (!(owner->canAttack(target))) return false;
            }
            else if (!IsHostileTo(target)) return false;
        }
    }
    else if (!IsHostileTo(target)) return false;

    if (!target->isAttackableByAOE()) return false;

    // can't attack unattackable units or GMs
    if (target->HasUnitState(UNIT_STAT_UNATTACKABLE)
            || (target->GetTypeId() == TYPEID_PLAYER
                    && target->ToPlayer()->isGameMaster())) return false;

    // check flags
    if (target->HasFlag(
            UNIT_FIELD_FLAGS,
            UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT
                    | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNK_16)
            || (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)
                    && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE))
            || (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)
                    && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE))
            || (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)
                    && target->HasFlag(UNIT_FIELD_FLAGS,
                            UNIT_FLAG_OOC_NOT_ATTACKABLE))
            || (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)
                    && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE))) return false;

    // CvC case - can attack each other only when one of them is hostile
    if (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)
            && !target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE)) return GetReactionTo(
            target) <= REP_HOSTILE || target->GetReactionTo(this) <= REP_HOSTILE;

    // PvP, PvC, CvP case
    // can't attack friendly targets
    if (GetReactionTo(target) > REP_NEUTRAL
            || target->GetReactionTo(this) > REP_NEUTRAL) return false;

    if (target->HasUnitState(UNIT_STAT_DIED))
    {
        if (!ToCreature() || !ToCreature()->isGuard()) return false;

        Player const* playerAffectingAttacker =
                HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) ?
                        GetAffectingPlayer() : NULL;
        Player const* playerAffectingTarget =
                target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) ?
                        target->GetAffectingPlayer() : NULL;
        // check duel - before sanctuary checks
        if (playerAffectingAttacker && playerAffectingTarget) if (playerAffectingAttacker->duel
                && playerAffectingAttacker->duel->opponent
                        == playerAffectingTarget
                && playerAffectingAttacker->duel->startTime != 0) return true;

        // additional checks - only PvP case
        if (playerAffectingAttacker && playerAffectingTarget)
        {
            if (target->GetByteValue(UNIT_FIELD_BYTES_2, 1)
                    & UNIT_BYTE2_FLAG_PVP) return true;
        ...
        if (!target->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_FEIGN_DEATH)) return false;
    }

    // can't attack own vehicle or passenger
    if (m_vehicle) if (IsOnVehicle(target)
            || m_vehicle->GetBase()->IsOnVehicle(target)) return false;

    if (!canSeeOrDetect(target)) return false;
...
{
    if (isAlive() == requireDeadTarget) return false;
...
{
    if (WorldObject::isAlwaysVisibleFor(seer)) return true;

    // Always seen by owner
    if (uint64 guid = GetCharmerOrOwnerGUID()) if (seer->GetGUID() == guid) return true;
...
{
    if (WorldObject::isAlwaysDetectableFor(seer)) return true;

    if (HasAuraTypeWithCaster(SPELL_AURA_MOD_STALKED, seer->GetGUID())) return true;
...
{
    if (!x) m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM,
            SEC_GAMEMASTER);
    else m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
...
void Unit::SetSpeed(UnitMoveType mtype, float rate, bool forced)
{
    ...
    if (m_speed_rate [mtype] == rate) return;
    ...
    else
    {
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (!isInCombat()) if (Pet* pet = this->ToPlayer()->GetPet()) pet->SetSpeed(
                    mtype, m_speed_rate [mtype], forced);
        }
...
{
    if (on) CastSpell(this, 11010, true);
    else RemoveAurasDueToSpell(11010);
}
...
void Unit::setDeathState(DeathState s)
{
    ...
    else if (s == JUST_ALIVED) RemoveFlag(UNIT_FIELD_FLAGS,
            UNIT_FLAG_SKINNABLE); // clear skinnable for creature and player (at battleground)

    if (oldDeathState != ALIVE && s == ALIVE)
    {
        // Reset display id on resurection - needed by corpse explosion to cleanup after display change
        // TODO: fix this
        if (!HasAuraType(SPELL_AURA_TRANSFORM)) SetDisplayId(
                GetNativeDisplayId());
    }
}
...
bool Unit::CanHaveThreatList() const
{
    ...
    if (GetTypeId() != TYPEID_UNIT) return false;

    // only alive units can have threat list
    if (!isAlive() || isDying()) return false;

    // totems can not have threat list
    if (this->ToCreature()->isTotem()) return false;
...
{
    if (!HasAuraType(SPELL_AURA_MOD_THREAT) || fThreat < 0) return fThreat;
        ...
        SpellEntry const *threatSpell)
{
    ...
    if (CanHaveThreatList()) m_ThreatManager.addThreat(pVictim, fThreat,
            schoolMask, threatSpell);
}
...
{
    if (CanHaveThreatList() && !m_ThreatManager.isThreatListEmpty()) SendClearThreatListOpcode();
...
void Unit::TauntApply(Unit* taunter)
{
    ...
    if (!taunter
            || (taunter->GetTypeId() == TYPEID_PLAYER
                    && taunter->ToPlayer()->isGameMaster())) return;

    if (!CanHaveThreatList()) return;

    if (this->ToCreature()->HasReactState(REACT_PASSIVE)) return;

    Unit *target = getVictim();
    if (target && target == taunter) return;

    SetInFront(taunter);
    if (this->ToCreature()->IsAIEnabled) this->ToCreature()->AI()->AttackStart(
            taunter);
...
void Unit::TauntFadeOut(Unit *taunter)
{
    ...
    {
        if (this->ToCreature()->IsAIEnabled) this->ToCreature()->AI()->EnterEvadeMode();
        return;
    }
...
Unit* Creature::SelectVictim()
{
    ...
    if (!tauntAuras.empty())
    {
        ...
        if (caster && caster->isAlive()) return getVictim();
    ...
    else if (!HasReactState(REACT_PASSIVE))
    {
        ...
        if (!target && isSummon())
        {
            ...
            {
                if (owner->isInCombat()) target = owner->getAttackerForHelper();
                if (!target)
                {
            ...
            itr != m_attackers.end(); ++itr)
    {
        if ((*itr) && !canCreatureAttack(*itr)
                ...
                && !(*itr)->ToCreature()->HasUnitTypeMask(
                        UNIT_MASK_CONTROLABLE_GUARDIAN)) return NULL;
    }

    // TODO: a vehicle may eat some mob, so mob should not evade
    if (GetVehicle()) return NULL;

    // search nearby enemy before enter evade mode
    if (HasReactState(REACT_AGGRESSIVE))
    {
        target = SelectNearestTargetInAttackDistance();
        if (target && _IsTargetAcceptable(target)) return target;
    }
        ...
        uint8 effect_index, int32 value) const
{
    if (Player* modOwner = GetSpellModOwner())
    {
        ...
        {
            case 0:
                modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_EFFECT1,
                        value);
                break;
            case 1:
                modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_EFFECT2,
                        value);
                break;
            case 2:
                modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_EFFECT3,
                        value);
                break;
        }
    }
...
int32 Unit::CalcSpellDuration(SpellEntry const* spellProto)
{
    ...
    if (comboPoints && minduration != -1 && minduration != maxduration) duration =
        ...
        int32 duration, bool positive)
{
    ...
    if (!positive)
    {
        ...
        for (uint8 i = 1; i <= MECHANIC_ENRAGED; ++i)
        {
            ...
            if (new_durationMod_always < durationMod_always) durationMod_always =
                    new_durationMod_always;
            if (new_durationMod_not_stack < durationMod_not_stack) durationMod_not_stack =
                    new_durationMod_not_stack;
        }

        // Select strongest negative mod
        if (durationMod_always > durationMod_not_stack) durationMod =
                durationMod_not_stack;
        else durationMod = durationMod_always;

        if (durationMod != 0) duration = int32(
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
                                    ...
                                    spellProto->Id, SPELL_GROUP_ELIXIR_GUARDIAN)))
            {
                if (target->HasAura(53042)
                        && target->HasSpell(spellProto->EffectTriggerSpell [0])) duration *=
                        2;
            }
        }
    }
    ...
    if (target == this)
    {
        switch (spellProto->SpellFamilyName)
        {
            case SPELLFAMILY_DRUID:
                ...
                {
                    // Glyph of Thorns
                    if (AuraEffect * aurEff = GetAuraEffect(57862, 0)) duration +=
                            aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                break;
            case SPELLFAMILY_PALADIN:
                if (spellProto->SpellFamilyFlags [0] & 0x00000002)
                {
                    ...
                    if (AuraEffect * aurEff = GetAuraEffect(57958, 0)) duration +=
                            aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                else if (spellProto->SpellFamilyFlags [0] & 0x00010000)
                {
                    // Glyph of Blessing of Wisdom
                    if (AuraEffect * aurEff = GetAuraEffect(57979, 0)) duration +=
                            aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                break;
        }
...
void Unit::ModSpellCastTime(SpellEntry const* spellProto, int32 & castTime,
        Spell * spell)
{
    if (!spellProto || castTime < 0) return;
    //called from caster
    if (Player* modOwner = GetSpellModOwner()) modOwner->ApplySpellMod(
            spellProto->Id, SPELLMOD_CASTING_TIME, castTime, spell);

    if (!(spellProto->Attributes & (SPELL_ATTR0_UNK4 | SPELL_ATTR0_TRADESPELL))
            && spellProto->SpellFamilyName) castTime = int32(
            float(castTime) * GetFloatValue(UNIT_MOD_CAST_SPEED));
    else if (spellProto->Attributes & SPELL_ATTR0_REQ_AMMO
            && !(spellProto->AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG)) castTime =
            int32(float(castTime) * m_modAttackSpeedPct [RANGED_ATTACK]);
...
void Unit::IncrDiminishing(DiminishingGroup group)
{
    ...
    {
        if (i->DRGroup != group) continue;
        if (int32(i->hitCount) < GetDiminishingReturnsMaxLevel(group)) i->hitCount +=
                1;
        return;
    }
...
{
    if (duration == -1 || group == DIMINISHING_NONE) return 1.0f;
    ...
    if (limitduration > 0 && duration > limitduration)
    {
        ...
        if ((target->GetTypeId() == TYPEID_PLAYER
                ...
                && source->GetTypeId() == TYPEID_PLAYER) duration =
                limitduration;
    }
...
void Unit::ApplyDiminishingAura(DiminishingGroup group, bool apply)
{
            ...
            i != m_Diminishing.end(); ++i)
    {
        ...
        else if (i->stack)
        {
            ...
            if (i->stack == 0) i->hitTime = getMSTime();
        }
        break;
    }
}
        ...
        const SpellRangeEntry * rangeEntry)
{
    ...
    if (rangeEntry->maxRangeHostile == rangeEntry->maxRangeFriend) return uint32(
            rangeEntry->maxRangeFriend);
    if (IsHostileTo(target)) return uint32(rangeEntry->maxRangeHostile);
...
{
    if (!rangeEntry) return 0;
    if (rangeEntry->minRangeHostile == rangeEntry->minRangeFriend) return uint32(
            rangeEntry->minRangeFriend);
    if (IsHostileTo(target)) return uint32(rangeEntry->minRangeHostile);
...
{
    if (!radiusEntry) return 0;
    if (radiusEntry->radiusHostile == radiusEntry->radiusFriend) return uint32(
            radiusEntry->radiusFriend);
    if (IsHostileTo(target)) return uint32(radiusEntry->radiusHostile);
...
uint32 Unit::GetCreatureType() const
{
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (ssEntry && ssEntry->creatureType > 0) return ssEntry->creatureType;
        else return CREATURE_TYPE_HUMANOID;
    }
    else return this->ToCreature()->GetCreatureInfo()->type;
}
        ...
        float amount, bool apply)
{
    ...
    if (!CanModifyStats()) return false;
        ...
        UnitModifierType modifierType) const
{
    ...
    if (modifierType == TOTAL_PCT
            && m_auraModifiersGroup [unitMod] [modifierType] <= 0.0f) return 0.0f;
...
float Unit::GetTotalStatValue(Stats stat) const
{
    ...
    if (m_auraModifiersGroup [unitMod] [TOTAL_PCT] <= 0.0f) return 0.0f;
...
{
    if (attType == OFF_ATTACK && !haveOffhandWeapon()) return 0.0f;
...
void Unit::SetLevel(uint8 lvl)
{
    ...
    // group update
    if (GetTypeId() == TYPEID_PLAYER && this->ToPlayer()->GetGroup()) this->ToPlayer()->SetGroupUpdateFlag(
            GROUP_UPDATE_FLAG_LEVEL);
}

void Unit::SetHealth(uint32 val)
{
    if (getDeathState() == JUST_DIED) val = 0;
    else if (GetTypeId() == TYPEID_PLAYER
            && (getDeathState() == DEAD || getDeathState() == DEAD_FALLING)) val =
            1;
    else
    {
...
void Unit::SetPower(Powers power, uint32 val)
{
    ...
    else if (this->ToCreature()->isPet())
    {
        ...
        if (pet->getPetType() == HUNTER_PET && power == POWER_HAPPINESS) pet->UpdateDamagePhysical(
                BASE_ATTACK);
    }
}
...
void Unit::SetMaxPower(Powers power, uint32 val)
{
    ...
    if (val < cur_power) SetPower(power, val);
}
    ...
    switch (power)
    {
        case POWER_MANA:
            if (getClass() == CLASS_HUNTER || getClass() == CLASS_WARRIOR
                    ...
                    || getClass() == CLASS_DEATH_KNIGHT) return false;
            else return GetCreateMana();

        case POWER_RAGE:
            return 1000;
        case POWER_FOCUS:
            if (GetTypeId() == TYPEID_PLAYER
                    && (ToPlayer()->getClass() == CLASS_HUNTER)) return 100;
        ...
        case POWER_SOUL_SHARDS:
            return (GetTypeId() == TYPEID_PLAYER
                    && ((Player const*) this)->getClass() == CLASS_WARLOCK ?
                    3 : 0);
        case POWER_ECLIPSE:
            return (GetTypeId() == TYPEID_PLAYER
                    && ((Player const*) this)->getClass() == CLASS_DRUID ?
                    100 : 0);
        case POWER_HOLY_POWER:
            return (GetTypeId() == TYPEID_PLAYER
                    && ((Player const*) this)->getClass() == CLASS_PALADIN ?
                    3 : 0);
        case POWER_HEALTH:
            return 0;
        default:
...
void Unit::CleanupsBeforeDelete(bool finalCleanup)
{
    ...
    if (Creature* thisCreature = ToCreature()) if (GetTransport()) GetTransport()->RemovePassenger(
            thisCreature);
}
...
void Unit::UpdateCharmAI()
{
    ...
    else
    {
        ...
        {
            i_disabledAI = i_AI;
            if (isPossessed() || IsVehicle()) i_AI = new PossessedAI(
                    this->ToCreature());
            else i_AI = new PetAI(this->ToCreature());
        }
    }
}
...
{
    if (!m_charmInfo) m_charmInfo = new CharmInfo(this);
...
{
    if (m_unit->GetTypeId() == TYPEID_UNIT) if (Creature *pCreature = m_unit->ToCreature()) pCreature->SetReactState(
            m_oldReactState);
}
...
{
    if (withAttack) SetActionBar(ACTION_BAR_INDEX_START, COMMAND_ATTACK,
            ACT_COMMAND);
    else SetActionBar(ACTION_BAR_INDEX_START, 0, ACT_PASSIVE);
...
void CharmInfo::InitPossessCreateSpells()
{
    ...
    if (m_unit->GetTypeId() == TYPEID_UNIT)
    {
        for (uint32 i = 0; i < CREATURE_MAX_SPELLS; ++i)
        {
            ...
            SpellEntry const *spellInfo = sSpellStore.LookupEntry(spellId);
            if (spellInfo
                    && spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD) spellId =
                    0;
            if (IsPassiveSpell(spellId)) m_unit->CastSpell(m_unit, spellId,
                    true);
            else AddSpellToActionBar(m_unit->ToCreature()->m_spells [i],
                    ACT_PASSIVE);
        }
    }
}
...
{
    if (IsPassiveSpell(spellid)) return;

    for (uint32 x = 0; x < MAX_SPELL_CHARM; ++x)
        if (spellid == m_charmspells [x].GetAction()) m_charmspells [x].SetType(
                apply ? ACT_ENABLED : ACT_DISABLED);
}

void CharmInfo::SetPetNumber(uint32 petnumber, bool statwindow)
{
    m_petnumber = petnumber;
    if (statwindow) m_unit->SetUInt32Value(UNIT_FIELD_PETNUMBER, m_petnumber);
    else m_unit->SetUInt32Value(UNIT_FIELD_PETNUMBER, 0);
}
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    if (tokens.size() != (ACTION_BAR_INDEX_END - ACTION_BAR_INDEX_START) * 2) return; // non critical, will reset to default
            ...
            index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
        ...
        {
            if (!sSpellStore.LookupEntry(PetActionBar [index].GetAction())) SetActionBar(
                    index, 0, ACT_PASSIVE);
            else if (!IsAutocastableSpell(PetActionBar [index].GetAction())) SetActionBar(
        ...
        SpellMissInfo missCondition)
{
    ...
    // Check victim state
    if (missCondition != SPELL_MISS_NONE) switch (missCondition)
    {
    ...
    {
        // On block
        if (damageInfo->blocked) procEx |= PROC_EX_BLOCK;
        // On absorb
        if (damageInfo->absorb) procEx |= PROC_EX_ABSORB;
        // On crit
        if (damageInfo->HitInfo & SPELL_HIT_TYPE_CRIT) procEx |=
                PROC_EX_CRITICAL_HIT;
        else procEx |= PROC_EX_NORMAL_HIT;
    }
    return procEx;
}
        ...
        SpellEntry const * procAura)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER
            && this->ToPlayer()->GetSession()->PlayerLoading()) return;
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
                    ...
                    & (PROC_EX_NORMAL_HIT | PROC_EX_MISS | PROC_EX_RESIST))
            {
                if (pTarget->GetTypeId() != TYPEID_PLAYER
                        && pTarget->GetCreatureType() != CREATURE_TYPE_CRITTER) this->ToPlayer()->UpdateCombatSkills(
                        pTarget, attType, isVictim);
            }
            // Update defence if player is victim and parry/dodge/block
            else if (isVictim
                    && procExtra
                            & (PROC_EX_DODGE | PROC_EX_PARRY | PROC_EX_BLOCK)) this->ToPlayer()->UpdateCombatSkills(
                    pTarget, attType, true);
        }
            ...
            itr != GetAppliedAuras().end(); ++itr)
    {
        ...
        if (procAura && procAura->Id == itr->first) continue;
        ProcTriggeredData triggerData(itr->second->GetBase());
        // Defensive procs are active on absorbs (so absorption effects are not a hindrance)
        bool active = (damage > 0)
                || (procExtra & (PROC_EX_ABSORB | PROC_EX_BLOCK) && isVictim);
        if (isVictim) procExtra &= ~PROC_EX_INTERNAL_REQ_FAMILY;
        ...
        if (!IsTriggeredAtSpellProcEvent(pTarget, triggerData.aura, procSpell,
                ...
                triggerData.spellProcEvent)) continue;
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (itr->second->HasEffect(i))
            {
                ...
                // Skip this auras
                if (isNonTriggerAura [aurEff->GetAuraType()]) continue;
                // If not trigger by default and spellProcEvent == NULL - skip
                if (!isTriggerAura [aurEff->GetAuraType()]
                        && triggerData.spellProcEvent == NULL) continue;
                // Some spells must always trigger
                if (!triggered || isAlwaysTriggeredAura [aurEff->GetAuraType()]) triggerData.effMask |=
                        1 << i;
            }
        }
        if (triggerData.effMask) procTriggered.push_front(triggerData);
    }

    // Nothing found
    if (procTriggered.empty()) return;

    if (procExtra & (PROC_EX_INTERNAL_TRIGGERED | PROC_EX_INTERNAL_CANT_PROC)) SetCantProc(
            true);
            ...
            i != procTriggered.end(); ++i)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER && i->spellProcEvent
                && i->spellProcEvent->cooldown) cooldown =
                i->spellProcEvent->cooldown;

        if (spellInfo->AttributesEx3 & SPELL_ATTR3_DISABLE_PROC) SetCantProc(
                true);
        ...
        if (!handled) for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS;
                ++effIndex)
        {
            ...
            switch (triggeredByAura->GetAuraType())
            {
                case SPELL_AURA_PROC_TRIGGER_SPELL:
                {
                    ...
                    if (HandleProcTriggerSpell(pTarget, damage, triggeredByAura,
                            procSpell, procFlag, procExtra, cooldown)) takeCharges =
                            true;
                    break;
                }
                ...
                case SPELL_AURA_OVERRIDE_CLASS_SCRIPTS:
                {
                    ...
                    if (HandleOverrideClassScriptAuraProc(pTarget, damage,
                            triggeredByAura, procSpell, cooldown)) takeCharges =
                            true;
                    break;
                }
                ...
                case SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK:
                    ...
                    if (procSpell && GetSpellCastTime(procSpell) != 0) takeCharges =
                            true;
                    break;
                ...
                case SPELL_AURA_MECHANIC_IMMUNITY:
                    ...
                    if (procSpell
                            && procSpell->Mechanic
                                    == uint32(triggeredByAura->GetMiscValue())) takeCharges =
                            true;
                    break;
                ...
                case SPELL_AURA_MOD_DAMAGE_FROM_CASTER:
                    // Compare casters
                    if (triggeredByAura->GetCasterGUID() == pTarget->GetGUID()) takeCharges =
                            true;
                    break;
                case SPELL_AURA_MOD_SPELL_CRIT_CHANCE:
                    ...
                    if (procSpell
                            && HandleSpellCritChanceAuraProc(pTarget, damage,
                                    ...
                                    procExtra, cooldown)) takeCharges = true;
                    break;
                ...
                case SPELL_AURA_TRANSFORM:
                {
                    ...
                    if (useCharges) takeCharges = true;
                    else
                    {
                                ...
                                && (procSpell->Id == triggeredByAura->GetId()))
                        {
                            ...
                            if (aura->GetDuration() == aura->GetMaxDuration()) break;
                        }
                        int32 damageLeft = triggeredByAura->GetAmount();
                        // No damage left
                        if (damageLeft < int32(damage)) i->aura->Remove();
                        else triggeredByAura->SetAmount(damageLeft - damage);
                    }
                    break;
                }
        ...
        if (useCharges && takeCharges) i->aura->DropCharge();
...
{
    if (GetTypeId() == TYPEID_PLAYER) return (Player*) this;
    if (this->ToCreature()->isPet() || this->ToCreature()->isTotem())
    {
        Unit* owner = GetOwner();
        if (owner && owner->GetTypeId() == TYPEID_PLAYER) return (Player*) owner;
    }
    return NULL;
}
...
void Unit::StopMoving()
{
    ...
    if (!(GetUnitMovementFlags() & MOVEMENTFLAG_ONTRANSPORT)) SendMonsterStop();
}
...
void Unit::SetStandState(uint8 state)
{
    ...
    if (IsStandState()) RemoveAurasWithInterruptFlags(
...
void Unit::RestoreDisplayId()
{
    ...
    if (handledAura) handledAura->HandleEffect(this,
            AURA_EFFECT_HANDLE_SEND_FOR_CLIENT, true);
    // we've found shapeshift
    else if (uint32 modelId = GetModelForForm(GetShapeshiftForm())) SetDisplayId(
            modelId);
    // no auras found - set modelid to default
    else SetDisplayId(GetNativeDisplayId());
}
...
void Unit::ClearComboPointHolders()
{
    while (!m_ComboPointHolders.empty())
    {
        ...
        else m_ComboPointHolders.erase(lowguid); // or remove manually
    }
}
...
void Unit::ClearAllReactives()
{
    ...
    if (HasAuraState(AURA_STATE_DEFENSE)) ModifyAuraState(AURA_STATE_DEFENSE,
            false);
    if (getClass() == CLASS_HUNTER && HasAuraState(AURA_STATE_HUNTER_PARRY)) ModifyAuraState(
            AURA_STATE_HUNTER_PARRY, false);
    if (getClass() == CLASS_WARRIOR && GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->ClearComboPoints();
}
...
void Unit::UpdateReactives(uint32 p_time)
{
    for (uint8 i = 0; i < MAX_REACTIVE; ++i)
    {
        ...
        if (!m_reactiveTimer [reactive]) continue;
        ...
        if (m_reactiveTimer [reactive] <= p_time)
        {
            ...
            switch (reactive)
            {
                case REACTIVE_DEFENSE:
                    if (HasAuraState(AURA_STATE_DEFENSE)) ModifyAuraState(
                            AURA_STATE_DEFENSE, false);
                    break;
                case REACTIVE_HUNTER_PARRY:
                    if (getClass() == CLASS_HUNTER
                            && HasAuraState(AURA_STATE_HUNTER_PARRY)) ModifyAuraState(
                            AURA_STATE_HUNTER_PARRY, false);
                    break;
                case REACTIVE_OVERPOWER:
                    if (getClass() == CLASS_WARRIOR
                            && GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->ClearComboPoints();
                    break;
                default:
                    break;
            }
...
Unit* Unit::SelectNearbyTarget(float dist) const
{
    ...
    if (getVictim()) targets.remove(getVictim());
            ...
            tIter != targets.end();)
    {
        if (!IsWithinLOSInMap(*tIter) || (*tIter)->isTotem()
                ...
                || (*tIter)->GetCreatureType() == CREATURE_TYPE_CRITTER) targets.erase(
                tIter++);
        else ++tIter;
    }
...
{
    if (val > 0) ApplyPercentModFloatValue(UNIT_MOD_CAST_SPEED, val, !apply);
    else ApplyPercentModFloatValue(UNIT_MOD_CAST_SPEED, -val, apply);
}
        ...
        DamageEffectType damagetype, uint32 CastingTime)
{
    ...
    if (CastingTime == 0 && GetTypeId() == TYPEID_UNIT
            && !this->ToCreature()->isPet()) return 3500;

    if (CastingTime > 7000) CastingTime = 7000;
    if (CastingTime < 1500) CastingTime = 1500;

    if (damagetype == DOT && !IsChanneledSpell(spellProto)) CastingTime = 3500;
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (spellProto->Effect [i])
        {
            ...
            case SPELL_EFFECT_APPLY_AURA:
                switch (spellProto->EffectApplyAuraName [i])
                {
                    ...
                    case SPELL_AURA_PERIODIC_HEAL:
                    case SPELL_AURA_PERIODIC_LEECH:
                        if (GetSpellDuration(spellProto)) overTime =
        ...
        if (IsAreaEffectTarget [spellProto->EffectImplicitTargetA [i]]
                || IsAreaEffectTarget [spellProto->EffectImplicitTargetB [i]]) AreaEffect =
                true;
    }
    ...
    if (overTime > 0 && CastingTime > 0 && DirectDamage)
    {
        ...
        if (damagetype == DOT) CastingTime = uint32(CastingTime * PtOT);
        else if (PtOT < 1.0f) CastingTime = uint32(CastingTime * (1 - PtOT));
        else CastingTime = 0;
    }

    // Area Effect Spells receive only half of bonus
    if (AreaEffect) CastingTime /= 2;
...
{
    if (!normalized || GetTypeId() != TYPEID_PLAYER) return float(
            GetAttackTime(attType)) / 1000.0f;

    Item *Weapon = this->ToPlayer()->GetWeaponForAttack(attType, true);
    if (!Weapon) return 2.4f; // fist attack
    ...
    if (!player
            || (attackedPlayer
                    && (attackedPlayer == player
                            || (player->duel
                                    && player->duel->opponent == attackedPlayer)))) return;
...
void Unit::AddPetAura(PetAura const* petSpell)
{
    ...
    if (Pet* pet = this->ToPlayer()->GetPet()) pet->CastPetAura(petSpell);
}
...
void Unit::RemovePetAura(PetAura const* petSpell)
{
    ...
    if (Pet* pet = this->ToPlayer()->GetPet()) pet->RemoveAurasDueToSpell(
            petSpell->GetAura(pet->GetEntry()));
}

Pet* Unit::CreateTamedPetFrom(Creature* creatureTarget, uint32 spell_id)
{
    if (GetTypeId() != TYPEID_PLAYER) return NULL;
...
bool Unit::InitTamedPet(Pet * pet, uint8 level, uint32 spell_id)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER) pet->SetUInt32Value(UNIT_FIELD_FLAGS,
        ...
        SpellProcEventEntry const *& spellProcEvent)
{
    ...
    else EventProcFlag = spellProto->procFlags; // else get from spell proto
    // Continue if no trigger exist
    if (!EventProcFlag) return false;

    // Additional checks for triggered spells (ignore trap casts)
    if (procExtra & PROC_EX_INTERNAL_TRIGGERED
            && !(procFlag & PROC_FLAG_DONE_TRAP_ACTIVATION))
    {
        if (!(spellProto->AttributesEx3 & SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED)) return false;
    }

    // Check spellProcEvent data requirements
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProcEvent,
            EventProcFlag, procSpell, procFlag, procExtra, active)) return false;
    ...
    if (procSpell && procSpell->Id == spellProto->Id
            && !(spellProto->procFlags
                    & (PROC_FLAG_TAKEN_PERIODIC | PROC_FLAG_KILL))) return false;
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if (spellProto->EquippedItemClass == ITEM_CLASS_WEAPON)
        {
            ...
            if (attType == BASE_ATTACK) item =
                    this->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
                            EQUIPMENT_SLOT_MAINHAND);
            else if (attType == OFF_ATTACK) item =
                    this->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
                            EQUIPMENT_SLOT_OFFHAND);
            else item = this->ToPlayer()->GetUseableItemByPos(
                    INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED);

            if (this->ToPlayer()->IsInFeralForm()) return false;
    ...
    if (spellProcEvent && spellProcEvent->customChance) chance =
            spellProcEvent->customChance;

    // Missile Barrage
    if (spellProto->SpellFamilyName == SPELLFAMILY_MAGE
            && spellProto->SpellIconID == 3261) if (!(procSpell->SpellFamilyFlags [0]
            & 0x20000000)) chance /= 2.0;
...
void Unit::Kill(Unit *pVictim, bool durabilityLoss)
{
    ...
    if (!pVictim->GetHealth()) return;

    // Inform pets (if any) when player kills target)
    if (this->ToPlayer())
    {
        Pet *pPet = this->ToPlayer()->GetPet();
        if (pPet && pPet->isAlive() && pPet->isControlled()) pPet->AI()->KilledUnit(
                pVictim);
    }
    ...
    if (creature)
    {
        ...
        if (!bRewardIsAllowed) creature->SetLootRecipient(NULL);
    }

    if (bRewardIsAllowed && creature && creature->GetLootRecipient()) player =
            creature->GetLootRecipient();
    ...
    if (bRewardIsAllowed && player && player != pVictim)
    {
        ...
        if (Group *group = player->GetGroup())
        {
            ...
            if (creature)
            {
                ...
                if (group->GetLooterGuid())
                {
                    ...
                    else group->SendLooter(creature, NULL);
                }
                else group->SendLooter(creature, NULL);
        ...
        if (creature)
        {
            ...
            if (creature->lootForPickPocketed) creature->lootForPickPocketed =
                    false;

            loot->clear();
            if (uint32 lootid = creature->GetCreatureInfo()->lootid) loot->FillLoot(
                    lootid, LootTemplates_Creature, pLooter, false, false,
    ...
    if (isPet() || isTotem()) if (Unit *owner = GetOwner()) owner->ProcDamageAndSpell(
            pVictim, PROC_FLAG_KILL, PROC_FLAG_NONE, PROC_EX_NONE, 0);

    if (pVictim->GetCreatureType() != CREATURE_TYPE_CRITTER) ProcDamageAndSpell(
            pVictim, PROC_FLAG_KILL, PROC_FLAG_KILLED, PROC_EX_NONE, 0);
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (GetTypeId() == TYPEID_UNIT && this->ToCreature()->IsAIEnabled) this->ToCreature()->AI()->KilledUnit(
                pVictim);
    ...
    else // creature died
    {
        ...
        if (!creature->isPet())
        {
            ...
            if (cInfo && (cInfo->lootid || cInfo->maxgold > 0)) creature->SetFlag(
        ...
        if (creature->IsAIEnabled) creature->AI()->JustDied(this);
        ...
        if (creature->ToTempSummon())
        {
            if (Unit* pSummoner = creature->ToTempSummon()->GetSummoner())
            {
                if (pSummoner->ToCreature()
                        && pSummoner->ToCreature()->IsAIEnabled) pSummoner->ToCreature()->AI()->SummonedCreatureDies(
                        creature, this);
            }
        }
            ...
            if (m->IsDungeon() && creditedPlayer)
            {
                if (m->IsRaidOrHeroicDungeon())
                {
                    if (creature->GetCreatureInfo()->flags_extra
                            & CREATURE_FLAG_EXTRA_INSTANCE_BIND) ((InstanceMap *) m)->PermBindAllPlayers(
                            creditedPlayer);
                }
                else
                {
                    ...
                    if (InstanceSave * save = sInstanceSaveMgr->GetInstanceSave(
                            creature->GetInstanceId())) if (save->GetResetTime()
                            < resettime) save->SetResetTime(resettime);
                }
            }
        }
    }
    ...
    if (player && this != pVictim) if (OutdoorPvP * pvp = player->GetOutdoorPvP()) pvp->HandleKill(
            player, pVictim);
    ...
    if (player && player->InBattleground())
    {
        ...
        {
            if (pVictim->GetTypeId() == TYPEID_PLAYER) bg->HandleKillPlayer(
                    (Player*) pVictim, player);
            else bg->HandleKillUnit(pVictim->ToCreature(), player);
        }
    }
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER)
    {
        if (GetTypeId() == TYPEID_UNIT) pVictim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE, GetEntry());
        else if (GetTypeId() == TYPEID_PLAYER && pVictim != this) pVictim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(
    ...
    if (apply)
    {
        ...
        switch (state)
        {
            case UNIT_STAT_STUNNED:
                ...
                break;
            case UNIT_STAT_ROOT:
                if (!HasUnitState(UNIT_STAT_STUNNED)) SetRooted(true);
                break;
    ...
    else
    {
        switch (state)
        {
            case UNIT_STAT_STUNNED:
                ...
                break;
            case UNIT_STAT_ROOT:
                if (HasAuraType(SPELL_AURA_MOD_ROOT) || GetVehicle()) return;
        ...
        if (HasUnitState(UNIT_STAT_STUNNED)) SetStunned(true);
        else
        {
            if (HasUnitState(UNIT_STAT_ROOT)) SetRooted(true);

            if (HasUnitState(UNIT_STAT_CONFUSED)) SetConfused(true);
            else if (HasUnitState(UNIT_STAT_FLEEING)) SetFeared(true);
        }
    }
}
...
void Unit::SetFeared(bool apply)
{
    if (apply)
    {
        ...
        Unit::AuraEffectList const& fearAuras = GetAuraEffectsByType(
                SPELL_AURA_MOD_FEAR);
        if (!fearAuras.empty()) caster = ObjectAccessor::GetUnit(*this,
                fearAuras.front()->GetCasterGUID());
        if (!caster) caster = getAttackerForHelper();
    ...
    else
    {
        if (isAlive())
        {
            if (GetMotionMaster()->GetCurrentMovementGeneratorType()
                    == FLEEING_MOTION_TYPE) GetMotionMaster()->MovementExpired();
            if (getVictim()) SetUInt64Value(UNIT_FIELD_TARGET,
                    getVictim()->GetGUID());
        }
    }

    if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->SetClientControl(this,
            !apply);
}
...
void Unit::SetConfused(bool apply)
{
    ...
    else
    {
        ...
        {
            if (GetMotionMaster()->GetCurrentMovementGeneratorType()
                    == CONFUSED_MOTION_TYPE) GetMotionMaster()->MovementExpired();
            if (getVictim()) SetUInt64Value(UNIT_FIELD_TARGET,
                    getVictim()->GetGUID());
        }
    }
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type,
        AuraApplication const * aurApp)
{
    if (!charmer) return false;

    // unmount players when charmed
    if (GetTypeId() == TYPEID_PLAYER) Unmount();
    ...
    if (aurApp && aurApp->GetRemoveMode()) return false;

    // Set charmed
    Map* pMap = GetMap();
    if (!IsVehicle() || (IsVehicle() && pMap && !pMap->IsBattleground())) setFaction(
            charmer->getFaction());
    ...
    {
        if (this->ToPlayer()->isAFK()) this->ToPlayer()->ToggleAFK();
    ...
    if (type != CHARM_TYPE_VEHICLE && !GetCharmInfo())
    {
        ...
        if (type == CHARM_TYPE_POSSESS) charmInfo->InitPossessCreateSpells();
        else charmInfo->InitCharmCreateSpells();
    }
            ...
            case CHARM_TYPE_CHARM:
                        ...
                        && charmer->getClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo()) GetCharmInfo()->SetPetNumber(
...
//        ASSERT(false);
    ...
    CharmType type;
    if (HasUnitState(UNIT_STAT_POSSESSED)) type = CHARM_TYPE_POSSESS;
    else if (charmer->IsOnVehicle(this)) type = CHARM_TYPE_VEHICLE;
    else type = CHARM_TYPE_CHARM;
    ...
    DeleteThreatList();
    Map* pMap = GetMap();
    if (!IsVehicle() || (IsVehicle() && pMap && !pMap->IsBattleground())) RestoreFaction();
    ...
    if (GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (type != CHARM_TYPE_VEHICLE) //Vehicles' AI is never modified
        {
            ...
            if (this->ToCreature()->AI() && charmer && charmer->isAlive()) this->ToCreature()->AI()->AttackStart(
                    charmer);
        }
    }
    else this->ToPlayer()->SetClientControl(this, 1);
            ...
            case CHARM_TYPE_CHARM:
                        ...
                        && charmer->getClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo()) GetCharmInfo()->SetPetNumber(0,
                                true);
                        else
                        sLog->outError(
    ...
    if (charmer->GetTypeId() == TYPEID_PLAYER
            && this != charmer->GetFirstControlled()) charmer->ToPlayer()->SendRemoveControlBar();
    else if (GetTypeId() == TYPEID_PLAYER
            || (GetTypeId() == TYPEID_UNIT && !this->ToCreature()->isGuardian())) DeleteCharmInfo();
}

void Unit::RestoreFaction()
{
    if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->setFactionForRace(
            getRace());
    else
    {
        if (HasUnitTypeMask(UNIT_MASK_MINION))
...
{
    if (Unit *veh = GetVehicleBase()) if (Creature *c = veh->ToCreature()) return c;
...
{
    if (GetVehicle()) return GetVehicle()->GetBase()->GetGUID();
    if (GetTransport()) return GetTransport()->GetGUID();
...
bool Unit::IsInPartyWith(Unit const *unit) const
{
    ...
    if (u1->GetTypeId() == TYPEID_PLAYER && u2->GetTypeId() == TYPEID_PLAYER) return u1->ToPlayer()->IsInSameGroupWith(
            u2->ToPlayer());
    else return false;
}
...
bool Unit::IsInRaidWith(Unit const *unit) const
{
    ...
    if (u1->GetTypeId() == TYPEID_PLAYER && u2->GetTypeId() == TYPEID_PLAYER) return u1->ToPlayer()->IsInSameRaidWith(
            u2->ToPlayer());
    else return false;
}
...
void Unit::GetPartyMemberInDist(std::list <Unit*> &TagUnitMap, float radius)
{
    ...
    Group *pGroup = NULL;
    if (owner->GetTypeId() == TYPEID_PLAYER) pGroup =
            owner->ToPlayer()->GetGroup();
    ...
    if (pGroup)
    {
                ...
                itr->next())
        {
            ...
            if (Target && Target->GetSubGroup() == subgroup
                    && !IsHostileTo(Target))
            {
                if (Target->isAlive() && IsWithinDistInMap(Target, radius)) TagUnitMap.push_back(
                        Target);

                if (Guardian* pet = Target->GetGuardianPet()) if (pet->isAlive()
                        && IsWithinDistInMap(pet, radius)) TagUnitMap.push_back(
                        pet);
            }
        }
    }
    ...
    {
        if (owner->isAlive()
                && (owner == this || IsWithinDistInMap(owner, radius))) TagUnitMap.push_back(
                owner);
        if (Guardian* pet = owner->GetGuardianPet()) if (pet->isAlive()
                && (pet == this && IsWithinDistInMap(pet, radius))) TagUnitMap.push_back(
                pet);
    }
}
    ...
    if (pGroup)
    {
                ...
                itr->next())
        {
            ...
            if (Target && Target->GetSubGroup() == subgroup
                    && !IsHostileTo(Target))
            {
                if (Target->isAlive() && IsInMap(Target)) TagUnitMap.push_back(
                        Target);

                if (Guardian* pet = Target->GetGuardianPet()) if (pet->isAlive()
                        && IsInMap(Target)) TagUnitMap.push_back(pet);
            }
        }
    }
    else
    {
        if (owner->isAlive() && (owner == this || IsInMap(owner))) TagUnitMap.push_back(
                owner);
        if (Guardian* pet = owner->GetGuardianPet()) if (pet->isAlive()
                && (pet == this || IsInMap(pet))) TagUnitMap.push_back(pet);
    }
}
...
Aura * Unit::AddAura(uint32 spellId, Unit *target)
{
    ...
    if (!target->isAlive() && !(spellInfo->Attributes & SPELL_ATTR0_PASSIVE)
            && !(spellInfo->AttributesEx2 & SPELL_ATTR2_ALLOW_DEAD_TARGET)) return NULL;
...
Aura * Unit::AddAura(SpellEntry const *spellInfo, uint8 effMask, Unit *target)
{
    ...
    if (target->IsImmunedToSpell(spellInfo)) return NULL;

    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (!(effMask & (1 << i))) continue;
        if (target->IsImmunedToSpellEffect(spellInfo, i)) effMask &= ~(1 << i);
    }
...
void Unit::SetAuraStack(uint32 spellId, Unit *target, uint32 stack)
{
    ...
    if (!aura) aura = AddAura(spellId, target);
    if (aura && stack) aura->SetStackAmount(stack);
}

void Unit::ApplyResilience(const Unit *pVictim, int32 *damage) const
{
    if (IsVehicle() || pVictim->IsVehicle()) return;
    ...
    if (!source)
    {
        ...
        if (source)
        {
            ...
            if (source) source = source->ToPlayer();
        }
    }
...
{
    if (newPhaseMask == GetPhaseMask()) return;

    if (IsInWorld()) RemoveNotOwnSingleTargetAuras(newPhaseMask); // we can lost access to caster or target
    ...
    for (ControlList::const_iterator itr = m_Controlled.begin();
            itr != m_Controlled.end(); ++itr)
        if ((*itr)->GetTypeId() == TYPEID_UNIT) (*itr)->SetPhaseMask(
                newPhaseMask, true);

    for (uint8 i = 0; i < MAX_SUMMON_SLOT; ++i)
        if (m_SummonSlot [i]) if (Creature *summon = GetMap()->GetCreature(m_SummonSlot[i])) summon->SetPhaseMask(
                newPhaseMask, true);
}

void Unit::UpdateObjectVisibility(bool forced)
{
    if (!forced) AddToNotify(NOTIFY_VISIBILITY_CHANGED);
    else
    {
...
void Unit::KnockbackFrom(float x, float y, float speedXY, float speedZ)
{
    ...
    else
    {
        ...
        if (player && player->m_mover != this) player = NULL;
    }
...
{
    if (GetTypeId() == TYPEID_PLAYER) return ((Player const*) this)->GetRatingBonusValue(
            cr);
    else if (((Creature const*) this)->isPet())
    {
        // Player's pet get resilience from owner
        if (Unit* owner = GetOwner()) if (owner->GetTypeId() == TYPEID_PLAYER) return ((Player*) owner)->GetRatingBonusValue(
                cr);
    }
...
uint32 Unit::GetModelForForm(ShapeshiftForm form)
{
    switch (form)
    {
        case FORM_CAT:
            ...
            else if (Player::TeamForRace(getRace()) == ALLIANCE) return 892;
            else return 8571;
        case FORM_DIREBEAR:
        case FORM_BEAR:
            // Based on Hair color
            ...
            else if (Player::TeamForRace(getRace()) == ALLIANCE) return 2281;
            else return 2289;
        case FORM_FLIGHT:
            switch (getRace())
            {
        ...
        default:
        {
            ...
            if (formEntry && formEntry->modelID_A)
            {
                ...
                if (GetTypeId() != TYPEID_PLAYER) return formEntry->modelID_A;
                else
                {
                    if (Player::TeamForRace(getRace()) == ALLIANCE) modelid =
                            formEntry->modelID_A;
                    else modelid = formEntry->modelID_H;

                    // If the player is horde but there are no values for the horde modelid - take the alliance modelid
                    if (!modelid && Player::TeamForRace(getRace()) == HORDE) modelid =
                            formEntry->modelID_A;
                }
            }
            return modelid;
...
void Unit::JumpTo(float speedXY, float speedZ, bool forward)
{
    ...
    if (GetTypeId() == TYPEID_UNIT) GetMotionMaster()->MoveJumpTo(angle,
            speedXY, speedZ);
    else
    {
...
{
    if (!isAlive() || GetVehicleKit() == vehicle) return;
    ...
    if (Player* plr = ToPlayer())
    {
        ...
        if (Battleground *bg = plr->GetBattleground()) bg->EventPlayerDroppedFlag(
                plr);
    }
...
void Unit::ChangeSeat(int8 seatId, bool next, bool byAura)
{
    ...
    else if (seatId == GetTransSeat() || !m_vehicle->HasEmptySeat(seatId)) return;
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
    ...
    {
        case TYPEID_UNIT:
            if (canFly()) const_cast <Unit*>(this)->AddUnitMovementFlag(
                    MOVEMENTFLAG_LEVITATING);
            if (IsVehicle()) const_cast <Unit*>(this)->AddExtraUnitMovementFlag(
    ...
    if (Vehicle* pVehicle = GetVehicle()) if (!HasUnitMovementFlag(
            MOVEMENTFLAG_ROOT))
    sLog->outError(
    ...
    *data << GetOrientation();
    ...
    {
        if (m_vehicle) data->append(m_vehicle->GetBase()->GetPackGUID());
        else if (GetTransport()) data->append(GetTransport()->GetPackGUID());
        else *data << (uint8) 0;
    ...
    if ((GetUnitMovementFlags() & (MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_FLYING))
            || (m_movementInfo.flags2 & MOVEMENTFLAG2_ALWAYS_ALLOW_PITCHING)) *data
            << (float) m_movementInfo.pitch;
    ...
    // 0x04000000
    if (GetUnitMovementFlags() & MOVEMENTFLAG_SPLINE_ELEVATION) *data
            << (float) m_movementInfo.splineElevation;
}
...
void Unit::NearTeleportTo(float x, float y, float z, float orientation,
        bool casting /*= false*/)
{
    if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->TeleportTo(
            GetMapId(),
            x,
            y,
            z,
        ...
        bool teleport)
{
    ...
    if (turn) RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_TURNING);
    ...
    if (relocated)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER) GetMap()->PlayerRelocation(
                (Player*) this, x, y, z, orientation);
        else GetMap()->CreatureRelocation(this->ToCreature(), x, y, z,
                orientation);
    }
    else if (turn) SetOrientation(orientation);

    if ((relocated || turn) && IsVehicle()) GetVehicleKit()->RelocatePassengers(
            x, y, z, orientation);

    return (relocated || turn);
}

bool Unit::UpdatePosition(float x, float y, float z, float orientation,
        bool teleport)
{
    // prevent crash when a bad coord is sent by the client
    if (!Trinity::IsValidMapCoord(x, y, z, orientation))
    {
        sLog->outDebug(LOG_FILTER_UNITS,
                "Unit::UpdatePosition(%f, %f, %f) .. bad coordinates!", x, y,
                z);
        return false;
    }
    ...
    if (relocated)
    {
        ...
        // move and update visible state if need
        if (GetTypeId() == TYPEID_PLAYER) GetMap()->PlayerRelocation(ToPlayer(),
                x, y, z, orientation);
        else GetMap()->CreatureRelocation(ToCreature(), x, y, z, orientation);
    }
    else if (turn) SetOrientation(orientation);
...
void Unit::StopAttackFaction(uint32 faction_id)
{
    if (Unit* victim = getVictim())
    {
        if (victim->getFactionTemplateEntry()->faction == faction_id)
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER) this->ToPlayer()->SendAttackSwingCancelAttack();
        }
    }
...
uint32 Unit::GetResistance(SpellSchoolMask mask) const
{
    ...
    for (int i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i)
        if (mask & (1 << i)
                && (resist < 0 || resist > int32(GetResistance(SpellSchools(i))))) resist =
                int32(GetResistance(SpellSchools(i)));
        ...
        uint8 effectIndex) const
{
            ...
            i != DoTAuras.end(); ++i)
    {
        if ((*i)->GetCasterGUID() != caster || (*i)->GetId() != spellId
                || (*i)->GetEffIndex() != effectIndex) continue;
        amount += ((*i)->GetAmount()
...
bool Unit::IsVisionObscured(Unit* pVictim)
{
    ...
    if ((myAura != NULL && myCaster == NULL)
            || (victimAura != NULL && victimCaster == NULL)) return false; // Failed auras, will result in crash
    ...
    {
        if (IsFriendlyTo(victimCaster)) return false;
        else return true;
    }
    ...
    {
        if (IsFriendlyTo(myCaster)) return false;
        else return true;
    }
    return false;
}
...
void Unit::SetEclipsePower(int32 power)
{
    ...
    if (eclipse == 0)
    {
        if (HasAura(67483)) RemoveAurasDueToSpell(67483);
        if (HasAura(67484)) RemoveAurasDueToSpell(67484);
    }
