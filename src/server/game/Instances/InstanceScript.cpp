...
void InstanceScript::HandleGameObject(uint64 GUID, bool open, GameObject *go) {
	...
	if (go)
		...
		sLog->outDebug(LOG_FILTER_TSCR,
				"TSCR: InstanceScript: HandleGameObject failed");
}
...
void InstanceScript::LoadMinionData(const MinionData *data) {
	while (data->entry) {
		if (data->bossId < bosses.size())
			minions.insert(
					std::make_pair(data->entry,
							MinionInfo(&bosses[data->bossId])));

		++data;
	}
	sLog->outDebug(LOG_FILTER_TSCR,
			"InstanceScript::LoadMinionData: " UI64FMTD " minions loaded.",
			uint64(minions.size()));
}
	...
	while (data->entry) {
		if (data->bossId < bosses.size())
			doors.insert(
					std::make_pair(
							data->entry,
							DoorInfo(&bosses[data->bossId], data->type,
									BoundaryType(data->boundary))));

		++data;
	}
	sLog->outDebug(LOG_FILTER_TSCR,
			"InstanceScript::LoadDoorData: " UI64FMTD " doors loaded.",
			uint64(doors.size()));
}
...
void InstanceScript::AddDoor(GameObject *door, bool add) {
	...
	for (DoorInfoMap::iterator itr = lower; itr != upper; ++itr) {
		if (add) {
			...
			case BOUNDARY_N:
			case BOUNDARY_S:
				itr->second.bossInfo->boundary[itr->second.boundary] =
						door->GetPositionX();
				break;
			case BOUNDARY_E:
			case BOUNDARY_W:
			...
			case BOUNDARY_SE:
				itr->second.bossInfo->boundary[itr->second.boundary] =
						door->GetPositionX() + door->GetPositionY();
				break;
			case BOUNDARY_NE:
			case BOUNDARY_SW:
				itr->second.bossInfo->boundary[itr->second.boundary] =
...
void InstanceScript::DoUseDoorOrButton(uint64 uiGuid, uint32 uiWithRestoreTime,
	...
	if (pGo) {
		if (pGo->GetGoType() == GAMEOBJECT_TYPE_DOOR
			...
			sLog->outError(
					"SD2: Script call DoUseDoorOrButton, but gameobject entry %u is type %u.",
					pGo->GetEntry(), pGo->GetGoType());
	}
}

void InstanceScript::DoRespawnGameObject(uint64 uiGuid,
...
void InstanceScript::DoUpdateWorldState(uint32 uiStateId, uint32 uiStateData) {
	...
	if (!lPlayers.isEmpty()) {
		...
		sLog->outDebug(
				LOG_FILTER_TSCR,
				"TSCR: DoUpdateWorldState attempt send data but no players in map.");
}
...
void InstanceScript::DoCompleteAchievement(uint32 achievement) {
	AchievementEntry const* pAE = GetAchievementStore()->LookupEntry(
			achievement);
	Map::PlayerList const &PlayerList = instance->GetPlayers();

	if (!pAE) {
		sLog->outError(
				"TSCR: DoCompleteAchievement called for not existing achievement %u",
				achievement);
		return;
	}
...
void InstanceScript::DoUpdateAchievementCriteria(AchievementCriteriaTypes type,
		uint32 miscValue1 /*= 0*/, uint32 miscValue2 /*= 0*/,
		Unit* unit /*= NULL*/) {
	Map::PlayerList const &PlayerList = instance->GetPlayers();
...
void InstanceScript::DoStopTimedAchievement(AchievementCriteriaTimedTypes type,
	...
	if (!PlayerList.isEmpty())
		for (Map::PlayerList::const_iterator i = PlayerList.begin();
				i != PlayerList.end(); ++i)
			if (Player *pPlayer = i->getSource())
				pPlayer->GetAchievementMgr().RemoveTimedAchievement(type,
						entry);
}
...
bool InstanceScript::CheckAchievementCriteriaMeet(uint32 criteria_id,
		Player const* /*source*/, Unit const* /*target*//*= NULL*/,
		uint32 /*miscvalue1*//*= 0*/) {
	sLog->outError(
			"Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map %u not have implementation for achievement criteria %u",
					...
					for (Map::PlayerList::const_iterator i = players.begin();
						...
						if (Player* player = i->getSource())
							if (!source
								...
								sLFGMgr->RewardDungeonDoneFor(dungeonId,
										player);
			}
			return;
		}
