...
enum SpellEffIndex {
    EFFECT_0 = 0, EFFECT_1 = 1, EFFECT_2 = 2,
};
...
enum Gender {
    GENDER_MALE = 0, GENDER_FEMALE = 1, GENDER_NONE = 2
};
...
enum MoneyConstants {
    COPPER = 1, SILVER = COPPER * 100, GOLD = SILVER * 100
};
...
const uint32 ItemQualityColors[MAX_ITEM_QUALITY] = { 0xff9d9d9d, //GREY
        0xffffffff, //WHITE
        0xff1eff00, //GREEN
        0xff0070dd, //BLUE
        0xffa335ee, //PURPLE
...
enum SpellAttr0 {
    ...
    SPELL_ATTR0_CANT_CANCEL = 0x80000000
// 31 positive aura can't be canceled
};
...
enum SpellAttr2 {
    ...
    SPELL_ATTR2_FOOD_BUFF = 0x80000000
// 31 Food or Drink Buff (like Well Fed)
};
...
enum SpellAttr4 {
    ...
    SPELL_ATTR4_PROC_ONLY_ON_DUMMY = 0x00000002, //  1 proc on finishing move?
...
// 31 Forces all nearby enemies to focus attacks caster
};
...
enum SpellAttr7 {
    ...
    SPELL_ATTR7_ZONE_TELEPORT = 0x00080000
// 19 Teleports to specific zones.
};
...
// used by addons, in 2.4.0 not exist, replaced by messagetype?
};

#define LANGUAGES_COUNT   21

enum TeamId {
    TEAM_ALLIANCE = 0, TEAM_HORDE, TEAM_NEUTRAL,
};

enum Team {
    HORDE = 67, ALLIANCE = 469,
    ...
    //TEAM_OUTLAND             = 980,
    TEAM_OTHER = 0,
// if ReputationListId > 0 && Flags != FACTION_FLAG_TEAM_HEADER
};
...
enum SpellEffects 
{
    ...
    SPELL_EFFECT_SUMMON_ARCHAEOLOGY_TELESCOPE = 105,
...
enum SpellCustomErrors {
    SPELL_CUSTOM_ERROR_NONE = 0, SPELL_CUSTOM_ERROR_CUSTOM_MSG = 1, // Something bad happened, and we want to display a custom message!
...
enum StealthType {
    STEALTH_GENERAL = 0, STEALTH_TRAP = 1,
...
enum ServerSideVisibilityType {
    SERVERSIDE_VISIBILITY_GM = 0, SERVERSIDE_VISIBILITY_GHOST = 1,

    TOTAL_SERVERSIDE_VISIBILITY_TYPES = 2
};

enum GhostVisibilityType {
    GHOST_VISIBILITY_ALIVE = 0x1, GHOST_VISIBILITY_GHOST = 0x2
};

// Spell aura states
enum AuraState { // (C) used in caster aura state     (T) used in target aura state
...
enum GameObjectFlags {
    ...
    GO_FLAG_UNK1 = 0x00000010, // GO_FLAG_NOT_SELECTABLE
...
enum LockKeyType {
    LOCK_KEY_NONE = 0, LOCK_KEY_ITEM = 1, LOCK_KEY_SKILL = 2
};
...
{
    TRAINER_TYPE_CLASS = 0, TRAINER_TYPE_MOUNTS = 1, // on blizz it's 2
...
uint32 const CREATURE_TYPEMASK_HUMANOID_OR_UNDEAD = (1
        << (CREATURE_TYPE_HUMANOID - 1)) | (1 << (CREATURE_TYPE_UNDEAD - 1));
uint32 const CREATURE_TYPEMASK_MECHANICAL_OR_ELEMENTAL = (1
        << (CREATURE_TYPE_MECHANICAL - 1))
...
enum CreatureTypeFlags {
    ...
    CREATURE_TYPEFLAGS_UNK24 = 0x800000
// ? First seen in 3.2.2. Related to banner/backpack of creature/companion?
};
...
enum DiminishingReturnsType {
    ...
    DRTYPE_ALL = 2
// this spell is diminished in every case
};
...
enum EventId {
    EVENT_SPELLCLICK = 1001, EVENT_FALL_GROUND = 1002, EVENT_CHARGE = 1003,
};
...
/// Ban function modes
enum BanMode {
    BAN_ACCOUNT, BAN_CHARACTER, BAN_IP
};

/// Ban function return codes
enum BanReturn {
    BAN_SUCCESS, BAN_SYNTAX_ERROR, BAN_NOTFOUND
};
...
enum SpellFamilyNames {
    SPELLFAMILY_GENERIC = 0, SPELLFAMILY_UNK1 = 1, // events, holidays
    // 2 - unused
    SPELLFAMILY_MAGE = 3,
...
// Related to trading soulbound loot items
};

enum XPColorChar {
    XP_RED, XP_ORANGE, XP_YELLOW, XP_GREEN, XP_GRAY
};
