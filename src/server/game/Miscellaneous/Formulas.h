...
inline uint32 BaseGain(uint8 pl_level, uint8 mob_level, ContentLevels content) {
    ...
    if (mob_level >= pl_level) {
        ...
        if (mob_level > gray_level) {
            ...
            baseGain = (pl_level * 5 + nBaseExp) * (ZD + mob_level - pl_level)
                    / ZD;
        } else
            baseGain = 0;
    }
...
inline uint32 Gain(Player *pl, Unit *u) {
    ...
    if (u->GetTypeId() == TYPEID_UNIT
            && (((Creature*) u)->isTotem() || ((Creature*) u)->isPet()
                    ...
                    || ((Creature*) u)->GetCreatureInfo()->type
                            == CREATURE_TYPE_CRITTER))
        gain = 0;
    else {
        gain = BaseGain(pl->getLevel(), u->getLevel(),
                GetContentLevelsForMapAndZone(u->GetMapId(), u->GetZoneId()));

        if (gain != 0 && u->GetTypeId() == TYPEID_UNIT
                && ((Creature*) u)->isElite()) {
